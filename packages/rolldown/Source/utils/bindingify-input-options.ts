import {
	BindingLogLevel,
	type BindingDeferSyncScanData,
	type BindingInjectImportNamed,
	type BindingInjectImportNamespace,
	...
	return {
		...
		experimental: {
			strictExecutionOrder:
				inputOptions.experimental?.strictExecutionOrder,
			disableLiveBindings: inputOptions.experimental?.disableLiveBindings,
			viteMode: inputOptions.experimental?.viteMode,
			resolveNewUrlToAsset:
				inputOptions.experimental?.resolveNewUrlToAsset,
		...
		deferSyncScanData: () => {
			...
			pluginContextData.moduleOptionMap.forEach((value, key) => {
				if (value.invalidate) {
					ret.push({
						id: key,
						sideEffects: bindingifySideEffects(
							value.moduleSideEffects,
						),
					});
				}
			});
