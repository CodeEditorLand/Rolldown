				...
				v.args(
					v.tuple([
						v.union([
							...
							v.pipe(
								v.function(),
								v.returns(RollupLogWithStringSchema),
							),
						]),
					]),
				),
...
const OutputOptionsSchema = v.strictObject({
	dir: v.pipe(
		v.optional(v.string()),
		v.description(
			"Output directory, defaults to `dist` if `file` is not set",
		),
	),
...
const CliOptionsSchema = v.strictObject({
	config: v.pipe(
		...
		v.description(
			"Path to the config file (default: `rolldown.config.js`)",
		),
	),
			...
			.map((issue) => {
				...
				if (issue.type === "union") {
					...
					if (subIssue) {
						if (subIssue.path) {
							issuePaths.push(
								subIssue.path.map((path) => path.key),
							);
						}
