...
function createComposedPlugin(plugins: Plugin[]): Plugin {
	...
	function applyFixedPluginResolveFn(ctx: PluginContext, plugin: Plugin) {
		const createFixedPluginResolveFn =
			createFixedPluginResolveFnMap.get(plugin);

		if (createFixedPluginResolveFn) {
			ctx.resolve = createFixedPluginResolveFn(
				ctx,
				ctx.resolve.bind(ctx),
			);
		}
	...
	if (batchedHooks.resolveId) {
		...
		for (
			...
			createFixedPluginResolveFnMap.set(
				plugin,
				createFixedPluginResolveFn,
			);
		}
		...
		switch (hookName) {
			...
			case "transform": {
				if (batchedHooks.transform) {
					...
					composed.transform = async function (
						initialCode,
						id,
						moduleType,
					) {
						let code = initialCode;
						let moduleSideEffects: ModuleSideEffects | undefined =
							undefined;
						...
						function updateOutput(
							...
							code = newCode;
							moduleSideEffects =
								newModuleSideEffects ?? undefined;
						}
						for (const [handler, plugin] of batchedHandlers) {
							const { handler: handlerFn } =
								normalizeHook(handler);
			...
			case "renderChunk": {
				if (batchedHooks.renderChunk) {
					...
					composed.renderChunk = async function (
						code,
						chunk,
						options,
						meta,
