...
export class PluginContextImpl extends MinimalPluginContextImpl {
	...
	constructor(
		...
		this.getModuleInfo = (id: string) =>
			this.data.getModuleInfo(id, context);
	}
	...
	public async load(
		...
		if (
			moduleInfo &&
			moduleInfo.code !== null /* module already parsed */
		) {
			return moduleInfo;
		}
	...
	public emitFile: PluginContext["emitFile"] = (file): string => {
		...
		if (file.type === "prebuilt-chunk") {
			return unimplemented(
				"PluginContext.emitFile with type prebuilt-chunk",
			);
		}
		if (file.type === "chunk") {
		...
		const fnSanitizedFileName =
			file.fileName ||
			typeof this.outputOptions.sanitizeFileName !== "function"
				? undefined
				: this.outputOptions.sanitizeFileName!(file.name || "asset");
		const filename = file.fileName
			? undefined
			: this.getAssetFileNames(file);
