...
pub fn parse_pattern(pattern: &str) -> DynamicImportPattern {
	...
	DynamicImportPattern {
		glob_params: if search.is_empty() {
			...
			Some(DynamicImportRequest { query: search.to_string(), import: has_special_query_param(search) })
		},
...
mod tests {
	...
	fn with_query() {
		assert_eq!(
			...
			super::DynamicImportPattern {
				glob_params: Some(super::DynamicImportRequest { query: "?foo=bar".to_string(), import: false }),
				user_pattern: "./mods/*.js".to_string(),
				raw_pattern: "./mods/*.js".to_string(),
			}
		);
	...
	fn with_special_query() {
		assert_eq!(
			...
			super::DynamicImportPattern {
				glob_params: Some(super::DynamicImportRequest { query: "?foo=bar&raw".to_string(), import: true }),
				user_pattern: "./mods/*.js".to_string(),
				raw_pattern: "./mods/*.js".to_string(),
			}
		);
		...
		assert_eq!(
			...
			super::DynamicImportPattern {
				glob_params: Some(super::DynamicImportRequest { query: "?worker&c=d".to_string(), import: true }),
				user_pattern: "./mods/*.js".to_string(),
				raw_pattern: "./mods/*.js".to_string(),
			}
		);
