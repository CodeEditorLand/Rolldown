...
use anyhow::Ok;
use oxc::ast::ast::{
	Argument, BindingPattern, BindingPatternKind, CallExpression, Expression, ImportOrExportKind, PropertyKey,
	Statement, StaticMemberExpression, VariableDeclaration, VariableDeclarationKind,
};
...
use rolldown_plugin::{
	...
	HookTransformAstArgs, HookTransformAstReturn, Plugin, PluginContext,
};
...
impl Plugin for BuildImportAnalysisPlugin {
	...
	async fn transform_ast(&self, _ctx: &PluginContext, args: HookTransformAstArgs<'_>) -> HookTransformAstReturn {
...
impl<'a> BuildImportAnalysisVisitor<'a> {
	pub fn new(builder: AstBuilder<'a>, insert_preload: bool, render_built_url: bool, is_relative_base: bool) -> Self {
		Self {
			builder,
	...
	fn rewrite_paren_member_expr(&mut self, member_expr: &mut StaticMemberExpression<'a>) {
		...
		let Expression::ImportExpression(ref import_expr) = expr.argument else {
			return;
		};
...
impl<'a> VisitMut<'a> for BuildImportAnalysisVisitor<'a> {
	...
	fn enter_scope(&mut self, flags: ScopeFlags, _scope_id: &std::cell::Cell<Option<oxc::semantic::ScopeId>>) {
