...
use anyhow::Ok;
use oxc::ast::ast::{
	Argument, BindingPattern, BindingPatternKind, CallExpression, Expression,
	ImportOrExportKind, PropertyKey, StaticMemberExpression,
...
use self::utils::{
	construct_snippet_for_expression, construct_snippet_from_await_decl,
};
mod utils;
...
impl<'a> BuildImportAnalysisVisitor<'a> {
	...
	fn is_top_level(&self) -> bool {
		self.scope_stack
			.last()
			.map_or(false, |flags| flags.contains(ScopeFlags::Top))
	}

	/// ```js
	/// (await import('foo')).foo
	/// ```
	fn rewrite_paren_member_expr(
		&mut self,
		member_expr: &mut StaticMemberExpression<'a>,
	) {
		let Expression::ParenthesizedExpression(ref mut paren) =
			member_expr.object
		else {
			return;
		};
		...
		let Expression::ImportExpression(ref import_expr) = expr.argument
		else {
			return;
		};
	...
	fn rewrite_import_expr(&mut self, expr: &mut CallExpression<'a>) {
		let Expression::StaticMemberExpression(ref mut callee) = expr.callee
		else {
			return;
		};
		let Expression::ImportExpression(ref import_expr) = callee.object
		else {
			return;
		};
		let source = match &import_expr.source {
		...
		let [Argument::ArrowFunctionExpression(arrow_expr)] =
			expr.arguments.as_slice()
		else {
			return;
		};
		...
		let BindingPatternKind::ObjectPattern(ref pat) =
			first_param.pattern.kind
		else {
			return;
		};
...
impl<'a> VisitMut<'a> for BuildImportAnalysisVisitor<'a> {
			...
			let helper_stmt = self.builder.statement_module_declaration(
				self.builder.module_declaration_import_declaration(
					...
					Some(
						self.builder.vec1(
							self.builder
								.import_declaration_specifier_import_specifier(
									SPAN,
									self.builder
										.module_export_name_identifier_name(
											SPAN,
											PRELOAD_METHOD,
										),
									self.builder.binding_identifier(
										SPAN,
										PRELOAD_METHOD,
									),
	...
	fn visit_variable_declaration(
		...
		let mut declarators_map = decl
			...
			.enumerate()
			.filter_map(|(i, decl)| {
				let Some(Expression::AwaitExpression(ref mut init)) = decl.init
				else {
					return None;
				};
				let Expression::ImportExpression(ref mut import) =
					init.argument
				else {
					return None;
				};
				...
				let source = match &import.source {
							...
							&& lit.expressions.is_empty() =>
					{
						...
						first
							...
							.cooked
							.clone()
							.unwrap_or(first.value.raw.clone())
					},
					_ => return None,
				};
	...
	fn visit_variable_declarator(
		&mut self,
		it: &mut oxc::ast::ast::VariableDeclarator<'a>,
	) {
	...
	fn visit_expression(&mut self, expr: &mut Expression<'a>) {
		...
		match expr {
			Expression::StaticMemberExpression(expr) => {
				self.rewrite_paren_member_expr(expr)
			},
