...
/// generate `__vitePreload(async () => { const {foo} = await
/// import('foo');return { foo }},...)`
		...
		NONE,
		{
			...
			items.push(ast_builder.argument_expression(ast_builder.expression_arrow_function(
				...
				ast_builder.function_body(SPAN, ast_builder.vec(), {
					let mut items = ast_builder.vec();
					items.push(ast_builder.statement_declaration(
						ast_builder.declaration_variable(
							SPAN,
							decl_kind,
									...
									ast_builder.binding_pattern_kind_object_pattern(
										...
										ast_builder.vec_from_iter(decls.iter().map(|name| {
											ast_builder.binding_property(
												SPAN,
												ast_builder
													.property_key_identifier_name(SPAN, name),
												ast_builder.binding_pattern(
													ast_builder
...
///   to `__vitePreload(async () => { const {foo} = await import('foo');return {
/// foo }},...).then(({foo})=>{})` 2.transform `(await import('foo')).foo`
///   to `__vitePreload(async () => { const {foo} = (await import('foo'));
/// return { foo }},...)).foo`
