...
impl Fixture {
	...
	async fn run_inner(self, plugins: Vec<SharedPluginable>) {
		let TestConfig { config: mut options, meta, config_variants } = read_test_config(&self.config_path);
		...
		IntegrationTest::new(meta)
			.run_multiple(configs, &self.fixture_path, plugins)
			.await;
	}
}
