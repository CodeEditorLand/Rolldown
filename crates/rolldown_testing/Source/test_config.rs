...
static COMPILED_SCHEMA: LazyLock<Validator> = LazyLock::new(|| {
	let schema_path =
		workspace::crate_dir("rolldown_testing").join("_config.schema.json");

	let schema_str = fs::read_to_string(&schema_path).unwrap_or_else(|e| {
		panic!("Failed to read schema file {schema_path:?}. Got {e:?}")
	});
	...
	Validator::options()
		...
		.unwrap_or_else(|e| {
			panic!(
				"Failed to compile {schema_path:?} to json schema. Got {e:?}"
			)
		})
});

pub fn read_test_config(config_path: &std::path::Path) -> TestConfig {
	let mut config_str = fs::read_to_string(config_path).unwrap_or_else(|e| {
		panic!("Failed to read config file in {config_path:?}. Got {e:?}")
	});

	json_strip_comments::strip(&mut config_str).unwrap_or_else(|e| {
		panic!("Failed to strip comments of {config_path:?}. Got {e:?}")
	});
