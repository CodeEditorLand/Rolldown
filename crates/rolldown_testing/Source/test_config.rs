...
static COMPILED_SCHEMA: LazyLock<Validator> = LazyLock::new(|| {
	...
	let schema_json: serde_json::Value = serde_json::from_str(&schema_str)
		.unwrap_or_else(|e| panic!("Failed to parse test config file ${schema_path:?} in json. Got {e:?}"));
...
pub fn read_test_config(config_path: &std::path::Path) -> TestConfig {
	...
	let config_json: serde_json::Value = serde_json::from_str(&config_str).expect("Failed to parse test config file");
