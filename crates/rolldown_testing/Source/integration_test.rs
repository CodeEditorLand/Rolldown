...
impl IntegrationTest {
	...
	pub async fn run_with_plugins(&self, mut options: BundlerOptions, plugins: Vec<SharedPluginable>) {
		self.apply_test_defaults(&mut options);
		...
		match bundle_output {
			Ok(bundle_output) => {
				...
				if !self.test_meta.expect_executed || self.test_meta.expect_error || !self.test_meta.write_to_disk {
					// do nothing
				} else {
	...
	fn snapshot_bundle_output(&self, bundle_output: BundleOutput, errs: Vec<BuildDiagnostic>, cwd: &Path) {
	...
	fn execute_output_assets(bundler: &Bundler, test_title: &str) {
		...
		if is_expect_executed_under_esm {
			...
			let mut json: Value = serde_json::from_str(&json_string).unwrap_or(Value::Object(Map::default()));
		...
		if !output.status.success() {
			...
			panic!("⬇️⬇️ stderr {test_title} ⬇️⬇️\n{stderr_utf8}\n⬇️⬇️ stdout ⬇️⬇️\n{stdout_utf8}\n⬆️⬆️ end  ⬆️⬆️",);
		}
	}
}
