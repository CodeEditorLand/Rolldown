...
use rolldown_plugin::{
	HookLoadArgs, HookLoadOutput, HookLoadReturn, HookResolveIdArgs, HookResolveIdOutput, HookResolveIdReturn, Plugin,
	PluginContext,
};
use rolldown_utils::{
	dashmap::FxDashMap,
		...
		async {
			if is_data_url(args.specifier) {
				...
				let decoded_data = if parsed.is_base64 {
					String::from_utf8_lossy(base64_simd::STANDARD.decode_to_vec(parsed.data)?.as_ref()).to_string()
				} else {
