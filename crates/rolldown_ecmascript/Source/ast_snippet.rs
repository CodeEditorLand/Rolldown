...
impl<'ast> AstSnippet<'ast> {
	...
	/// `name(arg1, arg2)`
	pub fn call_expr_with_2arg_expr(&self, name: PassedStr, arg1: PassedStr, arg2: PassedStr) -> ast::Expression<'ast> {
	...
	#[inline]
	/// `undefined` is acting like identifier, it might be shadowed by user
	/// code.
	...
	pub fn require_call_expr(&self, source: &str) -> Expression<'ast> {
		self.builder.expression_call(
			...
			NONE,
			self.builder
				.vec1(Argument::from(self.builder.expression_string_literal(SPAN, source))),
			false,
		)
	}
