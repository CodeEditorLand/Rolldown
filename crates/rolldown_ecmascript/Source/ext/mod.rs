...
use crate::{allocator_helpers::take_in::TakeIn, AstSnippet};
		...
		let left = match &mut self.kind {
			...
			ast::BindingPatternKind::ObjectPattern(obj_pat) => {
                ...
                .into_in(alloc)
						...
						_ => {
							unreachable!(
								"The kind of `BindingProperty`'s value should not be \
								 `ObjectPattern` and `ArrayPattern`"
							)
						},
					});
				});
			...
			ast::BindingPatternKind::ArrayPattern(arr_pat) => {
				...
				arr_pat.elements.take_in(alloc).into_iter().for_each(|binding_pat| {
					arr_target.elements.push(binding_pat.map(|binding_pat| {
			...
			ast::BindingPatternKind::AssignmentPattern(_) => {
				unreachable!(
					"`BindingPatternKind::AssignmentPattern` should be pre-handled in above"
				)
			},
		};
		left
...
pub trait StatementExt<'me, 'ast> {
	...
	fn as_export_named_declaration_mut(&mut self)
	-> Option<&mut ast::ExportNamedDeclaration<'ast>>;
...
impl<'me, 'ast> StatementExt<'me, 'ast> for ast::Statement<'ast> {
	fn is_import_declaration(&self) -> bool { matches!(self, ast::Statement::ImportDeclaration(_)) }
	...
	fn as_export_named_declaration_mut(
		&mut self,
	) -> Option<&mut ast::ExportNamedDeclaration<'ast>> {
		if let ast::Statement::ExportNamedDeclaration(export_named_decl) = self {
	...
	fn is_function_declaration(&self) -> bool { self.as_function_declaration().is_some() }

	/// Check if the statement is `[import|export] ... from ...` or `export ...
	/// from ...`
	fn is_module_declaration_with_source(&self) -> bool {
