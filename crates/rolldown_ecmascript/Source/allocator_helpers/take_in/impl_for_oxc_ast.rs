...
impl<'ast> TakeIn<'ast> for ast::VariableDeclarationKind {
	fn dummy(_alloc:&'ast Allocator) -> Self { Self::Var }
}
impl<'ast> TakeIn<'ast> for ast::ThisExpression {
	fn dummy(alloc:&'ast Allocator) -> Self { Self { span:TakeIn::dummy(alloc) } }
}
...
impl<'ast> TakeIn<'ast> for ast::FunctionType {
	fn dummy(_alloc:&'ast Allocator) -> Self { Self::FunctionDeclaration }
}

impl<'ast> TakeIn<'ast> for ast::FormalParameterKind {
	fn dummy(_alloc:&'ast Allocator) -> Self { Self::Signature }
}
...
impl<'ast> TakeIn<'ast> for ast::ClassType {
	fn dummy(_alloc:&'ast Allocator) -> Self { Self::ClassDeclaration }
}
...
impl<'ast> TakeIn<'ast> for ast::BindingPatternKind<'ast> {
	fn dummy(alloc:&'ast Allocator) -> Self { Self::BindingIdentifier(TakeIn::dummy(alloc)) }
}
...
impl<'ast> TakeIn<'ast> for Atom<'ast> {
	fn dummy(_alloc:&'ast Allocator) -> Self { Atom::from("") }
}
...
impl<'ast> TakeIn<'ast> for ast::ObjectPropertyKind<'ast> {
	fn dummy(alloc:&'ast Allocator) -> Self { Self::ObjectProperty(TakeIn::dummy(alloc)) }
}

impl<'ast> TakeIn<'ast> for ast::PropertyKind {
	fn dummy(_alloc:&'ast Allocator) -> Self { Self::Init }
}

impl<'ast> TakeIn<'ast> for ast::PropertyKey<'ast> {
	fn dummy(alloc:&'ast Allocator) -> Self { Self::Identifier(TakeIn::dummy(alloc)) }
}
...
impl<'ast> TakeIn<'ast> for ast::ImportOrExportKind {
	fn dummy(_alloc:&'ast Allocator) -> Self { Self::Value }
}
