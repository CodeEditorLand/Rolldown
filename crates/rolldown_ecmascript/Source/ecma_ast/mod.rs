...
/// - To access `&mut ast::Program`, use `ast.program.with_mut(|fields| {
///   fields.program; })`.
...
impl EcmaAst {
	pub fn source(&self) -> &ArcStr { &self.program.borrow_owner().source }

	pub fn allocator(&self) -> &Allocator { &self.program.borrow_owner().allocator }

	pub fn program(&self) -> &Program { &self.program.borrow_dependent().program }
}
...
impl Default for EcmaAst {
	fn default() -> Self { EcmaCompiler::parse("", "", SourceType::default()).unwrap() }
}
