...
self_cell!(
	/// `ProgramCell` is a wrapper of `Program` that provides a safe way to
	/// treat `Program<'ast>` as as owned value without considering the lifetime
	/// of `'ast`.
...
impl ProgramCell {
	/// Safely visit `&mut Program` and other fields in the cell within a
	/// closure.
	///
	/// ## Example
	///
