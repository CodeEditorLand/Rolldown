...
impl EcmaCompiler {
	pub fn parse(filename: &str, source: impl Into<ArcStr>, ty: SourceType) -> BuildResult<EcmaAst> {
		...
		let inner = ProgramCell::try_new(ProgramCellOwner { source: source.clone(), allocator }, |owner| {
	...
	pub fn parse_expr_as_program(filename: &str, source: impl Into<ArcStr>, ty: SourceType) -> BuildResult<EcmaAst> {
		let source: ArcStr = source.into();
		let allocator = oxc::allocator::Allocator::default();
		let inner = ProgramCell::try_new(ProgramCellOwner { source: source.clone(), allocator }, |owner| {
	...
	pub fn minify(
		...
		let program = Parser::new(&allocator, source_text, SourceType::default().with_jsx(true))
			.parse()
			.program;
