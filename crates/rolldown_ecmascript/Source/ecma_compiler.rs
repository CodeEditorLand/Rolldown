...
impl EcmaCompiler {
	pub fn parse(
		...
		let inner = ProgramCell::try_new(
			ProgramCellOwner { source: source.clone(), allocator },
			|owner| {
	...
	pub fn print(
		...
		let mut codegen = CodeGenerator::new()
			.with_capacity(ast.source().len())
			.enable_comment(
				ast.source(),
				ast.trivias.clone(),
				oxc::codegen::CommentOptions {
	...
	pub fn minify(
		...
		let program =
			Parser::new(&allocator, source_text, SourceType::default())
				.parse()
				.program;
		let program = allocator.alloc(program);
		let options =
			MinifierOptions { mangle: true, ..MinifierOptions::default() };
		...
		let codegen = if enable_sourcemap {
			codegen.enable_source_map(filename, source_text)
		} else {
			codegen
		};
