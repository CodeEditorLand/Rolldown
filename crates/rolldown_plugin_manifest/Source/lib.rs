...
impl Plugin for ManifestPlugin {
	fn name(&self) -> Cow<'static, str> { Cow::Borrowed("builtin:manifest-plugin") }
	...
	async fn generate_bundle(
		...
		let mut entry_css_asset_file_names =
			FxHashSet::with_capacity(entry_css_reference_ids.len());
		for reference_id in entry_css_reference_ids {
			if let Ok(file_name) = ctx.try_get_file_name(reference_id.as_str()) {
				...
				// It was already handled during the previous run of this
				// plugin.
			}
		}
		...
		for file in args.bundle.iter() {
			match file {
				Output::Chunk(chunk) => {
					...
					let chunk_manifest =
						Rc::new(self.create_chunk(args.bundle, chunk, name.clone()));
				...
				Output::Asset(asset) => {
					if !asset.names.is_empty() {
						...
						let asset_manifest =
							Rc::new(Self::create_asset(asset, src.clone(), is_entry));

						// If JS chunk and asset chunk are both generated from the same source file,
						// prioritize JS chunk as it contains more information
						if !manifest.get(&src).map_or(false, |m| {
							m.file.ends_with(".js")
								|| m.file.ends_with(".cjs")
								|| m.file.ends_with(".mjs")
						}) {
							manifest
								.insert(src.clone(), Rc::<ManifestChunk>::clone(&asset_manifest));
						}

						for original_file_name in &asset.original_file_names {
							if !manifest.get(original_file_name).map_or(false, |m| {
								m.file.ends_with(".js")
									|| m.file.ends_with(".cjs") || m.file.ends_with(".mjs")
							}) {
								manifest.insert(
									original_file_name.clone(),
									Rc::<ManifestChunk>::clone(&asset_manifest),
								);
							}
						}
					}
