use rolldown_common::{
	EmittedAsset, ModuleId, Output, OutputAsset, OutputChunk,
};
use rolldown_plugin::{HookNoopReturn, Plugin, PluginContext};
use rustc_hash::{FxHashMap, FxHashSet};
use serde::Serialize;
use std::{
	borrow::Cow, collections::BTreeMap, path::Path, rc::Rc, sync::LazyLock,
};
		...
		for file in bundle.iter() {
			match file {
				Output::Chunk(chunk) => {
					...
					let chunk_manifest =
						Rc::new(self.create_chunk(bundle, chunk, name.clone()));
					manifest.insert(name.clone(), chunk_manifest);
				},
				Output::Asset(asset) => {
					if let Some(name) = &asset.name {
						let asset_meta =
							file_name_to_asset_meta.remove(&asset.filename);
						...
						file_name_to_asset
							.insert(asset.filename.clone(), asset_manifest);
					}
				},
			}
		}
...
impl ManifestPlugin {
			...
			for chunk in bundle {
				if let Output::Chunk(output_chunk) = chunk {
					if output_chunk.filename == *file {
						filtered_imports
							.push(self.get_chunk_name(output_chunk));
						break;
					}
				}
			}
	...
	fn create_chunk(
		...
		ManifestChunk {
			...
			src: if chunk.facade_module_id.is_some() {
				Some(src)
			} else {
				None
			},
			...
			dynamic_imports: self
				.get_internal_imports(bundle, &chunk.dynamic_imports),
		}
	}
	fn create_asset(
		asset: &OutputAsset,
...
fn get_chunk_original_file_name(chunk: &OutputChunk, root: &str) -> String {
	if let Some(facade_module_id) = &chunk.facade_module_id {
		...
		format!(
			"_{}",
			Path::new(chunk.filename.as_str())
				.file_name()
				.unwrap()
				.to_string_lossy()
		)
