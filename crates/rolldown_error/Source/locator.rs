...
impl ByteLocator {
	pub fn new(source:&str) -> Self { Self { line_starts:line_starts(source).collect() } }

	/// line: 0-based
	/// column: 0-based
	pub fn byte_offset(&self, line:usize, column:usize) -> usize { self.line_starts[line] + column }
}

/// Creating a ByteLocator is none trivial, if you want to query offset multiple
/// times for same source, please creating a `ByteLocator` and calling
/// `byte_offset`
