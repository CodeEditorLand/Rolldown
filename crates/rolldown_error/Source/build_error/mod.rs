...
impl BuildDiagnostic {
	pub fn kind(&self) -> crate::event_kind::EventKind { self.inner.kind() }
...
impl From<std::io::Error> for BuildDiagnostic {
	fn from(e:std::io::Error) -> Self { Self::new_inner(e) }
}

#[cfg(feature = "napi")]
impl From<napi::Error> for BuildDiagnostic {
	fn from(e:napi::Error) -> Self { BuildDiagnostic::napi_error(e) }
}

impl From<BuildDiagnostic> for BatchedBuildDiagnostic {
	fn from(v:BuildDiagnostic) -> Self { Self::new(vec![v]) }
}

impl From<anyhow::Error> for BatchedBuildDiagnostic {
	fn from(err:anyhow::Error) -> Self { Self::new(vec![BuildDiagnostic::unhandleable_error(err)]) }
}

impl From<Vec<BuildDiagnostic>> for BatchedBuildDiagnostic {
	fn from(v:Vec<BuildDiagnostic>) -> Self { Self::new(v) }
}

impl From<anyhow::Error> for BuildDiagnostic {
	fn from(err:anyhow::Error) -> Self { BuildDiagnostic::unhandleable_error(err) }
}
...
impl BatchedBuildDiagnostic {
	pub fn new(vec:Vec<BuildDiagnostic>) -> Self { Self(vec) }

	pub fn into_vec(self) -> Vec<BuildDiagnostic> { self.0 }
}
