...
impl Diagnostic {
	...
	pub(crate) fn add_file(&mut self, filename: impl Into<ArcStr>, content: impl Into<ArcStr>) -> DiagnosticFileId {
	...
	pub(crate) fn add_label(
		...
		let label = Label::new(RolldownLabelSpan(file_id.0.clone().into(), range)).with_message(message);
