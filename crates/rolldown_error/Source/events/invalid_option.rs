...
impl BuildEvent for InvalidOption {
	fn kind(&self) -> EventKind { EventKind::InvalidOption }

	fn message(&self, _opts:&DiagnosticOptions) -> String {
		match &self.invalid_option_type {
			InvalidOptionType::UnsupportedCodeSplittingFormat(format) => {
				format!(
					"Invalid value \"{format}\" for option \"output.format\" - UMD and IIFE are \
					 not supported for code splitting. You may set `output.inlineDynamicImports` \
					 to `true` when using dynamic imports."
				)
			},
			InvalidOptionType::InvalidOutputFile => {
