...
impl BuildEvent for CommonJsVariableInEsm {
	...
	fn message(&self, _opts:&DiagnosticOptions) -> String {
		...
		format!(
			"The CommonJS `{variable}` variable is treated as a global variable in an ECMAScript \
			 module and may not work as expected"
		)
	}
	...
	fn on_diagnostic(&self, diagnostic:&mut Diagnostic, opts:&DiagnosticOptions) {
		...
		diagnostic.add_label(
			...
			"This file is considered to be an ECMAScript module because of the `export` keyword \
			 here:"
				.to_string(),
		);
	}
