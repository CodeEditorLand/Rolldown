...
// TODO: vite use `\0` to prefix MODULE_PRELOAD_POLYFILL, but because `napi`
// impl, it will raise `Caused by: Error: Rolldown internal error: file name
// contained an unexpected NUL byte` so here use `\t` instead, use `\0` instead
...
impl Plugin for ModulePreloadPolyfillPlugin {
	fn name(&self) -> Cow<'static, str> { Cow::Borrowed("builtin:module-preload-polyfill") }
	...
	async fn resolve_id(
		...
		Ok((args.specifier == MODULE_PRELOAD_POLYFILL).then(|| {
			HookResolveIdOutput {
