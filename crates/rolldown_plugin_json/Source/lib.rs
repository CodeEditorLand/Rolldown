	...
	async fn transform(
		...
		if self.stringify != JsonPluginStringify::False {
			if self.named_exports && code.trim_start().starts_with('{') {
				...
				let parsed = parsed.as_object().expect("should be object because the value starts with `{`");
...
fn serialize_value(value: &Value) -> Result<String, serde_json::Error> {
	...
	if value.is_object() && !value.is_null() && value_as_string.len() > 10 * 1000 {
		Ok(format!(
			"/*#__PURE__*/ JSON.parse({})",
			serde_json::to_string(&value_as_string)?
		))
	} else {
		Ok(value_as_string)
	}
...
fn to_esm(data: &Value, named_exports: bool) -> String {
	...
	let default_export_code: String = default_export_rows.iter().map(|s| s.as_str()).collect::<Vec<_>>().join(",\n");
