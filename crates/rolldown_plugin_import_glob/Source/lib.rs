...
use oxc::{
	...
	ast::{
		AstBuilder, NONE,
		ast::{
			Argument, ArrayExpressionElement, Expression, FormalParameterKind, ImportOrExportKind, NumberBase,
...
impl Plugin for ImportGlobPlugin {
	...
	async fn transform_ast(&self, _ctx: &PluginContext, mut args: HookTransformAstArgs<'_>) -> HookTransformAstReturn {
...
impl<'ast> GlobImportVisit<'ast, '_> {
	fn maybe_visit_obj_call(&mut self, expr: &mut Expression<'ast>) -> Option<bool> {
		...
		let [arg] = call_expr.arguments.as_mut_slice() else {
			return None;
		};
	...
	fn maybe_visit_glob_import_call(&mut self, expr: &mut Expression<'ast>, omit_type: Option<&OmitType>) {
		...
		let Expression::MetaProperty(p) = &callee.object else {
			return;
		};
		...
		// }
		*expr = self.generate_glob_object_expression(&files, &opts, call_expr.span, omit_keys, omit_values);
		self.current += 1;
	}
}
		...
		let properties = files.iter().enumerate().map(|(index, file_data)| {
			...
			let formatted_file = if let Some(query) = &opts.query {
				let normalized_query = if query == "?raw" {
					...
					let file_extension = Path::new(&import_path)
						.extension()
						.unwrap_or_default()
						.to_str()
						.unwrap_or_default();
			...
			let value = if omit_values {
				...
				let module_specifier = match opts.import.as_deref() {
					Some("default") => self.ast_builder.import_declaration_specifier_import_default_specifier(
						SPAN,
						self.ast_builder.binding_identifier(SPAN, &name),
					),
					Some("*") | None => self.ast_builder.import_declaration_specifier_import_namespace_specifier(
						SPAN,
						self.ast_builder.binding_identifier(SPAN, &name),
					),
				...
				self.import_decls
					.push(Statement::from(self.ast_builder.module_declaration_import_declaration(
						SPAN,
				...
				let mut import_expression = self.ast_builder.expression_import(
					SPAN,
					self.ast_builder
						.expression_string_literal(Span::default(), formatted_file.as_str(), None),
