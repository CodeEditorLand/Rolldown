...
pub async fn generate_manifest(
	...
	let manifest = Manifest {
		...
		meta_data: MetaData {
			...
			build_info: BuildInfo { build_version: "1.0.0".to_string(), build_name: options.name.clone() },
	...
	ctx.emit_file_async(EmittedAsset {
		file_name: Some(
			options
				.manifest
				.as_ref()
		...
		source: (serde_json::to_string_pretty(&manifest).expect("should success serialize manifest json")).into(),
	})
	.await?;
