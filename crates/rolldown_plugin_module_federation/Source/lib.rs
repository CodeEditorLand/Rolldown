				...
				shared
					...
					.map(|(key, value)| {
						concat_string!(
							...
							value
								.singleton
								.map(|v| if v { "singleton: true," } else { "" })
								.unwrap_or_default(),
							value
								.required_version
								.as_deref()
...
impl Plugin for ModuleFederationPlugin {
	...
	async fn build_start(
		...
		if let Some(filename) = self.options.filename.as_deref() {
			...
			ctx.emit_chunk(EmittedChunk { id: INIT_HOST.to_string(), ..Default::default() })
				.await?;
		}
		...
		if let Some(shared) = self.options.shared.as_ref() {
			for (key, item) in shared {
				if item.version.is_none() {
					...
					self.resolved_id_to_remote_module_keys
						.insert(resolve_id.id.clone(), key.as_str().into());
				}
			}
		}
	...
	async fn resolve_id(
		...
		if args.specifier.starts_with(SHARED_MODULE_PREFIX) {
			let resolve_id = ctx.resolve(&args.specifier[SHARED_MODULE_PREFIX.len()..], None, None).await??;
			return Ok(Some(rolldown_plugin::HookResolveIdOutput {
				id: resolve_id.id,
				..Default::default()
			}));
		...
		args.ast.program.with_mut(|fields| {
			...
			if args.is_user_defined_entry && self.options.filename.is_none() {
				...
				fields
					.program
					.body
					.push(Statement::from(ast_builder.module_declaration_import_declaration(
						SPAN,
						None,
	...
	async fn render_chunk(
			...
			|| args.chunk.module_ids.iter().any(|module_id| module_id.as_ref() == INIT_HOST)
		{
			...
			for remote_module_ref in &self.resolved_remote_modules {
				...
				code = code.replace(&remote_module.placeholder, if remote_module.is_cjs { "true" } else { "false" });
			}
