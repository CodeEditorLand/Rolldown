...
/// - Using `RD_LOG=trace` to enable tracing or other values for more specific
///   tracing.
...
/// - Using `RD_LOG=trace RD_LOG_OUTPUT=chrome-json` to collect tracing events
///   into a json file.
///   - Using `RD_LOG_OUTPUT_STYLE=async` to record traces as a group of
...
pub fn try_init_tracing() -> Option<FlushGuard> {
	if std::env::var(LOG_ENV_NAME).is_err() {
		// tracing will slow down the bundling process, so we only enable it when `LOG`
		// is set.
		return None;
	}
	...
	match output_mode.as_str() {
		"chrome-json" | "chrome-json-threaded" => {
			let trace_style = if output_mode == "chrome-json" {
				TraceStyle::Async
			} else {
				TraceStyle::Threaded
			};
