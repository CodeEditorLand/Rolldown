...
pub struct LinkingMetadata {
	/// A module could be wrapped for some reasons, eg. cjs module need to be
	/// wrapped with commonjs runtime function. The `wrap_ref` is the binding
	/// identifier that store return value of executed the wrapper function.
	///
	/// ## Example
	///
	/// ```js
	...
	// Store the export info for each module, including export named
	// declaration and export star declaration.
	...
	// If a esm module has export star from commonjs, it will be marked as
	// ESMWithDynamicFallback at linker. The unknown export name will be
	// resolved at runtime. esbuild add it to `ExportKind`, but the linker
	// shouldn't mutate the module.
	pub has_dynamic_exports:bool,
	pub shimmed_missing_exports:FxHashMap<Rstr, SymbolRef>,

	// Entry chunks need to generate code that doesn't belong to any module.
	// This is the list of symbols are referenced by the generated code.
	// Tree-shaking will cares about these symbols to make sure they are not
	// removed.
	pub referenced_symbols_by_entry_point_chunk:Vec<SymbolRef>,

	/// The dependencies of the module. It means if you want include this
	/// module, you need to include these dependencies too.
	...
	// We need to generate `const ext = require('ext')` for `export * from
	// 'ext'` in cjs output
...
impl LinkingMetadata {
	pub fn canonical_exports(&self) -> impl Iterator<Item = (&Rstr, &ResolvedExport)> {
		self.sorted_and_non_ambiguous_resolved_exports
			.iter()
