...
pub struct LinkingMetadata {
	...
	// Store the export info for each module, including export named declaration and export star
	// declaration.
	...
	// If a esm module has export star from commonjs, it will be marked as ESMWithDynamicFallback
	// at linker. The unknown export name will be resolved at runtime.
	...
	// Entry chunks need to generate code that doesn't belong to any module. This is the list of
	// symbols are referenced by the generated code. Tree-shaking will cares about these symbols
	// to make sure they are not removed.
	...
	pub fn referenced_canonical_exports_symbols<'b, 'a:'b>(
		...
		let partial_used_exports = match entry_point_kind {
			...
			rolldown_common::EntryPointKind::DynamicImport => {
				dynamic_import_exports_usage_map.get(&module_idx).and_then(|usage| {
					match usage {
		...
		self.canonical_exports().filter(move |(name, _)| {
			match partial_used_exports {
