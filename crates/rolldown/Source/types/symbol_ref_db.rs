use oxc::{
	index::IndexVec,
	semantic::{SymbolId, SymbolTable},
	span::CompactStr as CompactString,
};
...
impl SymbolRefDbForModule {
	pub fn fill_classic_data(&mut self, ast_symbols:SymbolTable) {
		self.classic_data = ast_symbols
			...
			.into_iter()
			.map(|name| {
				SymbolRefDataClassic { name, link:None, chunk_id:None, namespace_alias:None }
			})
			.collect();
	}
}
...
impl SymbolRefDb {
	...
	pub fn canonical_ref_for(&mut self, target:SymbolRef) -> SymbolRef {
		...
		if target != canonical {
			// update the link to the canonical so that the next time we can get the
			// canonical directly
