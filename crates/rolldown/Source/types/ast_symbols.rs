...
impl AstSymbols {
	...
	pub fn scope_id_for(&self, symbol_id:SymbolId) -> ScopeId { self.scope_ids[symbol_id] }

	pub fn get_span(&self, symbol_id:SymbolId) -> Span { self.spans[symbol_id] }

	pub fn get_name(&self, symbol_id:SymbolId) -> &str { &self.names[symbol_id] }

	pub fn get_flag(&self, symbol_id:SymbolId) -> SymbolFlags { self.flags[symbol_id] }
}
