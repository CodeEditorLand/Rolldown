...
impl GenerateContext<'_> {
	/// A `SymbolRef` might be identifier or a property access. This function
	/// will return correct string pattern for the symbol.
			...
			rolldown_common::OutputFormat::Cjs => {
				let chunk_idx_of_canonical_symbol =
					canonical_symbol.chunk_id.unwrap_or_else(|| {
						...
						// performance reason, because they are surely belong to the chunk
						// they are declared in and won't link to other chunks.
						let symbol_name = canonical_ref.name(symbol_db);
						panic!(
							"{canonical_ref:?} {symbol_name:?} is not in any chunk, which isn't \
							 unexpected"
						);
					});
				...
				if is_symbol_in_other_chunk {
					...
					// console.log(foo);`; If `foo` is split into another chunk, we need to
					// convert the code `console.log(foo);` to `console.log(require_xxxx.foo);`
					// instead of keeping `console.log(foo)` as we did in esm output. The reason
					// here is wee need to keep live binding in cjs output.

					let exported_name = &self.chunk_graph.chunk_table
