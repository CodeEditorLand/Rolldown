...
use rolldown_common::{
	Chunk, ChunkKind, ExportsKind, NormalizedBundlerOptions, OutputExports, OutputFormat,
	SymbolRef, WrapKind,
};
		...
		OutputFormat::Cjs | OutputFormat::Iife => {
			...
			match chunk.kind {
				ChunkKind::EntryPoint { module, .. } => {
					let module = &link_output.module_table.modules[module]
						.as_ecma()
						.expect("should be ecma module");
					if matches!(module.exports_kind, ExportsKind::Esm) {
						let rendered_items = export_items
							.into_iter()
							.map(|(exported_name, export_ref)| {
								let canonical_ref =
									link_output.symbols.par_canonical_ref_for(export_ref);
								let symbol = link_output.symbols.get(canonical_ref);
								let mut canonical_name =
									Cow::Borrowed(&chunk.canonical_names[&canonical_ref]);
								if let Some(ns_alias) = &symbol.namespace_alias {
									let canonical_ns_name =
										&chunk.canonical_names[&ns_alias.namespace_ref];
									let property_name = &ns_alias.property_name;
									s.push_str(&format!(
                    "var {canonical_name} = {canonical_ns_name}.{property_name};\n"
                  ));
									...
									if is_this_symbol_point_to_other_chunk {
										let require_binding = &ctx
											.chunk
											.require_binding_names_for_other_chunks[&canonical_ref_owner_chunk_idx];
										canonical_name = Cow::Owned(Rstr::new(&format!(
											"{require_binding}.{canonical_name}"
										)));
									}
								}
								...
								match export_mode {
									Some(OutputExports::Named) => {
										if !options.external_live_bindings
											&& export_ref.is_created_by_import_from_external(
