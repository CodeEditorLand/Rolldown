...
pub fn deconflict_chunk_symbols(
	...
	if matches!(format, OutputFormat::Iife | OutputFormat::Umd | OutputFormat::Cjs) {
		...
		match chunk.entry_module_idx() {
			Some(module) => {
				let entry_module = link_output.module_table.modules[module]
					.as_normal()
					.expect("should be normal module");

				link_output.metas[entry_module.idx]
					.star_exports_from_external_modules
					.iter()
					.for_each(|rec_idx| {
						let rec = &entry_module.ecma_view.import_records[*rec_idx];

						let external_module = &link_output.module_table.modules
							[rec.resolved_module]
							.as_external()
	...
	// Though, those symbols in `imports_from_other_chunks` doesn't belong to this
	// chunk, but in the final output, they still behave like declared in this
	// chunk. This is because we need to generate import statements in this chunk to
	// import symbols from other statements. Those `import {...} from
	// './other-chunk.js'` will declared these outside symbols in this chunk, so
	// symbols that point to them can be resolved in runtime.
	// So we add them in the deconflict process to generate conflict-less names in
	// this chunk.
	chunk
