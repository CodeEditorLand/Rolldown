...
pub fn finalize_assets(
		...
		.map(|asset| {
			match &asset.content {
				StrOrBytes::Str(content) => {
					extract_hash_placeholders(content)
						.iter()
						.filter_map(|placeholder| {
							asset_idx_by_placeholder.get(placeholder).copied()
						})
						.collect_vec()
				},
	...
	// Instead of using `index_direct_dependencies`, we are gonna use
	// `index_transitive_dependencies` to calculate the hash. The reason is that we
	// want to make sure, in `a -> b -> c`, if `c` is changed, not only the direct
	// dependency `b` is changed, but also the indirect dependency `a` is changed.
	...
	let index_final_hashes:IndexVec<AssetIdx, (String, u128)> = index_asset_hashers
		...
		.map(|(asset_idx, mut hasher)| {
			...
			// hash itself's preliminary filename to prevent different chunks that have the
			// same content from having the same hash
	...
	let mut assets:IndexAssets = preliminary_assets
		...
		.map(|(asset_idx, mut asset)| {
			...
			// TODO: PERF: should check if this asset has dependencies/placeholders to be
			// replaced
	...
	assets.par_iter_mut().for_each(|asset| {
		if let InstantiationKind::Ecma(ecma_meta) = &mut asset.meta {
			...
			ecma_meta.rendered_chunk.imports = chunk
				...
				.map(|importee_asset_idx| {
					index_asset_to_filename[*importee_asset_idx].clone().into()
				})
				.collect();
