...
use rolldown_utils::{
	...
	indexmap::FxIndexSet,
	rayon::{IntoParallelIterator, IntoParallelRefIterator, IntoParallelRefMutIterator, ParallelIterator},
...
pub fn finalize_assets(
	...
	let index_asset_hashers: IndexVec<AssetIdx, Xxh3> = index_vec![Xxh3::default(); preliminary_assets.len()];
	...
	let mut assets: IndexAssets = preliminary_assets
		...
		.map(|(asset_idx, mut asset)| {
			...
			let filename: ArcStr =
				replace_placeholder_with_hash(asset.preliminary_filename.as_str(), &final_hashes_by_placeholder).into();
			...
			match &mut asset.content {
				StrOrBytes::Str(content) => {
					*content =
						replace_placeholder_with_hash(mem::take(content), &final_hashes_by_placeholder).into_owned();
				},
				StrOrBytes::Bytes(_content) => {},
			}
...
fn collect_transitive_dependencies(
	...
	fn traverse(index: AssetIdx, dep_map: &IndexVec<AssetIdx, Vec<AssetIdx>>, visited: &mut FxIndexSet<AssetIdx>) {
	...
	let index_transitive_dependencies: IndexVec<AssetIdx, FxIndexSet<AssetIdx>> = index_direct_dependencies
		.par_iter()
		.enumerate()
		.map(|(idx, _deps)| {
