...
pub fn finalize_assets(
	...
	let index_final_hashes:IndexVec<AssetIdx, String> = index_asset_hashers
		...
		.map(|(asset_idx, mut hasher)| {
			...
			// hash itself's preliminary filename to prevent different chunks
			// that have the same content from having the same hash
	...
	let final_hashes_by_placeholder =
		chunk_graph
			.chunk_table
			.iter()
			.zip(&index_final_hashes)
			.filter_map(|(chunk, hash)| {
				chunk.preliminary_filename.as_ref().unwrap().hash_placeholder().map(
					|hash_placeholder| (hash_placeholder.into(), &hash[..hash_placeholder.len()]),
				)
			})
	...
	let mut assets:IndexAssets = preliminary_assets
		...
		.map(|mut asset| {
			...
			// TODO: PERF: should check if this asset has
			// dependencies/placeholders to be replaced
