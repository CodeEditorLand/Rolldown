...
use oxc::{
	...
	ast::{
		NONE,
		VisitMut,
		ast::{self, BindingPatternKind, Declaration, ImportOrExportKind, Statement},
		visit::walk_mut,
	},
	span::{SPAN, Span},
};
...
/// Pre-process is a essential step to make rolldown generate correct and
/// efficient code.
pub struct PreProcessor<'ast> {
	...
	/// For none top level statements, this is used to store split
	/// `VarDeclaration`.
			...
			.map(|(i, declarator)| {
				...
				if let Some(named_decl_span) = named_decl_span {
					Statement::ExportNamedDeclaration(
						self.snippet.builder.alloc_export_named_declaration(
...
impl<'ast> VisitMut<'ast> for PreProcessor<'ast> {
	fn visit_program(&mut self, program:&mut ast::Program<'ast>) {
		...
		self.stmt_temp_storage = Vec::with_capacity(
			original_body
				.iter()
				.filter(|stmt| !stmt.is_module_declaration_with_source())
				.count(),
		);
	...
	fn visit_export_named_declaration(
		&mut self,
		named_decl:&mut ast::ExportNamedDeclaration<'ast>,
	) {
		walk_mut::walk_export_named_declaration(self, named_decl);
		let named_decl_span = named_decl.span;

		let Some(Declaration::VariableDeclaration(ref mut var_decl)) = named_decl.declaration
		else {
			return;
		};
	...
	fn visit_expression(&mut self, it:&mut ast::Expression<'ast>) {
		let to_replaced = match it {
				...
				if expr.callee.is_specific_id("require") && expr.arguments.len() == 1 =>
			{
				...
				if let Some(cond_expr) = arg.as_expression_mut().and_then(|item| {
					match item {
