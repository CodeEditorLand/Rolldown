...
use super::{
	ecma_visitors::EnsureSpanUniqueness,
	parse_to_ecma_ast::ParseToEcmaAstResult,
	tweak_ast_for_scanning::PreProcessor,
};
...
impl PreProcessEcmaAst {
	...
	pub fn build(
		...
		let semantic_ret = ast
			.program
			.with_mut(|WithMutFields { program, .. }| SemanticBuilder::new().build(program));
		...
		// NOTE: Recreate semantic data because AST is changed in the transformations
		// above.
