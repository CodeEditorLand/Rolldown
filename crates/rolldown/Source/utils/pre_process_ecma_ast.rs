...
use oxc::{
	...
	semantic::{ScopeTree, SemanticBuilder, Stats, SymbolTable},
	span::SourceType,
...
use super::{ecma_visitors::EnsureSpanUniqueness, tweak_ast_for_scanning::tweak_ast_for_scanning};
...
impl PreProcessEcmaAst {
	// #[allow(clippy::match_same_arms)]: `OxcParseType::Tsx` will have special
	// logic to deal with ts compared to `OxcParseType::Jsx`
	...
	pub fn build(
		...
		// return Err(anyhow::anyhow!("Semantic Error: {:#?}",
		// semantic_ret.errors)); }
		...
		ast.program.with_mut(
			|WithMutFields { allocator, program, source }| -> anyhow::Result<()> {
				// Use built-in define plugin.
				if let Some(replace_global_define_config) = replace_global_define_config {
					let ret =
						ReplaceGlobalDefines::new(allocator, replace_global_define_config.clone())
				...
				if bundle_options.treeshake.enabled() {
					...
					// NOTE: `CompressOptions::dead_code_elimination` will remove
					// `ParenthesizedExpression`s from the AST.
					let compressor =
						Compressor::new(allocator, CompressOptions::dead_code_elimination());
					if self.ast_changed {
						let semantic_ret =
							SemanticBuilder::new(source).with_stats(self.stats).build(program);
						(symbols, scopes) =
		...
		// NOTE: Recreate semantic data because AST is changed in the
