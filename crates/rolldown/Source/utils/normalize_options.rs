...
pub fn normalize_options(mut raw_options:crate::BundlerOptions) -> NormalizeOptionsReturn {
	...
	let globals = raw_options
		.globals
		.unwrap_or(GlobalsOutputOption::FxHashMap(FxHashMap::default()));
			...
			if matches!(raw_options.inline_dynamic_imports, Some(false)) {
				warnings.push(
					BuildDiagnostic::invalid_option(
						InvalidOptionType::UnsupportedCodeSplittingFormat(format.to_string()),
					)
	...
	let normalized = NormalizedBundlerOptions {
		...
		entry_filenames:raw_options
			.entry_filenames
			.unwrap_or_else(|| "[name].js".to_string().into()),
