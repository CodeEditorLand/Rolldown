...
use oxc::{
	semantic::ScopeId,
	syntax::keyword::{GLOBAL_OBJECTS, RESERVED_KEYWORDS},
};
...
pub struct Renamer<'name> {
	...
	/// value is the how many same variable name in the top level are used
	/// before It is also used to calculate the candidate_name e.g.
	/// ```js
	/// // index.js
...
impl<'name> Renamer<'name> {
	pub fn new(symbols:&'name Symbols, _modules_len:usize, format:&OutputFormat) -> Self {
		...
		let manual_reserved = match format {
			...
			OutputFormat::Iife => vec!["exports"], /* Also for UMD, AMD, but
	...
	pub fn reserve(&mut self, name:Rstr) { self.used_canonical_names.insert(name, 0); }
		...
		loop {
			match self.used_canonical_names.entry(conflictless_name.clone()) {
				Entry::Occupied(mut occ) => {
					...
					*occ.get_mut() = next_conflict_index;
					conflictless_name =
						format!("{hint}${}", itoa::Buffer::new().format(next_conflict_index))
							.into();
				},
				Entry::Vacant(vac) => {
					vac.insert(0);
	...
	// non-top-level symbols won't be linked cross-module. So the canonical
	// `SymbolRef` for them are themselves.
	...
	pub fn rename_non_top_level_symbol(
					...
					Entry::Vacant(slot) => {
						loop {
							let is_shadowed = stack.iter().any(|used_canonical_names| {
								used_canonical_names.contains_key(&candidate_name)
							});

							if is_shadowed {
								candidate_name =
									format!("{binding_name}${}", itoa::Buffer::new().format(count))
										.into();
								count += 1;
							} else {
								used_canonical_names_for_this_scope
									.insert(candidate_name.clone(), 0);
		...
		let copied_scope_iter = modules_in_chunk
			.par_iter()
			.copied()
			.filter_map(|id| modules[id].as_normal())
			.flat_map(|module| {
				let child_scopes:&[ScopeId] =
					module.scope.get_child_ids(module.scope.root_scope_id());
	...
	pub fn into_canonical_names(self) -> FxHashMap<SymbolRef, Rstr> { self.canonical_names }
}
