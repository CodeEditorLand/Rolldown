...
impl WatcherImpl {
	...
	pub fn new(bundlers:Vec<Arc<Mutex<Bundler>>>) -> Result<Self> {
		...
		let tasks = bundlers
			.into_iter()
			.map(|bundler| WatcherTask::new(bundler, Arc::clone(&emitter)))
			.collect();
	...
	pub async fn run(&self) -> BuildResult<()> {
		...
		// The inner mutex should be dropped to avoid deadlock with bundler lock at
		// `Watcher::close`
		std::mem::drop(inner);
	...
	pub async fn start(&self) { let _ = self.run().await; }
}
	...
	let future = async move {
		...
		while run {
			...
			match rx.recv() {
				Ok(msg) => {
					match msg {
						WatcherChannelMsg::NotifyEvent(event) => {
							match event {
								Ok(event) => {
