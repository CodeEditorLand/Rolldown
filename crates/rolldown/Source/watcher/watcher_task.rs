...
impl WatcherTask {
	...
	pub async fn run(&self) -> BuildResult<()> {
		...
		// FIXME(hyf0): probably should have a more official API/better way to get watch
		// files
		...
		match output {
			Ok(_output) => {
				self.emitter.emit(WatcherEvent::Event(BundleEvent::BundleEnd(
					BundleEndEventData {
						output:bundler
							.options
							.cwd
							.join(&bundler.options.dir)
							.to_string_lossy()
							.to_string(),
