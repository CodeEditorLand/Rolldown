...
use crate::{
	...
	utils::chunk::{determine_use_strict::determine_use_strict, render_chunk_exports::render_chunk_exports},
};
...
fn render_cjs_chunk_imports(ctx: &GenerateContext<'_>) -> String {
	...
	ctx.chunk.imports_from_other_chunks.iter().for_each(|(exporter_id, items)| {
		...
		let require_path_str = concat_string!("require('", ctx.chunk.import_path_for(importee_chunk), "');\n");
		if items.is_empty() {
	...
	ctx.chunk.imports_from_external_modules.iter().for_each(|(importee_id, _)| {
		...
		let require_path_str = concat_string!("require(\"", &importee.get_import_path(ctx.chunk), "\")");
