...
pub fn render_cjs(
	...
	// Note that the determined `export_mode` should be used in
	// `render_chunk_exports` to render exports. We also need to get the export
	// mode for rendering the namespace markers. So we determine the export
	// mode (from auto) here and use it in the following code.
		...
		if let Module::Normal(entry_module) = &ctx.link_output.module_table.modules[entry_id] {
			if matches!(entry_module.exports_kind, ExportsKind::Esm) {
				...
				let has_default_export =
					export_items.iter().any(|(name, _)| name.as_str() == "default");
				...
				meta.require_bindings_for_star_exports.iter().for_each(
					|(importee_idx, binding_ref)| {
						let importee = &ctx.link_output.module_table.modules[*importee_idx];
						let binding_ref_name = ctx
							.link_output
							.symbols
							.canonical_name_for(*binding_ref, &ctx.chunk.canonical_names);
  ...
  if (k !== 'default' && !Object.prototype.hasOwnProperty.call(exports, k)) \
    ...
    get: function () { return $NAME[k]; }
				...
				// There is no need for a non-ESM export kind for determining
				// the export mode.
				None
			}
		} else {
	...
	// Runtime module should be placed before the generated `requires` in CJS
	// format. Because, we might need to generate `__toESM(require(...))` that
	// relies on the runtime module.
	let mut module_sources_peekable = module_sources.into_iter().peekable();
	match module_sources_peekable.peek() {
		Some((id, ..)) if *id == ctx.link_output.runtime.id() => {
	...
	if let ChunkKind::EntryPoint { module: entry_id, .. } = ctx.chunk.kind {
		...
		match entry_meta.wrap_kind {
			WrapKind::Esm => {
				...
				let wrapper_ref_name = ctx
					.link_output
					.symbols
					.canonical_name_for(*wrapper_ref, &ctx.chunk.canonical_names);
				concat_source
					.add_source(Box::new(RawSource::new(format!("{wrapper_ref_name}();",))));
			},
			WrapKind::Cjs => {
