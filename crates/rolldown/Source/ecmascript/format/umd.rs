...
use super::utils::{
	namespace::render_property_access,
...
use crate::{
	ecmascript::{
		...
		format::utils::namespace::generate_namespace_definition,
	},
...
pub async fn render_umd<'code>(
	...
	let factory_parameters =
		render_factory_parameters(ctx, &externals, has_exports && named_exports);
	let cjs_intro = if need_global {
		...
		format!(
			"typeof exports === 'object' && typeof module !== 'undefined' ? {cjs_export} \
			 factory({cjs_dependencies}) :",
		)
	} else {
		String::new()
	};
...
async fn render_iife_export(
	...
	for external in externals {
		...
		let target = match &global {
			Some(global_name) => {
				global_name.split('.').map(render_property_access).collect::<String>()
			},
			None => {
