...
use crate::{
	ecmascript::{ecma_generator::RenderedModuleSources, format::utils::namespace::generate_namespace_definition},
	...
	utils::chunk::{
		...
		render_chunk_exports::{get_chunk_export_names, render_chunk_exports, render_wrapped_entry_chunk},
	},
};
...
pub async fn render_umd<'code>(
	...
	let iife_export = render_iife_export(warnings, ctx, &externals, has_exports, named_exports).await?;
  ...
  {iife_start}{iife_export}{iife_end};
	...
	render_modules_with_peek_runtime_module_at_first(ctx, &mut source_joiner, module_sources, import_code);
...
fn render_amd_dependencies(ctx: &GenerateContext<'_>, externals: &[&ExternalModule], has_exports: bool) -> String {
...
fn render_cjs_dependencies(ctx: &GenerateContext<'_>, externals: &[&ExternalModule], has_exports: bool) -> String {
	let mut dependencies = Vec::with_capacity(externals.len());
	if has_exports {
		dependencies.reserve(1);
		dependencies.push("exports".to_string());
...
async fn render_iife_export(
	...
	if has_exports {
		let (stmt, namespace) =
			generate_namespace_definition(ctx.options.name.as_ref().expect("should have name"), "global", ",");
		if named_exports {
			Ok(format!(
