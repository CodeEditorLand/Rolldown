...
pub fn render_esm<'code>(
	...
	module_sources
		.iter()
		.for_each(|RenderedModuleSource { sources: module_render_output, .. }| {
...
fn render_esm_chunk_imports(ctx: &GenerateContext<'_>) -> String {
	...
	ctx.chunk
		...
		.iter()
		.for_each(|(importee_id, named_imports)| {
...
fn create_import_declaration(mut specifiers: Vec<String>, default_alias: &[ArcStr], path: &str) -> String {
