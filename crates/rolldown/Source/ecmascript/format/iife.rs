...
//! 2. Start the wrapper function, and determine the export mode (from auto or
//!    manual exports).
//! 3. Render the imports and modify the arguments of the wrapper function.
//!    Including:
//!       - Render the arguments including the function arguments and the
//!         external imports, according to the `output.globals`, or if you are
//!         using named export, the function will pass the `exports` argument
//!         with default `{}` as the first argument.
//!       - Generate the statement for a namespace level-by-level and define the
//!         IIFE wrapper function name if `output.extends` is false, or the
//!         export mode isn't `named`.
//!
//!    Note that in IIFE, the external imports are directly assigned to the
//! global variables.    And in the wrapper function, the global variables are
//! passed as arguments.
//! 4. Check if the chunk is suitable for strict mode, and add `"use strict";`
//!    if necessary.
//! 5. Render the intro if it exists.
//! 6. Render the chunk content.
//! 7. Render the exports if it exists. If you are using named export, it will
//!    modify the `exports` object. If you are using default export, it will
//!    return the default value.
//! 8. Render the outro if it exists.
//! 9. The wrapper function ends with `})({output_args});` if `invoke` is true,
//!    otherwise, it ends with `})`. (for UMD capability)
...
use crate::{
	ecmascript::{
		...
		format::utils::namespace::generate_identifier,
	},
	...
	utils::chunk::{
		collect_render_chunk_imports::{
			...
			RenderImportDeclarationSpecifier,
			RenderImportStmt,
		},
...
pub fn render_iife(
	...
	let entry_module = match ctx.chunk.kind {
		ChunkKind::EntryPoint { module, .. } => {
			&ctx.link_output.module_table.modules[module]
				.as_normal()
	...
	// We need to transform the `OutputExports::Auto` to suitable
	// `OutputExports`.
	...
	// The function argument and the external imports are passed as arguments to
	...
	let (input_args, output_args) = render_iife_arguments(
		...
		if has_exports && named_exports {
			if ctx.options.extend {
				// If using `output.extend`, the first caller argument should be
				// `name = name || {}`, then the result will be assigned to
				// `name`.
				Some(assignment.as_str())
			} else {
				// If not using `output.extend`, the first caller argument
				// should be `{}`, then the result will be assigned to
				// `exports`.
				Some("{}")
			}
		} else {
			// If there is no export or not using named export,
			// there shouldn't be an argument shouldn't be related to the
			// export.
			None
		},
	);
	...
	concat_source.add_source(Box::new(RawSource::new(format!(
		...
		if (ctx.options.extend && named_exports) || !has_exports || assignment.is_empty() {
			// If facing following situations, there shouldn't an assignment for
			// the wrapper function:
	...
	if named_exports && has_exports && !ctx.options.extend {
		// We need to add `return exports;` here only if using `named`, because
		// the default value is returned when using `default` in
...
/// Handling external imports needs to modify the arguments of the wrapper
/// function.
...
/// Rendering the arguments of the wrapper function, including the function
/// arguments and calling arguments.
/// - If `output.exports` is `named`, the first argument is `exports`.
///    - If you are using `extend: true`, the inputted argument will be
///      extended;
///    - If you aren't using it, the wrapper function will return the `exports`
///      object as the result.
///
///    If `output.exports` is `default`, there is no need to pass the `exports`
/// argument.    The return value of the wrapper function will be the default
/// export value.
/// - The rest of the arguments are the external imports, which are directly
///   assigned to the global variables.
///    - If the global variable is not defined in `output.globals`, you will be
///      warned and rolldown will use the defined name after legitimizing it.
///    - If the global variable is defined in `output.globals`, the global
///      variable will be used directly (after legitimizing it).
fn render_iife_arguments(
	...
	externals.iter().for_each(|external| {
		let symbol_name = ctx
			.link_output
			.symbols
			.canonical_name_for(external.symbol_ref, &ctx.chunk.canonical_names);
