...
pub async fn render_iife<'code>(
	...
	// The function argument and the external imports are passed as arguments to the
	// wrapper function.
	...
	source_joiner.append_source(concat_string!(
		...
		if (ctx.options.extend && named_exports) || !has_exports || assignment.is_empty() {
			// If facing following situations, there shouldn't an assignment for the wrapper
			// function:
	...
	if named_exports && has_exports && !ctx.options.extend {
		// We need to add `return exports;` here only if using `named`, because the
		// default value is returned when using `default` in `render_chunk_exports`.
