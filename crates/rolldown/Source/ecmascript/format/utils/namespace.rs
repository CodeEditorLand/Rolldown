...
pub fn generate_namespace_definition(name:&str, global:&str, delimiter:&str) -> (String, String) {
...
pub fn generate_identifier(
	...
	if ctx.options.extend {
		...
		let final_expr = if matches!(export_mode, OutputExports::Named) {
			...
			// the modification will be done extending the existed object (the `name`
			// option).
			format!("this{property} = this{property} || {{}}")
		} else {
			// If there isn't a name in default export, we shouldn't assign the function to
			// `this[""]`. If there is, we should assign the function to `this["name"]`,
	...
	if is_validate_assignee_identifier_name(name) {
		...
		// This behavior is aligned with Rollup. If using `output.extend: true`, this
		// error won't be triggered.
