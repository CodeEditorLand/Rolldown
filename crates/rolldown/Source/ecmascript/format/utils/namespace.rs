...
//! - The `name`: whether includes a dot or not, and whether is a valid
//!   identifier or not;
...
///
/// - The initialization code: ```js this.namespace = this.namespace || {};
///   this.namespace.module = this.namespace.module || {}; ```
///  - The final code: ```js this.namespace.module.hello ```
...
/// This function generates a namespace definition for the given name,
/// especially for IIFE format or UMD format. If the name contains a dot, it
/// will be regarded as a namespace definition. Otherwise, it will be regarded
/// as a variable definition.
///
/// - If you are using `extend: false` with a name, it will generate a variable
///   definition (using `default` as an example): ```js var name = (function() {
///   ... })(); ```
/// - If you are using `extend: true` with a name, it will generate an object
///   definition (using `named` as an example): ```js (function(exports) { ...
///   })(this.named = this.named || {}); ```
///
/// As for the namespaced name (including `.`), please refer to the
/// `generate_namespace_definition` function.
pub fn generate_identifier(
	...
	if ctx.options.extend {
		...
		let final_expr = if matches!(export_mode, OutputExports::Named) {
			// In named exports, the `extend` option will make the assignment
			// disappear and the modification will be done extending the
			// existed object (the `name` option).
			format!("this{caller} = this{caller} || {{}}")
		} else {
			// If there isn't a name in default export, we shouldn't assign the
			// function to `this[""]`. If there is, we should assign the
			// function to `this["name"]`, because there isn't an object that
			// we can extend.
	...
	if is_validate_assignee_identifier_name(name) {
		...
		// This behavior is aligned with Rollup. If using `output.extend: true`,
		// this error won't be triggered.
...
///
/// - If the name is not a reserved word and not an invalid identifier, it will
///   generate a caller like `.name`.
