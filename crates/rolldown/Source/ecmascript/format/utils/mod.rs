...
pub fn render_chunk_external_imports<'a>(ctx: &'a GenerateContext<'_>) -> (String, Vec<&'a ExternalModule>) {
...
pub fn render_modules_with_peek_runtime_module_at_first<'a>(
	...
	match module_sources_peekable.peek() {
		Some(RenderedModuleSource { module_idx, .. }) if *module_idx == ctx.link_output.runtime.id() => {
	...
	module_sources_peekable.for_each(|RenderedModuleSource { sources: module_render_output, .. }| {
