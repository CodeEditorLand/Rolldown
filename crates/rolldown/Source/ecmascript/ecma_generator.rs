...
impl Generator for EcmaGenerator {
	...
	async fn instantiate_chunk<'a>(
		...
		// Here file path is generated by chunk file name template, it maybe including
		// path segments. So here need to read it's parent directory as file_dir.
		...
		if let Some(map) = map.as_mut() {
			let paths = map
				.get_sources()
				.map(|source| source.as_path().relative(file_dir))
				.collect::<Vec<_>>();
			// Here not normalize the windows path, the rollup `sourcemap_path_transform`
			// ctx.options need to original path.
