...
impl Generator for EcmaGenerator {
	...
	async fn instantiate_chunk<'a>(
		...
		rendered_module_sources.iter().for_each(|(_, module_id, _)| {
			// FIXME: NAPI-RS used CStr under the hood, so it can't handle null
			// byte in the string.
		...
		let concat_source = match ctx.options.format {
			OutputFormat::Esm => {
				render_esm(ctx, rendered_module_sources, banner, footer, intro, outro)
			},
			OutputFormat::Cjs => {
			...
			OutputFormat::App => {
				render_app(ctx, rendered_module_sources, banner, footer, intro, outro)
			},
			OutputFormat::Iife => {
		...
		// Here file path is generated by chunk file name template, it maybe
		// including path segments. So here need to read it's parent directory
		// as file_dir.
		...
		if let Some(map) = map.as_mut() {
			let paths = map
				...
				.collect::<Vec<_>>();
			// Here not normalize the windows path, the rollup
			// `sourcemap_path_transform` ctx.options need to original path.
