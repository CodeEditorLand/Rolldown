...
pub async fn create_ecma_view<'any>(
	...
	// We should skip the `check_side_effects_for` if the hook side effects is not
	// `None`.
	...
	let side_effects = match args.hook_side_effects {
		...
		// If user don't specify the side effects, we use fallback value from
		// `option.treeshake.moduleSideEffects`;
		None => {
			match ctx.options.treeshake {
				...
				TreeshakeOptions::Option(ref opt) => {
					if opt.module_side_effects.is_fn() {
						...
						match opt
							.module_side_effects
							.native_resolve(&stable_id, ctx.resolved_id.is_external)
						{
	...
	// TODO: Should we check if there are `check_side_effects_for` returns false but
	// there are side effects in the module?
	let view = EcmaView {
