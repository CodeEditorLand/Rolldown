...
use arcstr::ArcStr;
use rolldown_common::{GetLocalDbMut, NormalizedBundlerOptions, ScanMode, SharedFileEmitter, SymbolRefDb};
use rolldown_error::{BuildDiagnostic, BuildResult};
use rolldown_fs::{FileSystem, OsFileSystem};
use rolldown_plugin::{__inner::SharedPluginable, HookBuildEndArgs, HookRenderErrorArgs, SharedPluginDriver};
use std::sync::Arc;
...
impl Bundler {
	...
	pub async fn scan(&mut self, changed_ids: Vec<ArcStr>) -> BuildResult<NormalizedScanStageOutput> {
		let mode = if !self.options.experimental.is_incremental_build_enabled() || changed_ids.is_empty() {
			ScanMode::Full
		} else {
		...
		let scan_stage_output = self.normalize_scan_stage_output_and_update_cache(scan_stage_output, is_full_scan_mode);
	...
	pub async fn bundle_write(&mut self, scan_stage_output: NormalizedScanStageOutput) -> BuildResult<BundleOutput> {
	...
	async fn bundle_up(
		...
		let bundle_output = GenerateStage::new(&mut link_stage_output, &self.options, &self.plugin_driver)
			.generate()
