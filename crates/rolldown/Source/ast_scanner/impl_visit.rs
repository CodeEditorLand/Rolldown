...
impl<'me, 'ast:'me> Visit<'ast> for AstScanner<'me, 'ast> {
	...
	fn enter_node(&mut self, kind:oxc::ast::AstKind<'ast>) { self.visit_path.push(kind); }

	fn leave_node(&mut self, _:oxc::ast::AstKind<'ast>) { self.visit_path.pop(); }
	...
	fn visit_program(&mut self, program:&ast::Program<'ast>) {
		for (idx, stmt) in program.body.iter().enumerate() {
			...
			self.current_stmt_info.side_effect = SideEffectDetector::new(
				...
				self.source,
				self.comments,
				// In `NormalModule` the options is always `Some`, for `RuntimeModule` always
				// enable annotations
			...
			self.result
				.stmt_infos
				.add_stmt_info(std::mem::take(&mut self.current_stmt_info));
		}
		...
		if self.result.has_eval {
			// if there exists `eval` in current module, assume all dynamic import are
			// completely used;
	...
	fn visit_assignment_expression(&mut self, node:&ast::AssignmentExpression<'ast>) {
		match &node.left {
			...
			ast::AssignmentTarget::StaticMemberExpression(member_expr) => {
				match member_expr.object {
					Expression::Identifier(ref id) => {
						...
						{
							self.cjs_module_ident
								.get_or_insert(Span::new(id.span.start, id.span.start + 6));
						}
						if id.name == "exports" && self.is_global_identifier_reference(id) {
							self.cjs_exports_ident
								.get_or_insert(Span::new(id.span.start, id.span.start + 7));
						}
					},
	...
	fn process_identifier_ref_by_scope(&mut self, ident_ref:&IdentifierReference) {
		match self.resolve_identifier_reference(ident_ref) {
			...
			super::IdentifierReferenceKind::Root(root_symbol_id) => {
				// if the identifier_reference is a NamedImport MemberExpr access, we store it
				// as a `MemberExpr` use this flag to avoid insert it as `Symbol` at the
				// same time.
				let mut is_inserted_before = false;

				if self.result.named_imports.contains_key(&root_symbol_id) {
					if let Some((span, props)) =
						self.try_extract_parent_static_member_expr_chain(usize::MAX)
					{
	...
	fn process_global_identifier_ref_by_ancestor(
		...
		match parent {
			AstKind::CallExpression(call_expr) => {
				match ident_ref.name.as_str() {
					"eval" => {
						// TODO: esbuild track has_eval for each scope, this could reduce bailout
						// range, and may improve treeshaking performance. https://github.com/evanw/esbuild/blob/360d47230813e67d0312ad754cad2b6ee09b151b/internal/js_ast/js_ast.go#L1288-L1291
					...
					for ancestor in self.visit_path.iter().rev().skip(1) {
						match ancestor {
							...
							AstKind::SequenceExpression(seq_expr) => {
								...
								// 1. `1, 2, (1, require('a'))` => since the last child contains
								//    `require`, and in the last position, it is still used if it
								//    meant any other astKind
								// 2. `1, 2, (1, require('a')), 1` => since the last child contains
								//    `require`, but it is not in the last position, the state
								//    should change to unused
								let last = seq_expr
									.expressions
									.last()
