	...
	fn visit_member_expression(&mut self, expr:&MemberExpression<'ast>) {
		match expr {
			MemberExpression::StaticMemberExpression(member_expr) => {
				// For member expression like `a.b.c.d`, we will first enter the
				// (object: `a.b.c`, property: `d`) expression.
				...
				match object_symbol_in_top_level {
					// - Import statements are hoisted to the top of the module, so in this time
					//   being, all imports are scanned.
					// - Having empty span will also results to bailout since we rely on span to
					//   identify ast nodes.
					Some(sym_ref)
						if self.result.named_imports.contains_key(&sym_ref)
							&& !expr.span().is_unspanned() =>
					{
						...
						// Don't walk again, otherwise we will add the
						// `object_symbol_in_top_level` again in
		...
		match &node.left {
			...
			ast::AssignmentTarget::StaticMemberExpression(member_expr) => {
				match member_expr.object {
					...
					// `module.exports.test` is also considered as commonjs
					// keyword
	...
	fn visit_call_expression(&mut self, expr:&ast::CallExpression<'ast>) {
		match &expr.callee {
			Expression::Identifier(id_ref) if id_ref.name == "eval" => {
				// TODO: esbuild track has_eval for each scope, this could
				// reduce bailout range, and may improve treeshaking performance. https://github.com/evanw/esbuild/blob/360d47230813e67d0312ad754cad2b6ee09b151b/internal/js_ast/js_ast.go#L1288-L1291
