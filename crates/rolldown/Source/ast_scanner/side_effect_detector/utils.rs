...
impl<'a> SideEffectDetector<'a> {
	/// Get the nearest comment before the `span`, return `None` if no leading
	/// comment is founded.
	///
	///  # Examples
	/// ```javascript
	...
	pub fn leading_comment_for(&self, span:Span) -> Option<(&Comment, &str)> {
		...
		// If there are non-whitespace characters between the `comment` and the
		// `span`, we treat the `comment` not belongs to the `span`.
	...
	match expr {
		...
		Expression::UnaryExpression(e) => {
			match e.operator {
				...
				UnaryOperator::UnaryPlus => PrimitiveType::Number, // Cannot
				// be bigint
				// because
				// that throws
				// an exception
		...
		Expression::AssignmentExpression(e) => {
			match e.operator {
				oxc::syntax::operator::AssignmentOperator::Assign => {
					known_primitive_type(scope, &e.right)
				},
...
pub fn is_primitive_literal(scope:&AstScopes, expr:&Expression) -> bool {
	match expr {
		...
		// Include `+1` / `-1`.
		Expression::UnaryExpression(e)
			if matches!(e.operator, |UnaryOperator::UnaryNegation| {
...
pub fn extract_member_expr_chain<'a>(
	...
	match expr {
		MemberExpression::ComputedMemberExpression(computed_expr) => {
			...
			extract_rest_member_expr_chain(&mut cur, &mut chain, max_len)
				.map(|ref_id| (ref_id, chain))
		},
...
fn extract_rest_member_expr_chain<'a>(
	...
	loop {
		...
		// If chain exceeds the max length, that means we are not interest in
		// this member expression. return `None`
