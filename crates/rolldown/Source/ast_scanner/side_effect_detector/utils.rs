	...
	match expr {
		...
		Expression::UnaryExpression(e) => {
			match e.operator {
				...
				UnaryOperator::UnaryPlus => PrimitiveType::Number, // Cannot be bigint because
...
fn extract_rest_member_expr_chain<'a>(
	...
	loop {
		...
		// If chain exceeds the max length, that means we are not interest in this
		// member expression. return `None`
						...
						match arg {
							...
							ast::Argument::Identifier(id)
								if id.name == "undefined"
									&& scope.is_unresolved(id.reference_id()) =>
							{
								return true;
							},
						...
						if let Some(primitive_ty) = known_primitive_type {
							if matches!(
								...
								PrimitiveType::Number
									...
									| PrimitiveType::Undefined | PrimitiveType::Boolean
							) {
								return true;
							}
						}
