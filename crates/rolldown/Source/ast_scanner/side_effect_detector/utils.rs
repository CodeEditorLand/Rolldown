...
fn merged_known_primitive_types(scope: &AstScopes, left: &Expression, right: &Expression) -> PrimitiveType {
...
pub fn known_primitive_type(scope: &AstScopes, expr: &Expression) -> PrimitiveType {
	match expr {
		...
		Expression::Identifier(id) if id.name == "undefined" && scope.is_unresolved(id.reference_id.get().unwrap()) => {
		...
		Expression::LogicalExpression(e) => match e.operator {
			LogicalOperator::Or | LogicalOperator::And => merged_known_primitive_types(scope, &e.left, &e.right),
		...
		Expression::BinaryExpression(e) => match e.operator {
			...
			BinaryOperator::Addition => {
				...
				} else if !matches!(left, PrimitiveType::Unknown | PrimitiveType::Mixed | PrimitiveType::BigInt)
					&& !matches!(right, PrimitiveType::Unknown | PrimitiveType::Mixed | PrimitiveType::BigInt)
				{
...
pub fn is_primitive_literal(scope: &AstScopes, expr: &Expression) -> bool {
	match expr {
		...
		Expression::Identifier(id) if id.name == "undefined" && scope.is_unresolved(id.reference_id()) => true,
		_ => false,
	}
}
...
pub fn is_side_effect_free_unbound_identifier_ref(
	...
	match bin_expr.operator {
		...
		| BinaryOperator::Inequality => {
			...
			if !(unary.operator == UnaryOperator::Typeof && matches!(unary.argument, Expression::Identifier(_))) {
				return Some(false);
			}
			let string = string.as_string_literal()?;

			if (string.value.eq("undefined") == is_yes_branch)
				== matches!(bin_expr.operator, BinaryOperator::Inequality | BinaryOperator::StrictInequality)
			{
...
pub fn maybe_side_effect_free_global_constructor(scope: &AstScopes, expr: &ast::NewExpression<'_>) -> bool {
	...
	if scope.is_unresolved(ident.reference_id()) {
		match ident.name.as_str() {
			...
			"Date" => match expr.arguments.len() {
				...
				1 => {
					let arg = &expr.arguments[0];
					let known_primitive_type = arg.as_expression().map(|item| known_primitive_type(scope, item));
