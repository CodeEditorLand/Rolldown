use crate::ast_scanner::side_effect_detector::utils::{extract_member_expr_chain, is_primitive_literal};
use oxc::ast::ast::{
	self, Argument, ArrayExpressionElement, AssignmentTarget, AssignmentTargetPattern, BindingPatternKind,
	CallExpression, ChainElement, Expression, IdentifierReference, PropertyKey, UnaryOperator, VariableDeclarationKind,
};
...
use rolldown_utils::global_reference::{
	is_global_ident_ref, is_side_effect_free_member_expr_of_len_three, is_side_effect_free_member_expr_of_len_two,
};
use utils::{
	can_change_strict_to_loose, is_side_effect_free_unbound_identifier_ref, maybe_side_effect_free_global_constructor,
};
...
impl<'a> SideEffectDetector<'a> {
				...
				is_computed && {
					...
					match key_expr {
						match_member_expression!(Expression) => {
							if let Some((ref_id, chain)) = extract_member_expr_chain(key_expr.to_member_expression(), 2)
							{
	...
	fn detect_side_effect_of_expr(&self, expr: &Expression) -> bool {
		match expr {
			...
			Expression::UnaryExpression(unary_expr) => match unary_expr.operator {
				ast::UnaryOperator::Typeof if matches!(unary_expr.argument, Expression::Identifier(_)) => false,
			...
			Expression::LogicalExpression(logic_expr) => match logic_expr.operator {
				ast::LogicalOperator::Or => {
					self.detect_side_effect_of_expr(&logic_expr.left)
						...
						.unwrap_or_default() && self.detect_side_effect_of_expr(&logic_expr.right))
				},
			...
			Expression::ParenthesizedExpression(paren_expr) => self.detect_side_effect_of_expr(&paren_expr.expression),
			...
			Expression::PrivateInExpression(private_in_expr) => self.detect_side_effect_of_expr(&private_in_expr.right),
			...
			Expression::ChainExpression(expr) => match &expr.expression {
				...
				ChainElement::TSNonNullExpression(expr) => self.detect_side_effect_of_expr(&expr.expression),
	...
	fn detect_side_effect_of_array_expr(&self, expr: &ast::ArrayExpression<'_>) -> bool {
		expr.elements.iter().any(|elem| match elem {
			...
			match_expression!(ArrayExpressionElement) => self.detect_side_effect_of_expr(elem.to_expression()),
		})
	}
	...
	fn detect_side_effect_of_var_decl(&self, var_decl: &ast::VariableDeclaration) -> bool {
		match var_decl.kind {
			...
			VariableDeclarationKind::Using => self.detect_side_effect_of_using_declarators(&var_decl.declarations),
				...
				match &declarator.id.kind {
					...
					BindingPatternKind::ArrayPattern(pat) => {
						...
						declarator
							.init
							.as_ref()
							.is_some_and(|init| self.detect_side_effect_of_expr(init))
					},
					BindingPatternKind::BindingIdentifier(_) | BindingPatternKind::AssignmentPattern(_) => declarator
						.init
						.as_ref()
						.is_some_and(|init| self.detect_side_effect_of_expr(init)),
				}
	...
	fn detect_side_effect_of_using_declarators(&self, declarators: &[ast::VariableDeclarator]) -> bool {
	...
	pub fn detect_side_effect_of_stmt(&self, stmt: &ast::Statement) -> bool {
		...
		match stmt {
			oxc::ast::match_declaration!(Statement) => self.detect_side_effect_of_decl(stmt.to_declaration()),
			...
			oxc::ast::match_module_declaration!(Statement) => match stmt.to_module_declaration() {
				ast::ModuleDeclaration::ExportAllDeclaration(_) | ast::ModuleDeclaration::ImportDeclaration(_) => {
			...
			Statement::DoWhileStatement(do_while) => {
				self.detect_side_effect_of_stmt(&do_while.body) || self.detect_side_effect_of_expr(&do_while.test)
			},
			Statement::WhileStatement(while_stmt) => {
				self.detect_side_effect_of_expr(&while_stmt.test) || self.detect_side_effect_of_stmt(&while_stmt.body)
			},
			Statement::IfStatement(if_stmt) => {
				self.detect_side_effect_of_expr(&if_stmt.test)
					...
					|| if_stmt
						.alternate
						.as_ref()
						.is_some_and(|stmt| self.detect_side_effect_of_stmt(stmt))
			},
			Statement::ReturnStatement(ret_stmt) => ret_stmt
				.argument
				.as_ref()
				.is_some_and(|expr| self.detect_side_effect_of_expr(expr)),
			Statement::LabeledStatement(labeled_stmt) => self.detect_side_effect_of_stmt(&labeled_stmt.body),
			...
			Statement::EmptyStatement(_) | Statement::ContinueStatement(_) | Statement::BreakStatement(_) => false,
...
mod test {
	...
	fn get_statements_side_effect(code: &str) -> bool {
		...
		let has_side_effect = ast.program().body.iter().any(|stmt| {
			SideEffectDetector::new(&ast_scopes, false, false, &Arc::new(NormalizedBundlerOptions::default()))
...
let remove15 = class {
	...
	fn extract_first_part_of_member_expr_like_helper(code: &str) -> String {
		...
		SideEffectDetector::extract_first_part_of_member_expr_like(&expr)
			.unwrap()
			.to_string()
	}
}
