...
impl<'a> SideEffectDetector<'a> {
	...
	fn detect_side_effect_of_member_expr(&self, expr:&ast::MemberExpression) -> bool {
		// MemberExpression is considered having side effect by default, unless it's
		// some builtin global variables.
	...
	fn detect_side_effect_of_call_expr(&mut self, expr:&CallExpression) -> bool {
		let is_pure =
			!self.ignore_annotations && self.is_pure_function_or_constructor_call(expr.span);
		...
		match expr {
			...
			Expression::TemplateLiteral(literal) => {
				literal.expressions.iter().any(|expr| {
					// Primitive type detection is more strict and faster than side_effects
					// detection of `Expr`, put it first to fail fast.
					...
					ast::LogicalOperator::Or => {
						self.detect_side_effect_of_expr(&logic_expr.left)
							...
							.unwrap_or_default() && self
								.detect_side_effect_of_expr(&logic_expr.right))
					},
		...
		declarators.iter().any(|decl| {
			decl.init.as_ref().map_or(false, |init| {
				match init {
					...
					// Side effect detection of identifier is different with other position when as
					// initialization of using declaration. Only global variable `undefined` is
			...
			oxc::ast::match_module_declaration!(Statement) => {
				match stmt.to_module_declaration() {
					...
					ast::ModuleDeclaration::ImportDeclaration(_) => {
						// We consider `import ...` has no side effect. However, `import ...` might
						// be rewritten to other statements by the bundler. In that case, we
						// will mark the statement as having side effect in link stage.
						false
					},
