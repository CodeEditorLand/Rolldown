...
use rolldown_utils::global_reference::{
	is_global_ident_ref,
	is_side_effect_free_member_expr_of_len_three,
...
impl<'a> SideEffectDetector<'a> {
						...
						match_member_expression!(Expression) => {
								...
								extract_member_expr_chain(key_expr.to_member_expression(), 2)
							{
								!(chain == ["Symbol", "iterator"]
									&& self.scope.is_unresolved(ref_id))
							} else {
								true
							}
						},
				...
				ClassElement::PropertyDefinition(def) => {
					...
					let value_side_effect = def.r#static
						&& def
							.value
							.as_ref()
							.map_or(false, |init| self.detect_side_effect_of_expr(init));
					value_side_effect
				},
	...
	fn detect_side_effect_of_member_expr(&self, expr:&ast::MemberExpression) -> bool {
		// MemberExpression is considered having side effect by default, unless
		// it's some builtin global variables.
	...
	fn detect_side_effect_of_expr(&mut self, expr:&Expression) -> bool {
		match expr {
			...
			Expression::ObjectExpression(obj_expr) => {
				obj_expr.properties.iter().any(|obj_prop| {
					match obj_prop {
						ast::ObjectPropertyKind::ObjectProperty(prop) => {
							...
							let prop_init_side_effect = prop
								.init
								.as_ref()
								.map_or(false, |expr| self.detect_side_effect_of_expr(expr));
						...
						ast::ObjectPropertyKind::SpreadProperty(_) => {
							// ...[expression] is considered as having side
							// effect. see crates/rolldown/tests/
							// fixtures/rollup/object-spread-side-effect
							true
						},
					}
				})
			...
			Expression::TemplateLiteral(literal) => {
				literal.expressions.iter().any(|expr| {
					// Primitive type detection is more strict and faster than
					// side_effects detection of `Expr`, put it first to
					// fail fast.
			...
			Expression::BinaryExpression(binary_expr) => {
				// For binary expressions, both sides could potentially have
				// side effects
		...
		declarators.iter().any(|decl| {
			decl.init.as_ref().map_or(false, |init| {
				match init {
					...
					// Side effect detection of identifier is different with
					// other position when as initialization of using
					// declaration. Only global variable `undefined` is
					// considered as side effect free.
			...
			oxc::ast::match_module_declaration!(Statement) => {
				match stmt.to_module_declaration() {
					...
					ast::ModuleDeclaration::ImportDeclaration(_) => {
						// We consider `import ...` has no side effect. However,
						// `import ...` might be rewritten to other statements
						// by the bundler. In that case, we will mark the
						// statement as having side effect in link stage.
						false
					},
					...
					ast::ModuleDeclaration::ExportNamedDeclaration(named_decl) => {
						if named_decl.source.is_some() {
							// `export { ... } from '...'` is considered as side
							// effect.
							true
						} else {
							named_decl
			...
			Statement::IfStatement(if_stmt) => {
				self.detect_side_effect_of_expr(&if_stmt.test)
					...
					|| if_stmt
						.alternate
						.as_ref()
						.map_or(false, |stmt| self.detect_side_effect_of_stmt(stmt))
			},
					...
					|| switch_stmt.cases.iter().any(|case| {
						case.test
							...
							|| case
								.consequent
								.iter()
								.any(|stmt| self.detect_side_effect_of_stmt(stmt))
					})
			},
