...
impl<'me, 'ast:'me> AstScanner<'me, 'ast> {
	pub fn update_dynamic_import_binding_usage_info(
		...
		// panic because if program reached here, means the BindingIdentifier has
		// referenced the IdentifierReference, but IdentifierReference did not saved
		// the related `SymbolId` Something wrong with semantic analyze
		...
		match self.dynamic_import_usage_info.dynamic_import_exports_usage.entry(rec_idx) {
			std::collections::hash_map::Entry::Occupied(mut occ) => {
				match partial_name {
					Some(name) => {
						occ.get_mut().merge(DynamicImportExportsUsage::Single(name.into()))
					},
	...
	pub fn init_dynamic_import_binding_usage_info(
		...
		let init_set = match self.visit_path.last()? {
			AstKind::MemberExpression(member_expr) => {
				self.init_dynamic_import_usage_with_member_expr(
					member_expr,
					ancestor_len,
					import_record_idx,
				)
		...
		match init_set {
			Some(init_set) => {
				self.dynamic_import_usage_info
			...
			ast::BindingPatternKind::ObjectPattern(obj) => {
				...
				if let Some(rest) = &obj.rest {
					match &rest.argument.kind {
						...
						// If the rest argument is not a BindingIdentifier, this is an unexpected
						// case because '...' must be followed by an identifier in declaration
						// contexts.
						_ => unreachable!(),
					}
				}
