...
pub struct ScanResult {
	...
	/// used for check if current class decl symbol was referenced in its class
	/// scope We needs to record the info in ast scanner since after that the
	/// ast maybe touched, etc (naming deconflict)
	pub self_referenced_class_decl_symbol_ids:FxHashSet<SymbolId>,
	/// hashbang only works if it's literally the first character.So we need to
	/// generate it in chunk level rather than module level, or a syntax error
	/// will be raised if there are multi modules has hashbang. Storing the
	/// span of hashbang used for hashbang codegen in chunk level
	pub hashbang_range:Option<Span>,
	pub has_star_exports:bool,
	/// we don't know the ImportRecord related ModuleIdx yet, so use
	/// ImportRecordIdx as key temporarily
...
pub struct AstScanner<'me, 'ast> {
	...
	/// The reason why we can't reuse `cjs_exports_ident` and `cjs_module_ident`
	/// is that any `module` or `exports` in the top-level scope should be
	/// treated as a commonjs module. `cjs_exports_ident` and
	/// `cjs_module_ident` only only recorded when they are appear in
...
impl<'me, 'ast:'me> AstScanner<'me, 'ast> {
	...
	pub fn new(
		...
		// This is used for converting "export default foo;" => "var default_symbol =
		// foo;"
		let legitimized_repr_name = legitimize_identifier_name(repr_name);

		let default_export_ref = symbol_ref_db.create_facade_root_symbol_ref(
			concat_string!(legitimized_repr_name, "_default").into(),
		);
		// This is used for converting "export default foo;" => "var
		// [default_export_ref] = foo;" And we consider [default_export_ref] never get
		// reassigned.
		default_export_ref
			.flags_mut(&mut symbol_ref_db)
			.insert(SymbolRefFlags::IS_NOT_REASSIGNED);
		...
		let result = ScanResult {
			...
			stmt_infos:{
				...
				// The first `StmtInfo` is used to represent the statement that declares and
				// constructs Module Namespace Object
	...
	pub fn scan(mut self, program:&Program<'ast>) -> BuildResult<ScanResult> {
		...
		if self.esm_export_keyword.is_some() {
			...
			// TODO(hyf0): Should add warnings if the module type doesn't satisfy the
			// exports kind.
			match self.module_type {
				...
				| ModuleDefFormat::EsmPackageJson
				| ModuleDefFormat::EsmMts => {
					exports_kind = ExportsKind::Esm;
				},
			...
			for (name, symbol_id) in self.scopes.get_bindings(self.scopes.root_scope_id()) {
				...
				if !scanned_symbols_in_root_scope.remove(&symbol_ref) {
					return Err(anyhow::format_err!(
						"Symbol ({name:?}, {symbol_id:?}, {scope_id:?}) is declared in the \
						 top-level scope but doesn't get scanned by the scanner",
					))?;
				}
			}
			// if !scanned_top_level_symbols.is_empty() {
			//   return Err(anyhow::format_err!(
			//     "Some top-level symbols are scanned by the scanner but not
			// declared in the top-level scope: {scanned_top_level_symbols:?}",
			//   ));
			// }
		}
	...
	fn set_esm_export_keyword(&mut self, span:Span) { self.esm_export_keyword.get_or_insert(span); }

	fn add_declared_id(&mut self, id:SymbolId) {
		self.current_stmt_info.declared_symbols.push((self.idx, id).into());
	}

	fn get_root_binding(&self, name:&str) -> Option<SymbolId> { self.scopes.get_root_binding(name) }

	/// `is_dummy` means if it the import record is created during ast
	/// transformation.
	fn add_import_record(
		...
		// If 'foo' in `import ... from 'foo'` is finally a commonjs module, we will
		// convert the import statement to `var import_foo = __toESM(require_foo())`,
		// so we create a symbol for `import_foo` here. Notice that we just create the
		// symbol. If the symbol is finally used would be determined in the linking
		// stage.
	...
	/// - Cjs compatibility. We need a [generated] binding to holds the value
	///   reexport from commonjs. For example
	/// ```js
	...
	/// ```
	/// `export { foo } from 'commonjs'` would be converted to `const
	/// import_commonjs = require()` in the linking stage.
	fn add_re_export(
		&mut self,
		export_name:&str,
		imported:&str,
		...
		let generated_imported_as_ref =
			self.result
				.symbol_ref_db
				.create_facade_root_symbol_ref(if export_name == "default" {
		...
		let local_binding_for_default_export = match &decl.declaration {
			...
			ast::ExportDefaultDeclarationKind::FunctionDeclaration(fn_decl) => {
				fn_decl.id.as_ref().map(|id| {
					(rolldown_ecmascript_utils::BindingIdentifierExt::expect_symbol_id(id), id.span)
				})
			},
			...
			ast::ModuleDeclaration::ExportDefaultDeclaration(decl) => {
				...
				match &decl.declaration {
					ast::ExportDefaultDeclarationKind::ClassDeclaration(class) => {
						...
						// walk::walk_declaration(self,
						// &ast::Declaration::ClassDeclaration(func));
					},
					_ => {},
				}
			},
	...
	/// return a `Some(SymbolRef)` if the identifier referenced a top level
	/// `IdentBinding`
