...
pub struct AstScanner<'me> {
	...
	/// The reason why we can't reuse `cjs_exports_ident` and
	/// `cjs_module_ident` is that any `module` or `exports` in the top-level
	/// scope should be treated as a commonjs module. `cjs_exports_ident` and
...
impl<'me> AstScanner<'me> {
	...
	pub fn new(
		...
		let symbol_id_for_default_export_ref = symbols.create_symbol(
		...
		let result = ScanResult {
			...
			stmt_infos:{
				...
				// The first `StmtInfo` is used to represent the statement that
				// declares and constructs Module Namespace Object
	...
	pub fn scan(mut self, program:&Program<'_>) -> UnhandleableResult<ScanResult> {
		...
		if self.esm_export_keyword.is_some() {
			...
			// TODO(hyf0): Should add warnings if the module type doesn't
			// satisfy the exports kind.
	...
	fn add_import_record(
		...
		// If 'foo' in `import ... from 'foo'` is finally a commonjs module, we
		// will convert the import statement to `var import_foo =
		// __toESM(require_foo())`, so we create a symbol for `import_foo` here.
		// Notice that we just create the symbol. If the symbol is finally
		// used would be determined in the linking stage.
	...
	fn add_re_export(
		...
		// We will pretend `export { [imported] as [export_name] }` to be
		// `import `
	...
	fn add_star_re_export(
		...
		let generated_imported_as_ref = (
			self.idx,
			self.symbols.create_symbol(export_name.into(), self.scopes.root_scope_id()),
		)
			.into();
		self.current_stmt_info.declared_symbols.push(generated_imported_as_ref);
		let name_import = NamedImport {
		...
		let local_binding_for_default_export = match &decl.declaration {
			...
			ast::ExportDefaultDeclarationKind::FunctionDeclaration(fn_decl) => {
				fn_decl.id.as_ref().map(|id| {
					(rolldown_ecmascript::BindingIdentifierExt::expect_symbol_id(id), id.span)
				})
			},
	...
	/// resolve the symbol from the identifier reference, and return if it is a
	/// top level symbol
	fn resolve_identifier_to_top_level_symbol(
		...
		match symbol_id {
			...
			None => {
				// atom cmp is not `O(1)`, so if the module already contains
				// both `module` and `exports`, don't need to check it
				// again.
