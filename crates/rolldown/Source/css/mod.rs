	...
	for lexed_dep in lexed_deps {
		match lexed_dep {
			...
			css_module_lexer::Dependency::Url { request, range, kind } => {
				// css_module_lexer return span of `request` if kind is `string`, return whole
				// span of `url(dep)`, if the kind is function so we need to tweak a little
				// to get the correct span we want that used to replace asset filename
