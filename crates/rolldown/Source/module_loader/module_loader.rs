...
use arcstr::ArcStr;
use oxc::{index::IndexVec, minifier::ReplaceGlobalDefinesConfig, span::Span};
use rolldown_common::{
...
use rolldown_utils::{ecma_script::legitimize_identifier_name, rustc_hash::FxHashSetExt};
use rustc_hash::{FxHashMap, FxHashSet};

use super::{
	module_task::{ModuleTask, ModuleTaskOwner},
	runtime_module_task::RuntimeModuleTask,
	task_context::TaskContextMeta,
	task_result::NormalModuleTaskResult,
	Msg,
};
use crate::{
	module_loader::{runtime_module_task::RuntimeModuleTaskResult, task_context::TaskContext},
...
impl ModuleLoader {
	pub fn new(
		...
		let meta = TaskContextMeta {
			replace_global_define_config:if options.define.is_empty() {
				...
				Some(ReplaceGlobalDefinesConfig::new(&options.define).map_err(|errs| {
					// TODO: maybe we should give better diagnostics here.
					// since oxc return `Vec<OxcDiagnostic>`
					anyhow::format_err!(
		...
		// so we are using spawn instead of spawn_blocking here to avoid an
		// additional blocking thread creation within tokio
		...
		match self.visited.entry(resolved_id.id.clone()) {
			...
			std::collections::hash_map::Entry::Vacant(not_visited) => {
				if resolved_id.is_external {
					let idx =
						self.intermediate_normal_modules.alloc_ecma_module_idx(&mut self.symbols);
					...
					let external_module_side_effects =
						if let Some(hook_side_effects) = resolved_id.side_effects {
							...
							match self.options.treeshake {
								...
								rolldown_common::TreeshakeOptions::Option(ref opt) => {
									match opt.module_side_effects {
					...
					let task =
						ModuleTask::new(Arc::clone(&self.shared_context), idx, resolved_id, owner);
					#[cfg(target_family = "wasm")]
					{
						let handle = tokio::runtime::Handle::current();
	...
	pub async fn fetch_all_modules(
		...
		while self.remaining > 0 {
			...
			match msg {
						...
						raw_import_records
							...
							.map(|(raw_rec, info)| {
								...
								let owner = ModuleTaskOwner::new(
									...
									Span::new(
										raw_rec.module_request_start,
										raw_rec.module_request_end(),
									),
								);
								let id = self.try_spawn_new_task(info, Some(owner));
								// Dynamic imported module will be considered as an
								// entry
								self.intermediate_normal_modules.importers[id].push(
									ImporterRecord {
										kind:raw_rec.kind,
				...
				Msg::RuntimeNormalModuleDone(task_result) => {
					let RuntimeModuleTaskResult { ast_symbols, mut module, runtime, ast } =
						task_result;
					let ast_idx =
						self.intermediate_normal_modules.index_ecma_ast.push((ast, module.idx));
				...
				// Expect cast to u32, since we are not going to have more than
				// 2^32 tasks, or the `remaining` will overflow
				#[allow(clippy::cast_possible_truncation)]
				Msg::Panics(err) => {
					// `self.remaining -1` for the panic task it self
					self.remaining -= 1;
					// gracefully shutdown all working thread, only receive and
					// do not spawn
					while self.remaining > 0 {
		...
		let modules:IndexVec<ModuleIdx, Module> = self
			...
			.map(|(id, mut module)| {
				...
				if let Some(module) = module.as_normal_mut() {
					...
					// The `dynamic_importers/importers` should be added after
					// `module_parsed` hook.
					for importer in
						std::mem::take(&mut self.intermediate_normal_modules.importers[id])
					{
		...
		// if `inline_dynamic_imports` is set to be true, here we should not put
		// dynamic imports to entries
		if !self.options.inline_dynamic_imports {
			let mut dynamic_import_entry_ids =
				dynamic_import_entry_ids.into_iter().collect::<Vec<_>>();
			dynamic_import_entry_ids.sort_unstable_by_key(|id| modules[*id].stable_id());

			entry_points.extend(
				dynamic_import_entry_ids
					.into_iter()
					.map(|id| EntryPoint { name:None, id, kind:EntryPointKind::DynamicImport }),
			);
		}
