...
use rolldown_utils::{ecmascript::legitimize_identifier_name, rustc_hash::FxHashSetExt};
...
use crate::{
	SharedOptions,
	SharedResolver,
	module_loader::task_context::TaskContext,
...
impl ModuleLoader {
	pub fn new(
		...
		// so we are using spawn instead of spawn_blocking here to avoid an additional
		// blocking thread creation within tokio
	...
	pub async fn fetch_all_modules(
		...
		self.shared_context
			.plugin_driver
			.set_context_load_modules_tx(Some(self.tx.clone()))
			.await;
			...
			match msg {
				ModuleLoaderMsg::NormalModuleDone(task_result) => {
					...
					let mut dynamic_import_rec_exports_usage = ecma_related
						...
						.unwrap_or_default();
					let import_records:IndexVec<
						ImportRecordIdx,
						rolldown_common::ResolvedImportRecord,
					> = raw_import_records
						...
						.map(|((rec_idx, raw_rec), info)| {
							...
							// defer usage merging, since we only have one consumer, we should keep
							// action during fetching as simple as possible
		...
		// if `inline_dynamic_imports` is set to be true, here we should not put dynamic
		// imports to entries
