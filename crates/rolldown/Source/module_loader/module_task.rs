...
impl ModuleTask {
	...
	async fn run_inner(&mut self) -> BuildResult<()> {
		...
		// TODO: module type should be able to updated by transform hook, for now we
		// don't impl it.
		if let ModuleType::Custom(_) = module_type {
			// TODO: should provide some diagnostics for user how they should handle the
			// module type. e.g.
		...
		let css_view = if matches!(module_type, ModuleType::Css) {
			...
			// FIXME: This makes creating `EcmaView` rely on creating `CssView` first, while
