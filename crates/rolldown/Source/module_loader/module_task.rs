...
use crate::{
	...
	types::module_factory::{
		CreateModuleContext,
		CreateModuleViewArgs,
		CreateModuleViewReturn,
		ModuleViewFactory,
	},
...
impl ModuleTask {
		...
		match self.run_inner().await {
			Ok(()) => {
				if !self.errors.is_empty() {
					self.ctx
						.tx
						.send(Msg::BuildErrors(self.errors))
						.await
	...
	async fn run_inner(&mut self) -> Result<()> {
		...
		// Run plugin load to get content first, if it is None using read fs as
		// fallback.
		...
		// TODO: module type should be able to updated by transform hook, for
		// now we don't impl it.
		if let ModuleType::Custom(_) = module_type {
			// TODO: should provide some diagnostics for user how they should
			// handle the module type. e.g.
			// sass -> recommended npm install `sass` etc
			return Err(anyhow::format_err!(
				"`{:?}` is not specified module type,  rolldown can't handle this asset \
				 correctly. Please use the load/transform hook to transform the resource",
				self.resolved_id.id
			));
		};
		...
		let css_view = if matches!(module_type, ModuleType::Css) {
			...
			// FIXME: This makes creating `EcmaView` rely on creating `CssView`
			// first, while they should be done in parallel.
		...
		let CreateModuleViewReturn { view, resolved_deps, ecma_related, raw_import_records } =
			match ret {
				Ok(ret) => ret,
				Err(errs) => {
		...
		if let Err(_err) = self
			...
			.await
		{
			// The main thread is dead, nothing we can do to handle these send
			// failures.
		}
