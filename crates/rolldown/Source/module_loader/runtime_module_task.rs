use arcstr::ArcStr;
use oxc::{index::IndexVec, span::SourceType};
use rolldown_common::{
...
impl RuntimeModuleTask {
	...
	pub fn run(mut self) -> anyhow::Result<()> {
		let source:ArcStr =
			arcstr::literal!(include_str!("../runtime/runtime-without-comments.js"));
		...
		let module = NormalModule {
			...
			ecma_view:EcmaView {
				...
				is_included:false,
				sourcemap_chain:vec![],
				// The internal runtime module `importers/imported` should be
				// skip.
				importers:vec![],
		...
		if let Err(_err) = self.tx.try_send(Msg::RuntimeNormalModuleDone(RuntimeModuleTaskResult {
			...
			// hyf0: If main thread is dead, we should handle errors of main
			// thread. So we just ignore the error here.
		};
