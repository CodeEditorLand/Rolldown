...
impl GenerateStage<'_> {
	...
	pub fn apply_advanced_chunks(
		...
		let Some(match_groups) = chunking_options.groups.as_ref().map(|inner| inner.iter().collect::<Vec<_>>()) else {
			return;
		};
		...
		for normal_module in self.link_output.module_table.modules.iter().filter_map(Module::as_normal) {
			...
			for (match_group_index, match_group) in match_groups.iter().copied().enumerate() {
				let is_matched = match_group.test.as_ref().is_none_or(|test| test.matches(&normal_module.id));

				if !is_matched {
					continue;
				}

				let allow_min_module_size = match_group.min_module_size.map_or(chunking_options.min_module_size, Some);
				let allow_max_module_size = match_group.max_module_size.map_or(chunking_options.max_module_size, Some);

				let is_min_module_size_satisfied =
					allow_min_module_size.is_none_or(|min_module_size| normal_module.size() >= min_module_size);
				let is_max_module_size_satisfied =
					allow_max_module_size.is_none_or(|max_module_size| normal_module.size() <= max_module_size);
				...
				let module_group_idx = name_to_module_group.entry(group_name.clone()).or_insert_with(|| {
		...
		while let Some(this_module_group) = module_groups.pop() {
					...
					if next_right_index + 1 < next_left_index {
						...
						loop {
							if next_left_index <= next_right_index && split_size < allow_max_size {
								split_size += self.link_output.module_table.modules[modules[next_left_index as usize]]
									.size() as f64;
								next_left_index += 1;
							} else {
								break;
			...
			let chunk = Chunk::new(
				...
				None,
				None,
				index_splitting_info[this_module_group.modules.iter().next().copied().expect("must have one")]
					.bits
					.clone(),
				vec![],
				ChunkKind::Common,
...
fn add_module_and_dependencies_to_group_recursively(
	...
	for dep in &module_metas[module_idx].dependencies {
		add_module_and_dependencies_to_group_recursively(module_group, *dep, module_metas, module_table, visited);
	}
}
