...
use rolldown_common::{
	ChunkIdx, ChunkKind, CssAssetNameReplacer, ImportMetaRolldownAssetReplacer, Module, PreliminaryFilename,
...
impl<'a> GenerateStage<'a> {
	...
	pub async fn generate(&mut self) -> BuildResult<BundleOutput> {
		...
		let index_chunk_id_to_name = self.generate_chunk_name_and_preliminary_filenames(&mut chunk_graph).await?;
		...
		chunk_graph.chunk_table.par_iter_mut().for_each(|chunk| {
			deconflict_chunk_symbols(chunk, self.link_output, self.options.format, &index_chunk_id_to_name);
		});
				...
				match chunk.kind {
					ChunkKind::EntryPoint { module: entry_module_id, is_user_defined, .. } => {
						...
						let generated = if is_user_defined {
							...
							sanitize_filename
								.call(&module.id().as_path().representative_file_name())
								.await?
						};
						Ok(generated)
					},
	...
	pub fn patch_asset_modules(&mut self, chunk_graph: &ChunkGraph) {
		chunk_graph.chunk_table.iter().for_each(|chunk| {
			...
			chunk.modules.iter().for_each(|module_idx| {
				...
				if let Some(css_view) = module.as_normal_mut().and_then(|normal_module| normal_module.css_view.as_mut())
				{
					for (idx, record) in css_view.import_records.iter_enumerated().filter(|(_idx, rec)| !rec.is_dummy())
					{
