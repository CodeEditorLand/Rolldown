...
impl<'a> GenerateStage<'a> {
	...
	async fn generate_chunk_name_and_preliminary_filenames(
		...
		for chunk_id in &chunk_graph.sorted_chunk_idx_vec {
			...
			// Notice we didn't used deconflict name here, chunk names are
			...
			let filename_template =
				chunk.filename_template(self.options, &pre_rendered_chunk).await?;
			...
			chunk.preliminary_filename =
				Some(PreliminaryFilename::new(preliminary, hash_placeholder));
