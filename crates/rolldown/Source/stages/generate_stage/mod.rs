...
use rolldown_std_utils::{OptionExt, PathBufExt, PathExt};
use rolldown_utils::{
	concat_string,
...
impl<'a> GenerateStage<'a> {
	...
	async fn generate_chunk_name_and_preliminary_filenames(
		...
		let mut index_pre_generated_names:IndexVec<ChunkIdx, ArcStr> = chunk_graph
			...
			.map(|chunk| {
				...
				match chunk.kind {
					...
					ChunkKind::Common => {
						// - rollup use the first entered/last executed module as the `[name]` of
						//   common chunks.
						// - esbuild always use 'chunk' as the `[name]`. However we try to make the
						//   name more meaningful here.
				...
				loop {
					match used_name_counts.entry(candidate.clone()) {
						Entry::Occupied(mut occ) => {
							...
							candidate = ArcStr::from(
								concat_string!(name, itoa::Buffer::new().format(next_count))
									.as_str(),
							);
						},
		...
		for chunk_id in &chunk_graph.sorted_chunk_idx_vec {
			...
			// Notice we didn't used deconflict name here, chunk names are allowed to be
			// duplicated.
			...
			let pre_rendered_chunk =
				generate_pre_rendered_chunk(chunk, self.link_output, self.options);
			...
			chunk
				...
				.iter()
				.copied()
				.filter_map(|idx| modules[idx].as_normal())
				.for_each(|module| {
					if module.asset_view.is_some() {
						...
						let name =
							module.id.as_path().file_stem().and_then(|s| s.to_str()).unpack();
	...
	pub fn patch_asset_modules(&mut self, chunk_graph:&ChunkGraph) {
		chunk_graph.chunk_table.iter().for_each(|chunk| {
			...
			chunk.asset_preliminary_filenames.iter().for_each(|(module_idx, preliminary)| {
				let Module::Normal(module) =
					&mut self.link_output.module_table.modules[*module_idx]
				else {
					return;
				};
			...
			chunk.modules.iter().for_each(|module_idx| {
					...
					module.as_normal_mut().and_then(|normal_module| normal_module.css_view.as_mut())
				{
					for (idx, record) in css_view.import_records.iter_enumerated() {
						if let Some(asset_filename) =
							module_idx_to_filenames.get(&record.resolved_module)
						{
							let span = css_view.record_idx_to_span[idx];
							css_view.mutations.push(Box::new(CssAssetNameReplacer {
								span,
								asset_name:asset_filename.clone(),
							}));
						}
					}
				}
