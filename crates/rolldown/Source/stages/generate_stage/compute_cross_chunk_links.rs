...
impl<'a> GenerateStage<'a> {
	...
	pub fn compute_cross_chunk_links(&mut self, chunk_graph:&mut ChunkGraph) {
		...
		index_imports_from_other_chunks
			.iter_enumerated()
			.for_each(|(chunk_id, importee_map)| {
				chunk_graph.chunk_table[chunk_id].require_binding_names_for_other_chunks =
					importee_map
						.keys()
						.map(|id| {
		...
		chunks_iter.par_bridge().for_each(
			|(
				...
				chunk.modules.iter().copied().for_each(|module_id| {
					let Module::Normal(module) = &self.link_output.module_table.modules[module_id]
					else {
						return;
					};
					module
						...
						.inspect(|rec| {
								...
								&self.link_output.module_table.modules[rec.resolved_module]
							{
								...
								// TODO: Is that possible that the module of the record is a
								// external module?
								if !importee_module.is_included {
									return;
								}
								if matches!(rec.kind, ImportKind::DynamicImport) {
									let importee_chunk = chunk_graph.module_to_chunk
										[importee_module.idx]
							...
							match reference_ref {
								rolldown_common::SymbolOrMemberExprRef::Symbol(referenced) => {
									let mut canonical_ref =
										symbols.par_canonical_ref_for(*referenced);
									if let Some(namespace_alias) =
										&symbols.get(canonical_ref).namespace_alias
									{
								...
								rolldown_common::SymbolOrMemberExprRef::MemberExpr(member_expr) => {
									if let Some(sym_ref) = member_expr.resolved_symbol_ref(
										&self.link_output.metas[module.idx]
											.resolved_member_expr_refs,
									) {
										let mut canonical_ref =
											self.link_output.symbols.par_canonical_ref_for(sym_ref);
										...
										// It would be rewrite to `undefined` in
										// the final code, so we don't need to
										// include anything to make `undefined`
										// work.
									}
								},
							};
				...
				if let ChunkKind::EntryPoint { module: entry_id, .. } = &chunk.kind {
					let entry =
						&self.link_output.module_table.modules[*entry_id].as_normal().unwrap();
					...
					if !matches!(entry_meta.wrap_kind, WrapKind::Cjs) {
						for export_ref in entry_meta.resolved_exports.values() {
							let mut canonical_ref =
								symbols.par_canonical_ref_for(export_ref.symbol_ref);
							let symbol = symbols.get(canonical_ref);
					...
					if !matches!(entry_meta.wrap_kind, WrapKind::None) {
						depended_symbols.insert(
							entry_meta.wrapper_ref.expect("cjs should be wrapped in esm output"),
						);
					}
					...
					{
						depended_symbols
							.insert(self.link_output.runtime.resolve_symbol("__toCommonJS"));
		...
		for (chunk_id, symbol_list) in chunk_id_to_symbols_vec {
			for declared in symbol_list {
				...
				debug_assert!(
					...
					"Symbol: {:?}, {:?} in {:?} should only belong to one chunk. Existed {:?}, \
					 new {chunk_id:?}",
					symbol.name,
					declared,
	...
	fn compute_chunk_imports(
		...
		chunk_graph.chunk_table.iter_enumerated().for_each(|(chunk_id, chunk)| {
			...
			// If this is an entry point, make sure we import all chunks
			// belonging to this entry point, even if there are no imports.
			// We need to make sure these chunks are evaluated for their
			// side effects too.
				...
				chunk_graph
					...
					.for_each(|(importee_chunk_id, _)| {
						...
						let imports_from_other_chunks =
							&mut index_imports_from_other_chunks[chunk_id];
	...
	fn deconflict_exported_names(
		...
		// Generate cross-chunk exports. These must be computed before
		// cross-chunk imports because of export alias renaming, which must
		// consider all export aliases simultaneously to avoid collisions.
		let mut name_count =
