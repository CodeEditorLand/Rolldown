...
use rolldown_common::{
	ChunkIdx, ChunkKind, CrossChunkImportItem, ExportsKind, ImportKind, ImportRecordMeta, Module, ModuleIdx,
...
type IndexChunkImportsFromExternalModules = IndexVec<ChunkIdx, FxHashMap<ModuleIdx, Vec<NamedImport>>>;
...
type IndexImportsFromOtherChunks = IndexVec<ChunkIdx, FxHashMap<ChunkIdx, Vec<CrossChunkImportItem>>>;
...
impl GenerateStage<'_> {
	...
	pub fn compute_cross_chunk_links(&mut self, chunk_graph: &mut ChunkGraph) {
		...
		let mut index_imports_from_other_chunks: IndexImportsFromOtherChunks =
			index_vec![FxHashMap::<ChunkIdx, Vec<CrossChunkImportItem>>::default(); chunk_graph.chunk_table.len()];
		...
		let index_sorted_imports_from_other_chunks = index_imports_from_other_chunks
			...
			.map(|(_chunk_id, importee_map)| {
				importee_map
					.into_iter()
					.sorted_by_key(|(importee_chunk_id, _)| chunk_graph.chunk_table[*importee_chunk_id].exec_order)
					.collect_vec()
			})
			.collect::<Vec<_>>();
		...
		.for_each(
			|(
				...
				chunk.cross_chunk_dynamic_imports = cross_chunk_dynamic_imports.into_iter().collect::<Vec<_>>();
			},
		);
	}
	...
	fn collect_depended_symbols(
		...
		chunks_iter.par_bridge().for_each(
			|((chunk_id, chunk), depended_symbols, imports_from_external_modules, cross_chunk_dynamic_imports)| {
				...
				chunk.modules.iter().copied().for_each(|module_id| {
					...
					module
						...
						.filter_map(|rec| self.link_output.module_table.modules[rec.resolved_module].as_external())
						.for_each(|importee| {
					...
					module.named_imports.iter().for_each(|(_, import)| {
						...
						if let Module::External(importee) = &self.link_output.module_table.modules[rec.resolved_module]
						{
							imports_from_external_modules
								.entry(importee.idx)
								.or_default()
									...
									match member_expr.resolved_symbol_ref(
										...
										Some(sym_ref) => {
											let mut canonical_ref =
												self.link_output.symbol_db.canonical_ref_for(sym_ref);
				...
				if let Some(entry_id) = &chunk.entry_module_idx() {
					...
					if !matches!(entry_meta.wrap_kind, WrapKind::None) {
						depended_symbols.insert(entry_meta.wrapper_ref.expect("cjs should be wrapped in esm output"));
					}
	...
	fn deconflict_exported_names(
		...
		let mut name_count = FxHashMap::with_capacity(index_chunk_exported_symbols.iter().map(FxHashSet::len).sum());
		...
		for (chunk_id, chunk) in chunk_graph.chunk_table.iter_mut_enumerated() {
			for chunk_export in index_chunk_exported_symbols[chunk_id]
				...
				.copied()
			{
				let original_name: rolldown_rstr::Rstr = chunk_export.name(&self.link_output.symbol_db).to_rstr();
