...
impl GenerateStage<'_> {
	...
	pub async fn generate_chunks(&mut self) -> ChunkGraph {
		...
		chunk_graph.chunk_table.iter_mut().for_each(|chunk| {
			chunk
				.modules
				.sort_unstable_by_key(|module_id| self.link_output.module_table.modules[*module_id].exec_order());
		});
			...
			.sorted_by(|a, b| {
				...
				match (&a.kind, &b.kind) {
					...
					(ChunkKind::EntryPoint { module: a_module_id, .. }, ChunkKind::Common) => {
						let a_module_exec_order = self.link_output.module_table.modules[*a_module_id].exec_order();
					...
					(ChunkKind::Common, ChunkKind::EntryPoint { module: b_module_id, .. }) => {
						let b_module_exec_order = self.link_output.module_table.modules[*b_module_id].exec_order();
