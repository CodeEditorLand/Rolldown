...
impl<'a> GenerateStage<'a> {
	...
	pub async fn generate_chunks(&mut self) -> anyhow::Result<ChunkGraph> {
		if matches!(self.options.format, OutputFormat::Iife | OutputFormat::Umd) {
			let user_defined_entry_count = self
				.link_output
				.entries
				.iter()
				...
				.count();
			debug_assert!(
				user_defined_entry_count == 1,
				"IIFE/UMD format only supports one entry point"
			);
		}
		...
		// If we are in test environment, to make the runtime module always fall into a
		// standalone chunk, we create a facade entry point for it.
		...
		for (entry_index, entry_point) in self.link_output.entries.iter().enumerate() {
			...
			bits.set_bit(count);
			let Module::Normal(module) = &self.link_output.module_table.modules[entry_point.id]
			else {
				continue;
			};
		...
		// Determine which modules belong to which chunk. A module could belong to
		// multiple chunks.
			...
			self.link_output.module_table.modules.iter().filter_map(Module::as_normal)
		{
			...
			debug_assert!(
				!bits.is_empty(),
				"Empty bits means the module is not reachable, so it should bail out with \
				 `is_included: false` {:?}",
		...
		// The esbuild using `Chunk#bits` to sorted chunks, but the order of
		// `Chunk#bits` is not stable, eg `BitSet(0) 00000001_00000000` > `BitSet(8)
		// 00000000_00000001`. It couldn't ensure the order of dynamic chunks and common
		// chunks. Consider the compare `Chunk#exec_order` should be faster than
		// `Chunk#bits`, we use `Chunk#exec_order` to sort chunks. Note Here could be
		// make sure the order of chunks.
	...
	fn apply_advanced_chunks(
		...
		// `ModuleGroup` is a temporary representation of `Chunk`. A valid `ModuleGroup`
		// would be converted to a `Chunk` in the end.
		struct ModuleGroup {
			name:ArcStr,
		...
		// These two sort ensure higher priority group goes first. If two groups have
		// the same priority, the one with the lower index goes first.
		...
		while let Some(this_module_group) = module_groups.pop() {
			...
			this_module_group.modules.iter().copied().for_each(|module_idx| {
				...
				chunk_graph.chunk_table[chunk_idx]
					.bits
					.union(&index_splitting_info[module_idx].bits);
