...
impl<'a> GenerateStage<'a> {
	...
	pub async fn generate_chunks(&mut self) -> anyhow::Result<ChunkGraph> {
		if matches!(self.options.format, OutputFormat::Iife) {
			...
			debug_assert!(
				...
				"IIFE format only supports one entry point"
			);
		}
		...
		// If we are in test environment, to make the runtime module always fall
		// into a standalone chunk, we create a facade entry point for it.
		...
		// Determine which modules belong to which chunk. A module could belong
		// to multiple chunks.
		...
		// `Chunk#bits` is not stable, eg `BitSet(0) 00000001_00000000` >
		// `BitSet(8) 00000000_00000001`. It couldn't ensure the order of
		// dynamic chunks and common chunks. Consider the compare
		// `Chunk#exec_order` should be faster than `Chunk#bits`, we use
		// `Chunk#exec_order` to sort chunks. Note Here could be make sure the
		// order of chunks.
	...
	fn apply_advanced_chunks(
		...
		// `ModuleGroup` is a temporary representation of `Chunk`. A valid
		// `ModuleGroup` would be converted to a `Chunk` in the end.
		struct ModuleGroup {
			name:ArcStr,
		...
		// These two sort ensure higher priority group goes first. If two groups
		// have the same priority, the one with the lower index goes first.
