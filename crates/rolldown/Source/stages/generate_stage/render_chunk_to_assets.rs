				...
				.map(|((chunk_idx, chunk), module_id_to_codegen_ret)| {
					async move {
						...
						let mut ctx = GenerateContext {
							...
							plugin_driver:self.plugin_driver,
							warnings:vec![],
							// FIXME: module_id_to_codegen_ret is currently not used in
							// CssGenerator. But we need to pass it to satisfy the args.
						...
						let mut ctx = GenerateContext {
							...
							warnings:vec![],
							// FIXME: module_id_to_codegen_ret is currently not used in
							// AssetGenerator. But we need to pass it to satisfy the args.
							module_id_to_codegen_ret:vec![],
						};
						...
						ecma_chunks.and_then(|ecma_chunks| {
							css_chunks.and_then(|css_chunks| {
								asset_chunks
									.map(|asset_chunks| [ecma_chunks, css_chunks, asset_chunks])
							})
						})
					}
				}),
