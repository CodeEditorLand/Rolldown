...
use rolldown_common::{
	Asset, EmittedChunkInfo, InstantiationKind, ModuleRenderArgs, ModuleRenderOutput, Output, OutputAsset, OutputChunk,
	SharedFileEmitter, SourceMapType,
};
...
use crate::{
	...
	utils::{
		augment_chunk_hash::augment_chunk_hash, chunk::finalize_chunks::finalize_assets, render_chunks::render_chunks,
		uuid::uuid_v4_string_from_u128,
	},
};
...
impl GenerateStage<'_> {
	...
	pub async fn render_chunk_to_assets(&mut self, chunk_graph: &mut ChunkGraph) -> BuildResult<BundleOutput> {
			...
			match rendered_chunk {
				InstantiationKind::Ecma(ecma_meta) => {
					...
					if let Some(map) = map.as_mut() {
						self.process_code_and_sourcemap(
							&mut code,
							map,
							&mut output_assets,
							&file_dir,
					...
					let sourcemap_filename = if matches!(self.options.sourcemap, Some(SourceMapType::Inline) | None) {
						None
					} else {
	...
	async fn instantiate_chunks(
		...
		index_chunk_to_assets.iter_mut().for_each(|assets| {
			assets.sort_by_cached_key(|asset_idx| index_preliminary_assets[*asset_idx].preliminary_filename.as_str());
		});
	...
	/// ]
	fn create_chunk_to_codegen_ret_map(&self, chunk_graph: &ChunkGraph) -> Vec<Vec<Option<ModuleRenderOutput>>> {
		let chunk_to_codegen_ret = chunk_graph
			...
			.map(|item| {
				item.modules
					.par_iter()
					.map(
						|&module_idx| match self.link_output.module_table.modules[module_idx].as_normal() {
							Some(module) => {
...
pub fn set_emitted_chunk_preliminary_filenames(file_emitter: &SharedFileEmitter, chunk_graph: &ChunkGraph) {
...
fn set_emitted_chunk_filenames(file_emitter: &SharedFileEmitter, assets: &IndexAssets, chunk_graph: &ChunkGraph) {
