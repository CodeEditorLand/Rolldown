...
use crate::{
	...
	utils::{
		...
		chunk::finalize_chunks::finalize_assets,
...
impl<'a> GenerateStage<'a> {
			...
			if let InstantiationKind::Ecma(ecma_meta) = rendered_chunk {
				...
				if let Some(map) = map.as_mut() {
					...
					if let Some(sourcemap_path_transform) = &self.options.sourcemap_path_transform {
						...
						map.set_sources(
							sources.iter().map(std::convert::AsRef::as_ref).collect::<Vec<_>>(),
						);
					}
					...
					let sources = map
						.get_sources()
						.map(|x| x.to_slash_lossy().to_string())
						.collect::<Vec<_>>();
					map.set_sources(
						sources.iter().map(std::convert::AsRef::as_ref).collect::<Vec<_>>(),
					);
	...
	async fn instantiate_chunks(
		...
		try_join_all(
			chunk_graph
				...
				.map(|((chunk_idx, chunk), module_id_to_codegen_ret)| {
					async move {
						...
						let mut ctx = GenerateContext {
							...
							plugin_driver:self.plugin_driver,
							warnings:vec![],
							// FIXME: module_id_to_codegen_ret is currently not
							// used in CssGenerator. But we need to pass it to
							// satisfy the args.
						...
						ecma_chunks.and_then(|ecma_chunks| {
							css_chunks.map(|css_chunks| [ecma_chunks, css_chunks])
						})
					}
				}),
		)
	...
	/// Create a IndexVecMap from chunk index to related modules codegen return
	/// list. e.g.
		...
		let chunk_to_codegen_ret = chunk_graph
			...
			.map(|item| {
				item.modules
					...
					.map(|&module_idx| {
						if let Some(module) =
							self.link_output.module_table.modules[module_idx].as_normal()
						{
							let enable_sourcemap =
								self.options.sourcemap.is_some() && !module.is_virtual();

							// Because oxc codegen sourcemap is last of
							// sourcemap chain, If here no extra
							// sourcemap need remapping, we using it as final
							// module sourcemap. So here make sure using
							// correct `source_name` and `source_content.
