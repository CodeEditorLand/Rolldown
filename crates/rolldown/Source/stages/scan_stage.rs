...
impl ScanStage {
	...
	async fn resolve_user_defined_entries(
		...
		let resolved_ids = join_all(self.options.input.iter().map(|input_item| {
			async move {
				struct Args<'a> {
					specifier:&'a str,
				}
				...
				resolved.map(|info| {
					(args, info.map(|info| ((input_item.name.clone().map(ArcStr::from)), info)))
				})
			}
		}))
		.await;
			...
			match resolve_id {
				Ok(item) => {
					if item.1.is_external {
						errors
							.push(BuildDiagnostic::entry_cannot_be_external(item.1.id.to_string()));
						continue;
					}
					ret.push(item);
				},
