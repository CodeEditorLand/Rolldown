...
use super::LinkStage;
use crate::types::{linking_metadata::LinkingMetadataVec, symbols::Symbols};
...
/// if no export is used, and the module has no side effects, the module should
/// not be included
fn include_module(ctx:&mut Context, module:&NormalModule) {
	fn forcefully_include_all_statements(ctx:&mut Context, module:&NormalModule) {
		// Skip the first statement, which is the namespace object. It should be
		// included only if it is used no matter tree shaking is enabled or
		// not.
		module
			.stmt_infos
			.iter_enumerated()
			.skip(1)
			.for_each(|(stmt_info_id, _stmt_info)| {
	...
	if module.idx == ctx.runtime_id {
		// runtime module has no side effects and it's statements should be
		// included by other modules's references.
		return;
	}
	...
	if ctx.tree_shaking && !forced_no_treeshake {
		module.stmt_infos.iter_enumerated().for_each(|(stmt_info_id, stmt_info)| {
			// No need to handle the first statement specially, which is the
			// namespace object, because it doesn't have side effects and
			// will only be included if it is used.
			let bail_eval = module.has_eval
	...
	stmt_info.referenced_symbols.iter().for_each(|reference_ref| {
		match reference_ref {
			...
			SymbolOrMemberExprRef::MemberExpr(member_expr) => {
				if let Some(symbol) = member_expr
					.resolved_symbol_ref(&ctx.metas[module.idx].resolved_member_expr_refs)
				{
					include_symbol(ctx, symbol);
				}
			},
	...
	pub fn include_statements(&mut self) {
		...
		self.module_table
			...
			.for_each(|module| {
				...
				is_included_vec[module.idx].iter_enumerated().for_each(
	...
	fn determine_side_effects(&mut self) {
		...
		fn determine_side_effects_for_module(
			...
			let ret = match *module.side_effects() {
				// should keep as is if the side effects is derived from
				// package.json, it is already true or `no-treeshake`
				...
				// this branch means the side effects of the module is analyzed
				// `false`
				DeterminedSideEffects::Analyzed(_) => {
					match module {
						Module::Normal(module) => {
							DeterminedSideEffects::Analyzed(module.import_records.iter().any(
								|import_record| {
