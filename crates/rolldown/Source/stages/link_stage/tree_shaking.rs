...
impl LinkStage<'_> {
	...
	pub fn include_statements(&mut self) {
		...
		self.module_table.modules.par_iter_mut().filter_map(Module::as_ecma_mut).for_each(
			|module| {
		...
		fn determine_side_effects_for_module(
			...
			let ret = match *module.side_effects() {
				...
				DeterminedSideEffects::Analyzed(_) => match module {
					Module::Ecma(module) => DeterminedSideEffects::Analyzed(
						module.import_records.iter().any(|import_record| {
