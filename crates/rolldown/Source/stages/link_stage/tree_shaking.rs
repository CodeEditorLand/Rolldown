...
fn include_module(ctx:&mut Context, module:&NormalModule) {
	fn forcefully_include_all_statements(ctx:&mut Context, module:&NormalModule) {
		...
		// included only if it is used no matter tree shaking is enabled or not.
		module
			.stmt_infos
			.iter_enumerated()
			.skip(1)
	...
	if ctx.tree_shaking && !forced_no_treeshake {
		module.stmt_infos.iter_enumerated().for_each(|(stmt_info_id, stmt_info)| {
			// No need to handle the first statement specially, which is the namespace
			// object, because it doesn't have side effects and will only be included if it
			// is used.
