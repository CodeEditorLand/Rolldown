...
use rolldown_common::{
	EcmaViewMeta, IndexModules, Module, ModuleIdx, ModuleType, NormalModule, NormalizedBundlerOptions, StmtInfoIdx,
	...
	pub fn include_statements(&mut self) {
		...
		if self.options.is_hmr_enabled() {
			...
			if let Some(runtime_module) = self.module_table.modules[self.runtime.id()].as_normal() {
				runtime_module
					.stmt_infos
					.iter_enumerated()
					.for_each(|(stmt_info_id, stmt_info)| {
		...
		self.module_table
			...
			.for_each(|module| {
				...
				is_included_vec[module.idx]
					.iter_enumerated()
					.for_each(|(stmt_info_id, is_included)| {
...
fn include_module(ctx: &mut Context, module: &NormalModule) {
	...
	if ctx.tree_shaking && !forced_no_treeshake {
		module.stmt_infos.iter_enumerated().for_each(|(stmt_info_id, stmt_info)| {
			...
			let bail_eval =
				module.meta.has_eval() && !stmt_info.declared_symbols.is_empty() && stmt_info_id.index() != 0;
	...
	module_meta
		...
		.iter()
		.copied()
		.for_each(|dependency_idx| match &ctx.modules[dependency_idx] {
	...
	tracing::trace!(
		...
		module_meta
			.dependencies
			.iter()
			.map(|idx| { ctx.modules[*idx].id().to_string() })
			.collect_vec()
	);
...
fn include_statement(ctx: &mut Context, module: &NormalModule, stmt_info_id: StmtInfoIdx) {
	...
	*is_included = true;
	...
	stmt_info
		.referenced_symbols
		.iter()
		.for_each(|reference_ref| match reference_ref {
			SymbolOrMemberExprRef::Symbol(symbol_ref) => {
				include_symbol(ctx, *symbol_ref);
			},
			SymbolOrMemberExprRef::MemberExpr(member_expr) => {
				if let Some(symbol) = member_expr.resolved_symbol_ref(&ctx.metas[module.idx].resolved_member_expr_refs)
				{
					include_symbol(ctx, symbol);
				}
			},
