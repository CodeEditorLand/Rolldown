...
use rolldown_common::{
	ESTarget, EcmaAstIdx, EcmaModuleAstUsage, ExportsKind, LocalExport, Module, ModuleIdx, ModuleType, NormalModule,
	StmtInfo, StmtInfoIdx, SymbolOrMemberExprRef, SymbolRef, SymbolRefDbForModule, WrapKind,
};
...
fn update_module_default_export_info(module: &mut NormalModule, default_symbol_ref: SymbolRef, idx: StmtInfoIdx) {
	module
		.named_exports
...
fn json_object_expr_to_esm(link_staged: &mut LinkStage, module_idx: ModuleIdx, ast_idx: EcmaAstIdx) -> bool {
		...
		for property in &mut obj_expr.properties {
			match property {
				ast::ObjectPropertyKind::ObjectProperty(property) => {
					...
					declaration_binding_names.push((legitimized_ident.clone(), key.to_rstr(), is_legal_ident));
	...
	let mut all_declared_symbols = stmt_info.flat_map(|info| info.referenced_symbols).collect::<Vec<_>>();
	for (i, (local, exported, _)) in declaration_binding_names.iter().enumerate() {
		let symbol_id = symbol_ref_db
			.scoping()
			.get_root_binding(local.as_str())
