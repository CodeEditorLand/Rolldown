	...
	pub fn generate_lazy_export(&mut self) {
		...
		self.module_table.modules.par_iter_mut().for_each(|module| {
			...
			if module.exports_kind == ExportsKind::CommonJs {
				// since the wrap arguments are generate on demand, we need to insert the module
				// ref usage here.
		...
		for (ast_idx, exports_kind, is_json_module) in module_idx_to_exports_kind {
			...
			if matches!(exports_kind, ExportsKind::CommonJs) {
				ecma_ast.program.with_mut(|fields| {
					...
					let Some(stmt) = fields.program.body.first_mut() else {
						unreachable!()
					};
					let expr = match stmt {
						ast::Statement::ExpressionStatement(stmt) => {
							stmt.expression.take_in(snippet.alloc())
						},
						_ => {
							unreachable!()
						},
			...
			if is_json_module {
				...
				// if json is not a ObjectExpression, we will fallback to normal esm lazy export
				// transform
			...
			// shadowing the previous mutable ref, to avoid reference mutable ref twice at
			// the same time.
			let Some((ecma_ast, _)) = self.ast_table.get_mut(ast_idx) else {
				unreachable!()
			};
			ecma_ast.program.with_mut(|fields| {
				let snippet = AstSnippet::new(fields.allocator);
...
fn update_module_default_export_info(
	...
	// needs to support `preferConst`, so default statement may not be the second
	// stmt info
...
fn json_object_expr_to_esm(
	...
	let transformed = ecma_ast.program.with_mut(|fields| {
		...
		let Some(stmts) = program.body.first_mut() else {
			unreachable!()
		};
			...
			match property {
				ast::ObjectPropertyKind::ObjectProperty(ref mut property) => {
					...
					let value = std::mem::replace(
						...
						snippet
							.builder
							.expression_identifier_reference(SPAN, legitimized_ident.as_str()),
					);
	...
	// clear stmt info, since we need to split `ObjectExpression` into multiple
	// decl, the original stmt info is invalid. preserve the first one, which is
	// `NamespaceRef`
	...
	for (i, (local, exported, _)) in declaration_binding_names.iter().enumerate() {
		...
		let stmt_info =
			StmtInfo::default().with_stmt_idx(i).with_declared_symbols(vec![symbol_ref]);
