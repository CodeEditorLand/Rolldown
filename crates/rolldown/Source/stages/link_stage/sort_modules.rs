...
impl LinkStage<'_> {
	...
	/// - Async entries is sorted by `Module#debug_id` of entry module to ensure
	///   deterministic output.
	/// - `require(...)` is treated as implicit static `import`, which required
	///   modules are executed before the module that requires them.
	/// - Since import statements are hoisted, `require(...)` is always placed
	///   after static `import` statements.
	/// - Order of `require(...)` is determined by who shows up first while
	///   scanning ast. For such code
	///
	/// ```js
	/// () => require('b')
	/// require('c')
	...
	/// - We only ensure execution order is relative correct, which means
	///   imported/required modules are executed before the module that
	///   imports/require them.
	...
	pub fn sort_modules(&mut self) {
			...
			match status {
				Status::ToBeExecuted(id) => {
					if executed_ids.contains(&id) {
						if self.options.checks.circular_dependency.unwrap_or(false) {
							...
							if let Some(index) = stack_indexes_of_executing_id.get(&id).copied() {
								...
								let cycles = execution_stack[index..]
									...
									.filter_map(|action| {
										match action {
											// Only modules with `Status::WaitForExit` are on the
											// execution chain
						...
						// It's already executed in other import chain, no need
						// to execute again
					} else {
		...
		if self.options.checks.circular_dependency.unwrap_or(true)
			&& !circular_dependencies.is_empty()
		{
			...
			for cycle in cycles {
				...
				self.warnings
					.push(BuildDiagnostic::circular_dependency(paths).with_severity_warning());
			}
		}
