...
// TODO: The current implementation for matching imports is enough so far but incomplete. It
// needs to be refactored if we want more enhancements related to exports.
use rolldown_common::{
	ExportsKind,
	IndexModules,
	Module,
	...
	pub fn bind_imports_and_exports(&mut self) {
		...
		for (module_idx, map) in &binding_ctx.external_import_binding_merger {
			for (key, value) in map {
				let target_symbol =
					self.symbols.create_facade_root_symbol_ref(*module_idx, key.clone());
	...
	fn add_exports_for_export_star(
		...
		for dep_id in module.star_export_module_ids() {
			...
			for (exported_name, named_export) in &dep_module.named_exports {
				...
				// This export star is shadowed if any file in the stack has a matching real
				// named export
				if module_stack
					.iter()
				...
				// We have filled `resolve_exports` with `named_exports`. If the export is
				// already exists, it means that the importer has a named export with the
				// same name. So the export from dep module is shadowed.
						...
						module.stmt_infos.iter().for_each(|stmt_info| {
							stmt_info.referenced_symbols.iter().for_each(|symbol_ref| {
								if let SymbolOrMemberExprRef::MemberExpr(member_expr_ref) =
									symbol_ref
								{
									// First get the canonical ref of `foo_ns`, then we get the
									// `NormalModule#namespace_object_ref` of `foo.js`.
									let mut canonical_ref =
										self.symbols.canonical_ref_for(member_expr_ref.object_ref);
									...
									while cursor < member_expr_ref.props.len() && is_namespace_ref {
										...
										let Some(export_symbol) = export_symbol else {
											// when we try to resolve `a.b.c`, and found that `b` is
											// not exported by module that `a` pointed
											// to, convert the `a.b.c` into `void 0` if module `a`
											// do not have any dynamic exports.
										...
										// TODO(hyf0): suspicious cjs might just fallback to dynamic
										// lookup?
										...
										canonical_ref = self
											.symbols
											.canonical_ref_for(export_symbol.symbol_ref);
...
impl BindImportsAndExportsContext<'_> {
	...
	fn match_imports_with_exports(&mut self, module_id:ModuleIdx) {
		...
		for (imported_as_ref, named_import) in &module.named_imports {
			...
			let is_external =
				matches!(self.index_modules[rec.resolved_module], Module::External(_));
			if is_esm && is_external {
				if let Specifier::Literal(ref name) = named_import.imported {
					if name.as_str() != "default" && is_validate_identifier_name(name) {
						self.external_import_binding_merger
			...
			match ret {
				...
				MatchImportKind::Ambiguous { symbol_ref, potentially_ambiguous_symbol_refs } => {
					...
					exporter.extend(potentially_ambiguous_symbol_refs.iter().filter_map(
						|&symbol_ref| {
