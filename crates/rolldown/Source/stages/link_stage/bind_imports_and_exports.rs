use arcstr::ArcStr;
// TODO: The current implementation for matching imports is enough so far
// but incomplete. It needs to be refactored if we want more enhancements
// related to exports.
use rolldown_common::{
	ExportsKind,
	IndexModules,
...
pub enum ImportStatus {
	...
	/// The import was treated as a CommonJS import but the file is known to
	/// have no exports
...
impl<'link> LinkStage<'link> {
	...
	/// ```
	///
	/// Unlike import from normal modules, the imported variable deosn't have a
	/// place that declared the variable. So we consider `import { a } from
	/// 'external'` in `foo.js` as the declaration statement of `a`.
	pub fn bind_imports_and_exports(&mut self) {
		// Initialize `resolved_exports` to prepare for matching imports with
		// exports
	...
	fn add_exports_for_export_star(
		...
		for dep_id in module.star_export_module_ids() {
			...
			for (exported_name, named_export) in &dep_module.named_exports {
				...
				// This export star is shadowed if any file in the stack has a
				// matching real named export
				if module_stack
					.iter()
				...
				// We have filled `resolve_exports` with `named_exports`. If the
				// export is already exists, it means that the importer
				// has a named export with the same name. So the export from dep
				// module is shadowed.
	...
	/// export const c = 1;
	/// ```
	/// The final pointed `SymbolRef` of `foo_ns.bar_ns.c` is the `c` in
	/// `bar.js`.
					...
					Module::Normal(module) => {
						...
						module.stmt_infos.iter().for_each(|stmt_info| {
							stmt_info.referenced_symbols.iter().for_each(|symbol_ref| {
								if let SymbolOrMemberExprRef::MemberExpr(member_expr_ref) =
									symbol_ref
								{
									// First get the canonical ref of
									// `foo_ns`, then we get the
									// `NormalModule#namespace_object_ref`
									// of `foo.js`.
									let mut canonical_ref = self
										.symbols
										.par_canonical_ref_for(member_expr_ref.object_ref);
									let mut canonical_ref_owner = self.module_table.modules
										[canonical_ref.owner]
										.as_normal()
									...
									while cursor < member_expr_ref.props.len() && is_namespace_ref {
										...
										let export_symbol =
											meta.resolved_exports.get(&name.to_rstr());
										let Some(export_symbol) = export_symbol else {
											// when we try to resolve
											// `a.b.c`, and found that `b`
											// is not exported by module
											// that `a` pointed to, convert
											// the `a.b.c` into `void 0` if
											// module `a` do not
											// have any dynamic exports.
											if !self.metas[canonical_ref_owner.idx]
												.has_dynamic_exports
											{
										...
										if !meta
											.sorted_and_non_ambiguous_resolved_exports
											.contains(&name.to_rstr())
										{
											resolved.insert(member_expr_ref.span, None);
											return;
										};

										// TODO(hyf0): suspicious cjs might
										// just fallback to dynamic lookup?
										if !self.module_table.modules
											[export_symbol.symbol_ref.owner]
											.as_normal()
											.unwrap()
											.exports_kind
											.is_esm()
										...
										canonical_ref = self
											.symbols
											.par_canonical_ref_for(export_symbol.symbol_ref);
										canonical_ref_owner = self.module_table.modules
											[canonical_ref.owner]
											.as_normal()
											.unwrap();
										cursor += 1;
										is_namespace_ref = canonical_ref_owner.namespace_object_ref
											== canonical_ref;
									}
									if cursor > 0 {
										resolved.insert(
											...
											Some((
												canonical_ref,
												member_expr_ref.props[cursor..].to_vec(),
											)),
										);
									}
								}
...
impl<'a> BindImportsAndExportsContext<'a> {
		...
		for (imported_as_ref, named_import) in &module.named_imports {
			...
			match ret {
				...
				MatchImportKind::Ambiguous { symbol_ref, potentially_ambiguous_symbol_refs } => {
					...
					let mut exporter =
						Vec::with_capacity(potentially_ambiguous_symbol_refs.len() + 1);
					...
					exporter.extend(
						potentially_ambiguous_symbol_refs
							...
							.filter_map(|&symbol_ref| {
								if let Some(owner) =
									self.normal_modules[symbol_ref.owner].as_normal()
								{
									if let Specifier::Literal(name) = &named_import.imported {
										let named_export = &owner.named_exports[name];
	...
	fn advance_import_tracker(&self, ctx:&mut MatchingContext) -> ImportStatus {
		...
		let importee = &self.normal_modules[importee_id]
			.as_normal()
			.expect("external module is bailout above");
	...
	fn match_import_with_export(
		...
		let ret = loop {
			...
			let importer_record =
				&importer.as_normal().unwrap().import_records[named_import.record_id];
			...
			let kind = match import_status {
				...
				ImportStatus::Found { symbol, potentially_ambiguous_export_star_refs, .. } => {
					for ambiguous_ref in &potentially_ambiguous_export_star_refs {
						...
						if let Some(another_named_import) = ambiguous_ref_owner
							.as_normal()
							.unwrap()
							.named_imports
							.get(ambiguous_ref)
						{
							...
							ambiguous_results
								.push(MatchImportKind::Normal { symbol:*ambiguous_ref });
						}
					}

					// If this is a re-export of another import, continue for
					// another iteration of the loop to resolve that import
					// as well
					let owner = &normal_modules[symbol.owner];
					if let Some(another_named_import) =
						owner.as_normal().unwrap().named_imports.get(&symbol)
					{
						let rec = &owner.as_normal().unwrap().import_records
							[another_named_import.record_id];
		...
		if let Module::Normal(importee) = &self.normal_modules[tracker.importee] {
				...
				&& matches!(ret, MatchImportKind::NoMatch)
			{
				match &tracker.imported {
					Specifier::Star => {
						unreachable!("star should always exist, no need to shim")
					},
