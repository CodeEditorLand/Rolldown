...
fn init_entry_point_stmt_info(
	...
	referenced_symbols.extend(
		meta.referenced_canonical_exports_symbols(entry.id, entry.kind, dynamic_import_exports_usage_map)
			...
			.for_each(|ecma_module| {
				...
				if matches!(ecma_module.exports_kind, ExportsKind::Esm) {
					...
					if !meta.is_canonical_exports_empty() {
						...
						referenced_symbols.extend(meta.canonical_exports().map(|(_, export)| export.symbol_ref.into()));
					}
