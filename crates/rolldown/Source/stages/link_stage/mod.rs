...
impl<'a> LinkStage<'a> {
				...
				importer.import_records.iter().for_each(|rec| {
					...
					match rec.kind {
						ImportKind::Import => {
							...
							{
								// `import` a module that has `ExportsKind::None`, which will be
								// turned into `ExportsKind::Esm` SAFETY: If `importee` and
								// `importer` are different, so this is safe. If they are the same,
								// then behaviors are still expected.
								unsafe {
						...
						ImportKind::Require => {
							match importee.exports_kind {
								...
								ExportsKind::None => {
									self.metas[importee.idx].wrap_kind = WrapKind::Cjs;
									// SAFETY: If `importee` and `importer` are different, so this
									// is safe. If they are the same, then behaviors are still
									// expected. A module with `ExportsKind::None` that
									// `require` self should be turned into `ExportsKind::CommonJs`.
									unsafe {
										let importee_mut = addr_of!(*importee).cast_mut();
							...
							if self.options.inline_dynamic_imports {
								...
								match importee.exports_kind {
									...
									ExportsKind::None => {
										...
										// still expected. A module with
										// `ExportsKind::None` that `require` self should be turned
			...
			.for_each(|importer| {
				...
				stmt_infos.infos.iter_mut_enumerated().for_each(|(stmt_idx, stmt_info)| {
							...
							|| is_external_dynamic_import(&self.module_table, rec, importer_idx)
						{
								...
								|| !self.options.format.keep_esm_import_export_syntax()
							{
										...
										.polyfill_require_for_esm_format_with_node_platform()
								{
									...
									record_meta_pairs
										.push((*rec_id, ImportRecordMeta::CALL_RUNTIME_REQUIRE));
								}
							}
						}
								...
								match rec.kind {
											...
											WrapKind::None => {
												...
												if is_reexport_all {
													...
													if meta.has_dynamic_exports {
														...
														stmt_info.referenced_symbols.push(
															importee.namespace_object_ref.into(),
														);
													}
												}
											},
									...
									ImportKind::Require => {
										match importee_linking_info.wrap_kind {
											...
											WrapKind::Esm => {
												// convert require record into `(init_foo(),
												// __toCommonJS(foo_exports))` if
												// `require('xxx)` is used, else convert it to
												// `init_foo()`
												stmt_info.referenced_symbols.push(
													importee_linking_info
														.wrapper_ref
												...
												if !rec
													.meta
													.contains(ImportRecordMeta::IS_REQUIRE_UNUSED)
												{
													stmt_info.referenced_symbols.push(
														self.runtime
									...
									ImportKind::AtImport => {
										unreachable!(
											"A Js module would never import a CSS module via \
											 `@import`"
										);
									},
					...
					if keep_names && stmt_info.meta.intersects(StmtInfoMeta::KeepNamesType) {
						stmt_info
							.referenced_symbols
							.push(self.runtime.resolve_symbol("__name").into());
					}
				});
	...
	fn create_exports_for_ecma_modules(&mut self) {
		self.module_table.modules.iter_mut().filter_map(|m| m.as_normal_mut()).for_each(
			|ecma_module| {
				...
				// so they can participate in the symbol de-conflict and tree-shaking
				// process.
				...
				// Though Module Namespace Object is created in runtime, as a bundler, we have
				// stimulus the behavior in compile-time and generate a real statement to
				// construct the Module Namespace Object and assign it to a variable.
				...
				if matches!(ecma_module.exports_kind, ExportsKind::Esm) {
					...
					// Create a StmtInfo to represent the statement that declares and constructs the
					// Module Namespace Object. Corresponding AST for this statement will be
	...
	fn patch_module_dependencies(&mut self) {
		self.metas.par_iter_mut_enumerated().for_each(|(module_idx, meta)| {
			...
			module.stmt_infos.iter().filter(|stmt_info| stmt_info.is_included).for_each(
				|stmt_info| {
					// We need this step to include the runtime module, if there are symbols of it.
					// TODO: Maybe we should push runtime module to `LinkingMetadata::dependencies`
					// while pushing the runtime symbols.
						...
						match reference_ref {
							...
							rolldown_common::SymbolOrMemberExprRef::MemberExpr(member_expr) => {
									...
									member_expr.resolved_symbol_ref(&meta.resolved_member_expr_refs)
								{
									...
									// `None` means the member expression
									// resolve to a ambiguous export, which
									// means it actually resolve to nothing.
									// It would be rewrite to `undefined` in the
									// final code, so we don't need to include
									// anything to make `undefined` work.
								}
							},
						};
					});
...
pub fn init_entry_point_stmt_info(
	...
	if !matches!(meta.wrap_kind, WrapKind::None) {
		// If a commonjs module becomes an entry point while targeting esm, we need to
		// at least add a `export default require_foo();` statement as some kind of
		// syntax sugar. So users won't need to manually create a proxy file with
		// `export default require('./foo.cjs')` in it.
		referenced_symbols.push(meta.wrapper_ref.unwrap());
	}

	referenced_symbols.extend(
		meta.referenced_canonical_exports_symbols(
			entry.id,
			entry.kind,
	...
	// Entry chunk need to generate exports, so we need reference to all exports to
	// make sure they are included in tree-shaking.
