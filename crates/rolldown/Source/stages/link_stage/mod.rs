...
impl<'a> LinkStage<'a> {
							...
							match rec.kind {
										...
										WrapKind::Cjs => {
											if is_reexport_all {
												...
												symbols
													...
													.name = format!("import_{}", &importee.repr_name).into();
											}
										}
										WrapKind::Esm => {
											stmt_info.side_effect = true;
											...
											stmt_info.referenced_symbols.push(
												importee_linking_info.wrapper_ref.unwrap().into(),
											);
											if is_reexport_all
								...
								ImportKind::Require => match importee_linking_info.wrap_kind {
									...
									WrapKind::Esm => {
										...
										stmt_info.referenced_symbols.push(
											self.runtime.resolve_symbol("__toCommonJS").into(),
										);
										stmt_info
											.referenced_symbols
											.push(importee.namespace_object_ref.into());
	...
	fn create_exports_for_ecma_modules(&mut self) {
		self.module_table.modules.iter_mut().filter_map(|m| m.as_ecma_mut()).for_each(
			|ecma_module| {
