...
use rolldown_common::{
	EntryPoint, EntryPointKind, ImportKind, ImportRecordMeta, ModuleIdx, ModuleTable, RuntimeModuleBrief, SymbolRef,
				...
				.map(|module| LinkingMetadata {
					dependencies: module
						...
						.filter_map(|rec| match rec.kind {
							ImportKind::DynamicImport => options.inline_dynamic_imports.then_some(rec.resolved_module),
			...
			.filter_map(|item| match item.kind {
				...
				EntryPointKind::DynamicImport => {
					...
					let lived = item.related_stmt_infos.iter().any(|(module_idx, stmt_idx)| {
						...
						let all_dead_pure_dynamic_import = stmt_info.import_records.iter().all(|import_record_idx| {
							...
							let importee_side_effects = self.module_table.modules[import_record.resolved_module]
								.side_effects()
								.has_side_effects();
							let ret = import_record.meta.contains(ImportRecordMeta::TOP_LEVEL_PURE_DYNAMIC_IMPORT)
