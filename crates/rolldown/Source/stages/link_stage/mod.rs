...
impl<'a> LinkStage<'a> {
				...
				.map(|module| {
					LinkingMetadata {
						dependencies:module
							...
							.collect(),
						star_exports_from_external_modules:module.as_normal().map_or(
							vec![],
							|inner| {
								inner
									.star_exports_from_external_modules(
										&scan_stage_output.module_table.modules,
									)
									.collect()
							},
						),
					...
					match rec.kind {
							...
							if matches!(importee.exports_kind, ExportsKind::None) {
								if compat_mode {
									...
									if rec.meta.intersects(
										...
										// SAFETY: If `importee` and `importer` are different, so
										// this is safe. If they are the same, then behaviors are
										// still expected.
										unsafe {
						...
						ImportKind::Require => {
							match importee.exports_kind {
								...
								ExportsKind::None => {
									if compat_mode {
										self.metas[importee.idx].wrap_kind = WrapKind::Cjs;
										// SAFETY: If `importee` and `importer`
										// are different, so this is safe. If
										// they are the same, then behaviors are
										// still expected.
										// A module with `ExportsKind::None`
										// that `require` self should be turned
										// into `ExportsKind::CommonJs`.
										unsafe {
											let importee_mut = addr_of!(*importee).cast_mut();
											(*importee_mut).exports_kind = ExportsKind::CommonJs;
										}
						...
						ImportKind::DynamicImport => {
							if self.options.inline_dynamic_imports {
								// For iife, then import() is just a require()
								// that returns a promise, so the
								// imported file must also be wrapped
								match importee.exports_kind {
									ExportsKind::Esm => {
										self.metas[importee.idx].wrap_kind = WrapKind::Esm;
									},
									...
									ExportsKind::None => {
										if compat_mode {
											self.metas[importee.idx].wrap_kind = WrapKind::Cjs;
											// SAFETY: If `importee` and
											// `importer` are different, so this
											// is safe. If they are the same,
											// then behaviors are still
											// expected.
											// A module with `ExportsKind::None`
											// that `require` self should be
											// turned into
											// `ExportsKind::CommonJs`.
											unsafe {
												let importee_mut = addr_of!(*importee).cast_mut();
												(*importee_mut).exports_kind =
													ExportsKind::CommonJs;
											}
										} else {
											self.metas[importee.idx].wrap_kind = WrapKind::Esm;
						...
						match &self.module_table.modules[rec.resolved_module] {
							...
							Module::External(ext) => {
								self.metas[importer.idx]
									...
									.or_insert_with(|| {
										// Created `SymbolRef` is only join the
										// de-conflict process to avoid conflict
										// with other symbols.
										self.symbols.create_symbol(
											importer.idx,
											legitimize_identifier_name(&ext.name)
												.into_owned()
												.into(),
										)
									});
	...
	fn reference_needed_symbols(&mut self) {
		...
		self.module_table
			...
			.for_each(|importer| {
				...
				// - Mutating on `stmt_infos` doesn't rely on other mutating operations of other
				//   modules
					...
					stmt_info.import_records.iter().for_each(|rec_id| {
						...
						match &self.module_table.modules[rec.resolved_module] {
							Module::External(importee) => {
								// Make sure symbols from external modules are included and
								// de_conflicted
								match rec.kind {
									ImportKind::Import => {
										let is_reexport_all =
											importer.star_exports.contains(rec_id);
										if is_reexport_all {
											// export * from 'external' would be just removed. So it
											// references nothing.
											symbols
												.lock()
												.unwrap()
												.get_mut(rec.namespace_ref)
												.name = format!(
												"import_{}",
												legitimize_identifier_name(&importee.name)
											)
											.into();
										} else {
											// import ... from 'external' or export ... from
											// 'external'
											let cjs_format =
											...
											if cjs_format
												&& !rec
													.meta
													.contains(ImportRecordMeta::IS_PLAIN_IMPORT)
											{
												stmt_info.referenced_symbols.push(
													self.runtime.resolve_symbol("__toESM").into(),
												);
											}
										}
									},
									...
									ImportKind::Import => {
										...
										match importee_linking_info.wrap_kind {
											WrapKind::None => {
												...
												if is_reexport_all {
													...
													if meta.has_dynamic_exports {
														...
														stmt_info.referenced_symbols.push(
															self.runtime
																.resolve_symbol("__reExport")
																.into(),
														);
														stmt_info.referenced_symbols.push(
															importer.namespace_object_ref.into(),
														);
													}
												}
											},
											WrapKind::Cjs => {
												if is_reexport_all {
													stmt_info.side_effect = true;
													// Turn `export * from 'bar_cjs'` into
													// `__reExport(foo_exports,
													// __toESM(require_bar_cjs()))`
													...
													stmt_info
														.referenced_symbols
														.push(importer.namespace_object_ref.into());
												} else {
													stmt_info.side_effect =
														importee.side_effects.has_side_effects();
													// Turn `import * as bar from 'bar_cjs'` into
													// `var import_bar_cjs =
													// __toESM(require_bar_cjs())` Turn
													// `import { prop } from 'bar_cjs'; prop;` into
													// `var import_bar_cjs =
													// __toESM(require_bar_cjs());
													// import_bar_cjs.prop;`
													// Reference to `require_bar_cjs`
													...
													declared_symbol_for_stmt_pairs
														.push((stmt_idx, rec.namespace_ref));
													symbols
														.lock()
														.unwrap()
														.get_mut(rec.namespace_ref)
														.name = format!("import_{}", &importee.repr_name)
														.into();
												}
											},
											WrapKind::Esm => {
												stmt_info.side_effect = true;
												// Turn `import ... from 'bar_esm'` into
												// `init_bar_esm()` Reference to
												// `init_foo`
												stmt_info.referenced_symbols.push(
													importee_linking_info
														.wrapper_ref
												...
												{
													// Turn `export * from 'bar_esm'` into
													// `init_bar_esm();__reExport(foo_exports,
													// bar_esm_exports);`
													// something like `__reExport(foo_exports,
													// other_exports)`
													stmt_info.referenced_symbols.push(
														self.runtime
															.resolve_symbol("__reExport")
													...
													stmt_info
														.referenced_symbols
														.push(importee.namespace_object_ref.into());
												}
											},
										}
									},
	...
	fn create_exports_for_ecma_modules(&mut self) {
		self.module_table.modules.iter_mut().filter_map(|m| m.as_normal_mut()).for_each(
			|ecma_module| {
				...
				// Create facade StmtInfo that declares variables based on the
				// missing exports, so they can participate in the symbol
				// de-conflict and tree-shaking process.
				...
				// Generate export of Module Namespace Object for Namespace
				// Import
				...
				// Though Module Namespace Object is created in runtime, as a
				// bundler, we have stimulus the behavior in compile-time and
				// generate a real statement to construct the Module
				// Namespace Object and assign it to a variable.
				// This is only a concept of esm, so no need to care about this
				// in commonjs.
				if matches!(ecma_module.exports_kind, ExportsKind::Esm) {
					...
					if !meta.is_canonical_exports_empty() {
						...
						referenced_symbols.extend(
							meta.canonical_exports().map(|(_, export)| export.symbol_ref.into()),
						);
					}
					if !meta.star_exports_from_external_modules.is_empty() {
						...
						match self.options.format {
							OutputFormat::Esm => {
								meta.star_exports_from_external_modules.iter().copied().for_each(
									|rec_idx| {
										referenced_symbols.push(
											ecma_module.import_records[rec_idx]
												.namespace_ref
												.into(),
										);
										declared_symbols.push(
					...
					// Create a StmtInfo to represent the statement that
					// declares and constructs the Module Namespace Object.
					// Corresponding AST for this statement will be created by
					// the finalizer.
	...
	fn patch_module_dependencies(&mut self) {
		self.metas.iter_mut_enumerated().for_each(|(module_idx, meta)| {
			// Symbols from runtime are referenced by bundler not import
			// statements.
			...
			module.stmt_infos.iter().for_each(|stmt_info| {
				...
				// We need this step to include the runtime module, if there are
				// symbols of it. TODO: Maybe we should push runtime module
				// to `LinkingMetadata::dependencies` while pushing the runtime
				// symbols.
					...
					match reference_ref {
						...
						rolldown_common::SymbolOrMemberExprRef::MemberExpr(member_expr) => {
								...
								member_expr.resolved_symbol_ref(&meta.resolved_member_expr_refs)
							{
								...
								// `None` means the member expression resolve to
								// a ambiguous export, which means it actually
								// resolve to nothing. It would be
								// rewrite to `undefined` in the final code, so
								// we don't need to include anything to make
								// `undefined` work.
							}
						},
					};
...
pub fn init_entry_point_stmt_info(meta:&mut LinkingMetadata) {
	...
	if !matches!(meta.wrap_kind, WrapKind::None) {
		// If a commonjs module becomes an entry point while targeting esm, we
		// need to at least add a `export default require_foo();` statement as
		// some kind of syntax sugar. So users won't need to manually create a
		// proxy file with `export default require('./foo.cjs')` in it.
		referenced_symbols.push(meta.wrapper_ref.unwrap());
	}

	// Entry chunk need to generate exports, so we need reference to all exports
	// to make sure they are included in tree-shaking.
