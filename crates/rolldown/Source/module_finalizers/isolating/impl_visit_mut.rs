use oxc::{
	ast::{
		VisitMut,
		ast::{self, ExportDefaultDeclarationKind, Expression, Statement},
		visit::walk_mut,
	},
	...
	fn visit_expression(&mut self, expr:&mut Expression<'ast>) {
		if let Expression::Identifier(ident) = expr {
				...
				.and_then(|symbol_ref| self.ctx.module.named_imports.get(&symbol_ref))
			{
				...
				let namespace_object_ref = self
					.create_namespace_object_ref_for_module(&self.ctx.modules[rec.resolved_module]);
...
impl<'ast> IsolatingModuleFinalizer<'_, 'ast> {
	pub fn transform_import_declaration(&mut self, import_decl:&ast::ImportDeclaration<'ast>) {
		// The specifiers rewrite with reference the namespace object, see
		// `IsolatingModuleFinalizer#visit_expression`
	...
	pub fn transform_export_default_declaration(
		...
		match &mut export_default_decl.declaration {
			decl @ ast::match_expression!(ExportDefaultDeclarationKind) => {
				...
				self.snippet
					.var_decl_stmt(default_export_ref, decl.to_expression_mut().take_in(self.alloc))
			},
			ast::ExportDefaultDeclarationKind::FunctionDeclaration(func) => {
				...
				self.snippet.builder.statement_expression(
					SPAN,
					Expression::FunctionExpression(func.take_in(self.alloc)),
				)
			},
			ast::ExportDefaultDeclarationKind::ClassDeclaration(class) => {
				let from =
					class.id.as_ref().map_or(default_export_ref, |ident| ident.name.as_str());
				...
				self.snippet.builder.statement_expression(
					SPAN,
					Expression::ClassExpression(class.take_in(self.alloc)),
				)
			},
	...
	pub fn transform_named_declaration(
		...
		match &export_named_decl.source {
			Some(_) => {
				...
				self.generated_exports.extend(export_named_decl.specifiers.iter().map(
					|specifier| {
						self.snippet.object_property_kind_object_property(
							...
							match &specifier.local {
								ast::ModuleExportName::IdentifierName(ident) => {
									Expression::StaticMemberExpression(
										self.snippet.builder.alloc_static_member_expression(
											...
											self.snippet
												.builder
												.identifier_name(SPAN, ident.name.as_str()),
											false,
										),
									)
								},
								...
								ast::ModuleExportName::IdentifierReference(_) => {
									unreachable!(
										"ModuleExportName IdentifierReference is invalid in \
				...
				if let Some(decl) = &mut export_named_decl.declaration {
					match decl {
						ast::Declaration::VariableDeclaration(var_decl) => {
							self.generated_exports.extend(var_decl.declarations.iter().filter_map(
								|decl| {
						...
						ast::Declaration::FunctionDeclaration(func_decl) => {
							let from = func_decl
								.id
								.as_ref()
								.expect("FunctionDeclaration should have ident")
								.name
								.as_str();
						...
						ast::Declaration::ClassDeclaration(class_decl) => {
							let from = class_decl
								.id
								.as_ref()
								.expect("ClassDeclaration should have ident")
								.name
								.as_str();
							self.generated_exports.push(
								self.snippet.object_property_kind_object_property(
						...
						self.snippet.object_property_kind_object_property(
							...
							match &specifier.local {
								...
								ast::ModuleExportName::StringLiteral(_) => {
									unreachable!(
										"ModuleExportName StringLiteral is invalid in \
										 ExportNamedDeclaration without source"
									)
								},
