...
impl<'ast> IsolatingModuleFinalizer<'_, 'ast> {
		...
		match &export_named_decl.source {
			Some(_) => {
				...
				self.generated_exports
					.extend(export_named_decl.specifiers.iter().map(|specifier| {
						self.snippet.object_property_kind_object_property(
							&specifier.exported.name(),
							match &specifier.local {
								ast::ModuleExportName::IdentifierName(ident) => Expression::StaticMemberExpression(
								...
								ast::ModuleExportName::StringLiteral(str) => Expression::ComputedMemberExpression(
									self.snippet.builder.alloc_computed_member_expression(
										SPAN,
										self.snippet.id_ref_expr(&namespace_object_ref, SPAN),
										self.snippet.builder.expression_string_literal(SPAN, str.value.as_str(), None),
										false,
									),
								),
						...
						self.snippet.object_property_kind_object_property(
							...
							match &specifier.local {
								...
								ast::ModuleExportName::StringLiteral(_) => {
									unreachable!(
										"ModuleExportName StringLiteral is invalid in ExportNamedDeclaration without source"
									)
								},
	...
	pub fn transform_export_all_declaration(&mut self, export_all_decl: &ast::ExportAllDeclaration<'ast>) {
