...
impl<'me, 'ast> ScopeHoistingFinalizer<'me, 'ast> {
	pub fn is_global_identifier_reference(&self, id_ref:&IdentifierReference) -> bool {
		let Some(reference_id) = id_ref.reference_id.get() else {
			...
			// `ReferenceId`. They might be global variables. But we don't
			// care about them in this method. This method is only used to check
			// if a `IdentifierReference` from user code is a global variable.
			return false;
		};
		...
		let mut expr = match self.ctx.options.format {
			rolldown_common::OutputFormat::Cjs => {
				if canonical_symbol.chunk_id.is_none() {
					// Scoped scopes must belong to its own chunk, so they don't
					// get assigned to a chunk.
					...
					if is_symbol_in_other_chunk {
						// In cjs output, we need convert the `import { foo }
						// from 'foo'; console.log(foo);`; If `foo` is
						// split into another chunk, we need to convert the code
						// `console.log(foo);` to
						// `console.log(require_xxxx.foo);` instead of
						// keeping `console.log(foo)` as we did in esm output.
						// The reason here is wee need to keep live binding in
						// cjs output.
	...
	fn convert_decl_to_assignment(
		...
		match decl {
					...
					ast::ExpressionStatement {
						expression:ast::Expression::AssignmentExpression(
							ast::AssignmentExpression {
								left:self
									.snippet
									.simple_id_assignment_target(&cls_name, cls_decl.span),
								right:ast::Expression::ClassExpression(
									cls_decl.take_in(self.alloc),
								),
								..TakeIn::dummy(self.alloc)
							}
							.into_in(self.alloc),
			...
			ast::Declaration::FunctionDeclaration(_) => {
				// Function declaration itself as a whole will be hoisted, so we
				// don't need to convert it to an assignment.
				None
			},
			...
			match self.ctx.options.format {
				OutputFormat::Esm => {
					let stmts = export_all_externals_rec_ids.iter().copied().flat_map(|idx| {
						...
						// Insert `import * as ns from 'ext'`external module
						// in esm format Insert `__reExport(exports,
						// ns)`
