...
impl<'me, 'ast> ScopeHoistingFinalizer<'me, 'ast> {
	pub fn is_global_identifier_reference(&self, id_ref:&IdentifierReference) -> bool {
		let Some(reference_id) = id_ref.reference_id.get() else {
			// Some `IdentifierReference`s constructed by bundler don't have a
			// `ReferenceId`. They might be global variables. But we don't care about
			// them in this method. This method is only used to check if a
			// `IdentifierReference` from user code is a global variable.
			return false;
		};
	...
	fn transform_or_remove_import_export_stmt(
		...
		match importee_linking_info.wrap_kind {
			...
			WrapKind::Cjs => {
				// Replace the statement with something like `var import_foo =
				// __toESM(require_foo())`
				...
				// , we need to consider to stimulate the Node.js ESM behavior for maximum
				// compatibility.
			...
			// Replace the import statement with `init_foo()` if `ImportDeclaration` is not a plain
			// import or the importee have side effects.
			WrapKind::Esm => {
				...
				rolldown_common::OutputFormat::Cjs => {
					let chunk_idx_of_canonical_symbol =
						canonical_symbol.chunk_id.unwrap_or_else(|| {
							// Scoped symbols don't get assigned a `ChunkId`. There are skipped for
							// performance reason, because they are surely belong to the
							// chunk they are declared in and won't link to other chunks.
							let symbol_name = canonical_ref.name(self.ctx.symbol_db);
							eprintln!(
								"{canonical_ref:?} {symbol_name:?} is not in any chunk, which is \
								 unexpected",
							);
							panic!(
								"{canonical_ref:?} {symbol_name:?} is not in any chunk, which is \
					...
					if is_symbol_in_other_chunk {
						// In cjs output, we need convert the `import { foo } from 'foo';
						// console.log(foo);`; If `foo` is split into another chunk, we need
						// to convert the code `console.log(foo);` to
						// `console.log(require_xxxx.foo);` instead of keeping
						// `console.log(foo)` as we did in esm output. The reason here is wee need
						// to keep live binding in cjs output.

						let exported_name = &self.ctx.chunk_graph.chunk_table
			...
			ast::Declaration::VariableDeclaration(var_decl) => {
				...
				var_decl.declarations.iter_mut().for_each(|var_decl| {
					...
					if let Some(init_expr) = &mut var_decl.init {
						let left =
							var_decl.id.take_in(self.alloc).into_assignment_target(self.alloc);
				...
				if seq_expr.expressions.is_empty() {
					...
					Some(ast::Statement::ExpressionStatement(
						ast::ExpressionStatement {
							expression:ast::Expression::SequenceExpression(
								seq_expr.into_in(self.alloc),
							),
							..TakeIn::dummy(self.alloc)
						}
						.into_in(self.alloc),
	...
	fn generate_declaration_of_module_namespace_object(&self) -> Vec<ast::Statement<'ast>> {
		...
		let export_all_externals_rec_ids =
			&self.ctx.linking_info.star_exports_from_external_modules;
				...
				ast::ObjectProperty {
					key:if is_validate_identifier_name(prop_name) {
						...
						ast::PropertyKey::StringLiteral(
							self.snippet.alloc_string_literal(prop_name, SPAN),
						)
					},
		...
		let mut export_call_expr =
			self.snippet.call_expr(self.canonical_name_for_runtime("__export"));

		export_call_expr
			.arguments
			.push(ast::Argument::from(self.snippet.id_ref_expr(var_name, SPAN)));
			...
			match property_name {
				...
				"url" => {
					let new_expr = if is_node_cjs {
						...
						let require_path_to_file_url_call =
							self.snippet.builder.alloc_call_expression(
								...
								self.snippet.builder.vec1(ast::Argument::Identifier(
									self.snippet
										.builder
										.alloc_identifier_reference(SPAN, "__filename"),
								)),
								false,
							);
						...
						// If we don't support polyfill `import.meta.url` in this platform and
						// format, we just keep it as it is so users may handle it in their
						// own way.
						None
					};
					return new_expr;
	...
	pub fn handle_new_url_with_string_literal_and_import_meta_url(
		...
		let is_second_arg_import_meta_url = expr.arguments.get(1).map_or(false, |arg| {
			arg.as_expression().is_some_and(ExpressionExt::is_import_meta_url)
		});
		...
		match member_expr {
					...
					self.ctx.linking_info.resolved_member_expr_refs.get(&inner_expr.span)
				{
					match object_ref {
						...
						None => {
							return Some(
								self.snippet
									.member_expr_with_void_zero_object(props, inner_expr.span),
							);
						},
					}
				}
			...
			MemberExpression::StaticMemberExpression(inner_expr) => {
					...
					self.ctx.linking_info.resolved_member_expr_refs.get(&inner_expr.span)
				{
					...
					// these two branch are exclusive since `import.meta` is a
					// global member_expr
	...
	fn get_conflicted_info(
		...
		(original_name != canonical_name.as_str()).then_some((
			symbol_id,
			original_name,
			canonical_name,
		))
