use oxc::{
	...
	ast::{
		Comment, NONE,
		ast::{
			self, BindingIdentifier, ClassElement, Expression, IdentifierReference, ImportExpression, MemberExpression,
...
use rolldown_common::{
	AstScopes, ExportsKind, ImportRecordIdx, ImportRecordMeta, Module, ModuleIdx, ModuleType, OutputFormat, Platform,
	SymbolRef, WrapKind,
};
use rolldown_ecmascript_utils::{
	AllocatorExt, AstSnippet, BindingPatternExt, CallExpressionExt, ExpressionExt, StatementExt, TakeIn,
};
...
impl<'me, 'ast> ScopeHoistingFinalizer<'me, 'ast> {
	...
	fn transform_or_remove_import_export_stmt(&mut self, stmt: &mut Statement<'ast>, rec_id: ImportRecordIdx) -> bool {
		...
		match importee_linking_info.wrap_kind {
			...
			WrapKind::Esm => {
				if (rec.meta.contains(ImportRecordMeta::IS_PLAIN_IMPORT) && !importee.side_effects.has_side_effects())
				...
				let wrapper_ref_expr =
					self.finalized_expr_for_symbol_ref(importee_linking_info.wrapper_ref.unwrap(), false, None);
				...
				if self.ctx.linking_info.is_tla_or_contains_tla_dependency {
					...
					*stmt = self.snippet.builder.statement_expression(
						SPAN,
						ast::Expression::AwaitExpression(self.snippet.builder.alloc_await_expression(SPAN, init_call)),
					);
				} else {
					// `init_foo()`
	...
	fn finalized_expr_for_symbol_ref(
			...
			match self.ctx.options.format {
				rolldown_common::OutputFormat::Cjs => {
					...
					if is_symbol_in_other_chunk {
						...
						self.snippet
							.literal_prop_access_member_expr_expr(require_binding, exported_name)
					} else {
						self.snippet.id_ref_expr(self.canonical_name_for(canonical_ref), SPAN)
					}
				},
		...
		if let Some(ns_alias) = namespace_alias {
			...
			expr = if meta.safe_cjs_to_eliminate_interop_default
				&& original_reference_id.is_some_and(|item| self.interested_namespace_alias_ref_id.contains(&item))
			{
				expr
			} else {
	...
	fn generate_declaration_of_module_namespace_object(&self) -> Vec<ast::Statement<'ast>> {
		let binding_name_for_namespace_object_ref = self.canonical_name_for(self.ctx.module.namespace_object_ref);
			...
			arg_obj_expr.properties.push(ast::ObjectPropertyKind::ObjectProperty(
				ast::ObjectProperty {
					key: if is_validate_identifier_name(prop_name) {
						ast::PropertyKey::StaticIdentifier(self.snippet.id_name(prop_name, SPAN).into_in(self.alloc))
					} else {
			...
			match property_name {
				...
				"url" => {
					let new_expr = if is_node_cjs {
						...
						let require_path_to_file_url_href = self.snippet.builder.alloc_static_member_expression(
							original_expr_span,
				...
				"dirname" | "filename" => {
					return is_node_cjs.then_some(ast::Expression::Identifier(
						self.snippet
							.builder
							.alloc_identifier_reference(SPAN, format!("__{property_name}")),
					));
				},
				_ => {},
			}
	...
	fn rewrite_rollup_file_url(&self, property_name: &str) -> Option<Expression<'ast>> {
		...
		if let Some(reference_id) = property_name.strip_prefix("ROLLUP_FILE_URL_") {
			...
			let absolute_asset_file_name =
				asset_file_name.absolutize_with(self.ctx.options.cwd.as_path().join(&self.ctx.options.out_dir));
			let relative_asset_path =
				&self.ctx.chunk_graph.chunk_table[self.ctx.chunk_id].relative_path_for(&absolute_asset_file_name);
	...
	pub fn handle_new_url_with_string_literal_and_import_meta_url(
		...
		let import_path =
			self.ctx.chunk_graph.chunk_table[self.ctx.chunk_id].relative_path_for(asset_filename.as_path());
	...
	fn try_rewrite_member_expr(&self, member_expr: &ast::MemberExpression<'ast>) -> Option<Expression<'ast>> {
		match member_expr {
			MemberExpression::ComputedMemberExpression(inner_expr) => {
				if let Some((object_ref, props)) = self.ctx.linking_info.resolved_member_expr_refs.get(&inner_expr.span)
				{
					match object_ref {
						Some(object_ref) => {
	...
	fn get_conflicted_info(&self, id: &BindingIdentifier<'ast>) -> Option<(&str, &rolldown_rstr::Rstr)> {
	...
	fn try_rewrite_global_require_call(&self, call_expr: &mut ast::CallExpression<'ast>) -> Option<Expression<'ast>> {
		if call_expr.is_global_require_call(self.scope) && !call_expr.span.is_unspanned() {
			...
			if let Some(rec_id) = self.ctx.module.imports.get(&call_expr.span).copied() {
				...
				if rec.meta.contains(ImportRecordMeta::CALL_RUNTIME_REQUIRE) {
					*call_expr.callee.get_inner_expression_mut() = self.finalized_expr_for_runtime_symbol("__require");
				}
				let rewrite_ast = match &self.ctx.modules[rec.resolved_module] {
					...
					Module::External(importee) => {
						let request_path = call_expr.arguments.get_mut(0).expect("require should have an argument");
						let importer_chunk = &self.ctx.chunk_graph.chunk_table[self.ctx.chunk_id];
						// Rewrite `require('xxx')` to `require('fs')`, if there is an alias that maps 'xxx' to 'fs'
						*request_path = ast::Argument::StringLiteral(
							self.snippet
								.alloc_string_literal(&importee.get_import_path(importer_chunk), request_path.span()),
						);
						None
					},
				};
	...
	fn try_rewrite_inline_dynamic_import_expr(&self, import_expr: &ImportExpression<'ast>) -> Option<Expression<'ast>> {
						...
						WrapKind::Cjs => {
							...
							Some(
								self.snippet.promise_resolve_then_call_expr(
									...
									self.snippet
										.builder
										.vec1(self.snippet.return_stmt(self.snippet.wrap_with_to_esm(
	...
	fn process_fn(
		...
		self.ctx
			.keep_name_statement_to_insert
			.push((insert_position, original_name, new_name));
		None
	}

	fn keep_name_helper_for_class(&self, id: Option<&BindingIdentifier<'ast>>) -> Option<ClassElement<'ast>> {
		if !self.ctx.options.keep_names {
			return None;
		}
	...
	fn generate_esm_namespace_in_cjs(&self) -> Vec<ast::Statement<'ast>> {
		...
		if let Some(esm_ns) = &self.ctx.module.esm_namespace_in_cjs {
			if self.ctx.module.stmt_infos[esm_ns.stmt_info_idx].is_included {
				// `__toESM`
				let to_esm_fn_name =
					self.finalized_expr_for_symbol_ref(self.canonical_ref_for_runtime("__toESM"), false, None);

				// `require_foo`
				let importee_wrapper_ref_name =
					self.finalized_expr_for_symbol_ref(self.ctx.linking_info.wrapper_ref.unpack(), false, None);
				...
				let var_init = ast::Statement::VariableDeclaration(self.snippet.builder.alloc_variable_declaration(
					SPAN,
					ast::VariableDeclarationKind::Var,
					declarations,
					false,
