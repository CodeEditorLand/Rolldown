...
impl<'ast> VisitMut<'ast> for ScopeHoistingFinalizer<'_, 'ast> {
	...
	fn visit_program(&mut self, program:&mut ast::Program<'ast>) {
		...
		// we don't want oxc to generate hashbang statement in module level since we
		// already handle them in chunk level
		...
		shimmed_exports.into_iter().for_each(|(_name, symbol_ref)| {
			debug_assert!(
				!self.ctx.module.stmt_infos.declared_stmts_by_symbol(symbol_ref).is_empty()
			);
			...
			if is_included {
				...
				program
					.body
					.push(self.snippet.var_decl_stmt(canonical_name, self.snippet.void_zero()));
			}
		});
		...
		if needs_wrapper {
			match self.ctx.linking_info.wrap_kind {
				WrapKind::Cjs => {
					let wrap_ref_name =
						self.canonical_name_for(self.ctx.linking_info.wrapper_ref.unwrap());
					...
					old_body.into_iter().for_each(|mut stmt| {
						match &mut stmt {
							ast::Statement::VariableDeclaration(_) => {
								if let Some(converted) = self.convert_decl_to_assignment(
									stmt.to_declaration_mut(),
									&mut hoisted_names,
								) {
							...
							ast::match_module_declaration!(Statement) => {
								if stmt.is_typescript_syntax() {
									unreachable!(
										"At this point, typescript module declarations should \
										 have been removed or transformed"
									)
								}
	...
	fn visit_binding_identifier(&mut self, ident:&mut ast::BindingIdentifier<'ast>) {
		if let Some(symbol_id) = ident.symbol_id.get() {
			...
			// Some `BindingIdentifier`s constructed by bundler don't have
			// `SymbolId` and we just ignore them.
		}
	}
	...
	fn visit_statements(&mut self, it:&mut allocator::Vec<'ast, ast::Statement<'ast>>) {
		...
		let previous_keep_name_statement =
	...
	fn visit_expression(&mut self, expr:&mut ast::Expression<'ast>) {
		match expr {
			...
			ast::Expression::ThisExpression(this_expr) => {
				if let Some(kind) =
					self.ctx.module.ecma_view.this_expr_replace_map.get(&this_expr.span)
				{
					match kind {
						ThisExprReplaceKind::Exports => {
							*expr = self
								.snippet
								.builder
								.expression_identifier_reference(SPAN, "exports");
						},
	...
	// The `foo_exports.bar.a` ast is
	// `StaticMemberExpression(StaticMemberExpression)`, The outer
	// StaticMemberExpression span is `foo_exports.bar.a`, the
	// `visit_expression(Exprssion::MemberExpression)` is called with
	// `foo_exports.bar`, the span is inner StaticMemberExpression.
	...
	fn visit_import_expression(&mut self, expr:&mut ast::ImportExpression<'ast>) {
		// Make sure the import expression is in correct form. If it's not, we should
		// leave it as it is.
			...
			ast::Expression::StringLiteral(str) if expr.arguments.len() == 0 => {
				...
				match &self.ctx.modules[importee_id] {
					Module::Normal(_importee) => {
						let importer_chunk_id = self.ctx.chunk_graph.module_to_chunk
							[self.ctx.module.idx]
							...
							(&decl.id.kind, decl.init.as_mut())
						{
							match init {
								...
								ast::Expression::FunctionExpression(fn_expression) => {
									// The `var fn = function foo() {}` shoulde be generate
									// `__name(fn, 'foo')` to keep the name
									self.process_fn(
										Some(id),
										Some(fn_expression.id.as_ref().unwrap_or_else(|| id)),
									);
								},
								_ => {},
							}
...
impl<'ast> ScopeHoistingFinalizer<'_, 'ast> {
	...
	fn try_rewrite_global_require_call(
		...
		if call_expr.is_global_require_call(self.scope) && !call_expr.span.is_unspanned() {
			//  `require` calls that can't be recognized by rolldown are ignored in
			// scanning, so they were not stored in `NomralModule#imports`.  we just keep
			// these `require` calls as it is
			if let Some(rec_id) = self.ctx.module.imports.get(&call_expr.span).copied() {
				...
				if rec.meta.contains(ImportRecordMeta::CALL_RUNTIME_REQUIRE) {
					*call_expr.callee.get_inner_expression_mut() = self
						.finalized_expr_for_symbol_ref(
							self.canonical_ref_for_runtime("__require"),
							false,
						);
				}
				...
				let rewrite_ast = match &self.ctx.modules[rec.resolved_module] {
					Module::Normal(importee) => {
						match importee.module_type {
							ModuleType::Json => {
								// Nodejs treats json files as an esm module with a default export
								// and rolldown follows this behavior. And to make sure the
								// runtime behavior is correct, we need to rewrite
								// `require('xxx.json')` to `require('xxx.json').default` to align
								// with the runtime behavior of nodejs.

								// Rewrite `require(...)` to `require_xxx(...)` or `(init_xxx(),
								// __toCommonJS(xxx_exports).default)`
								let importee_linking_info = &self.ctx.linking_infos[importee.idx];

								let wrap_ref_name = self
									.canonical_name_for(importee_linking_info.wrapper_ref.unwrap());
								...
								if matches!(importee.exports_kind, ExportsKind::CommonJs) {
									...
									let to_commonjs_ref_name =
										self.canonical_name_for_runtime("__toCommonJS");
									Some(
							...
							_ => {
								...
								if matches!(importee.exports_kind, ExportsKind::CommonJs) {
									...
									if rec.meta.contains(ImportRecordMeta::IS_REQUIRE_UNUSED) {
										...
										// `xxx_exports`
										let namespace_object_ref_expr = self
											.finalized_expr_for_symbol_ref(
										...
										let to_commonjs_ref =
											self.canonical_ref_for_runtime("__toCommonJS");
										// `__toCommonJS`
										let to_commonjs_expr = self
											.finalized_expr_for_symbol_ref(to_commonjs_ref, false);
					...
					Module::External(importee) => {
						let request_path = call_expr
							.arguments
							.get_mut(0)
							.expect("require should have an argument");

						// Rewrite `require('xxx')` to `require('fs')`, if there is an alias that
						// maps 'xxx' to 'fs'
	...
	fn try_rewrite_inline_dynamic_import_expr(
			...
			match &self.ctx.modules[importee_id] {
				Module::Normal(importee) => {
					...
					let new_expr = match importee_linking_info.wrap_kind {
						...
						WrapKind::None => {
							// The nature of `import()` is to load the module dynamically/lazily, so
							// imported modules would must be wrapped, so we could make sure
							// the module is executed lazily.
		...
		if matches!(self.ctx.options.format, OutputFormat::Cjs) {
			// Convert `import('./foo.mjs')` to `Promise.resolve().then(function() { return
			// require('foo.mjs') })`
			let rec_id = self.ctx.module.imports.get(&import_expr.span)?;
			let rec = &self.ctx.module.import_records[*rec_id];
			let importee_id = rec.resolved_module;
			match &self.ctx.modules[importee_id] {
							...
							self.snippet.builder.alloc_return_statement(
								...
								Some(ast::Expression::CallExpression(
									self.snippet.builder.alloc_call_expression(
										...
										self.snippet
											.builder
											.expression_identifier_reference(SPAN, "require"),
										NONE,
										self.snippet.builder.vec1(ast::Argument::StringLiteral(
											self.snippet.alloc_string_literal(
												&import_path,
												import_expr.span,
											),
										)),
				...
				Module::External(_) => {
					// For `import('external')`, we just keep it as it is to
					// preserve user's intention
				},
			}
		}
		None
	...
	fn remove_unused_top_level_stmt(&mut self, program:&mut ast::Program<'ast>) {
		...
		// skip first statement info to make sure `program.body` has same index as
		// `stmt_infos`
		old_body
			.into_iter()
			.enumerate()
			.zip(self.ctx.module.stmt_infos.iter().skip(1))
			.for_each(|((_top_stmt_idx, mut top_stmt), stmt_info)| {
						...
						match &self.ctx.modules[rec.resolved_module] {
							Module::Normal(importee) => {
								...
								if matches!(importee_linking_info.wrap_kind, WrapKind::Esm) {
									let wrapper_ref_name = self.canonical_name_for(
										importee_linking_info.wrapper_ref.unwrap(),
									);
									program
										.body
										.push(self.snippet.call_expr_stmt(wrapper_ref_name));
								}

								match importee.exports_kind {
									ExportsKind::Esm => {
										if importee_linking_info.has_dynamic_exports {
											let re_export_fn_name =
												self.canonical_name_for_runtime("__reExport");
											let importer_namespace_name = self.canonical_name_for(
												self.ctx.module.namespace_object_ref,
											);
											// __reExport(exports, otherExports)
											let importee_namespace_name = self
												.canonical_name_for(importee.namespace_object_ref);
											program.body.push(
												self.snippet
									...
									ExportsKind::CommonJs => {
										...
										let to_esm_fn_name =
											self.canonical_name_for_runtime("__toESM");

										let importee_wrapper_ref_name = self.canonical_name_for(
											importee_linking_info.wrapper_ref.unwrap(),
										);
										...
										program.body.push(
											self.snippet
												.alloc_call_expr_with_2arg_expr_expr(
													re_export_fn_name,
													self.snippet
														.id_ref_expr(importer_namespace_name, SPAN),
													self.snippet.to_esm_call_with_interop(
														to_esm_fn_name,
														self.snippet.call_expr_expr(
					...
					match &mut default_decl.declaration {
						...
						ast::ExportDefaultDeclarationKind::FunctionDeclaration(func) => {
							...
							if func.id.is_none() {
								...
								func.id = Some(
									self.snippet.id(canonical_name_for_default_export_ref, SPAN),
								);
							}
							top_stmt =
								ast::Statement::FunctionDeclaration(func.take_in(self.alloc));
						},
	...
	fn process_fn(
		...
		let (symbol_id, _, canonical_name) =
			self.get_conflicted_info(symbol_binding_id.as_ref()?)?;
