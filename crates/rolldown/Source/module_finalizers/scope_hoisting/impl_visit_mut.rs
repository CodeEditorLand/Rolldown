...
impl<'ast> VisitMut<'ast> for ScopeHoistingFinalizer<'_, 'ast> {
	...
	fn visit_program(&mut self, program: &mut ast::Program<'ast>) {
		...
		self.namespace_alias_symbol_id = self
			...
			.filter_map(|(symbol_ref, v)| {
				...
				self.ctx
					...
					.namespace_alias
					.as_ref()
					.and_then(|alias| (alias.property_name.as_str() == "default").then_some(symbol_ref.symbol))
			})
			.collect::<FxHashSet<_>>();
		...
		let mut shimmed_exports = self.ctx.linking_info.shimmed_missing_exports.iter().collect::<Vec<_>>();
	...
	fn visit_expression(&mut self, expr: &mut ast::Expression<'ast>) {
		match expr {
			...
			_ => {
				if let Some(new_expr) = expr.as_member_expression().and_then(|expr| self.try_rewrite_member_expr(expr))
				{
					*expr = new_expr;
				}
			},
	...
	fn visit_assignment_target_property(&mut self, property: &mut ast::AssignmentTargetProperty<'ast>) {
		if let ast::AssignmentTargetProperty::AssignmentTargetPropertyIdentifier(prop) = property {
			if let Some(target) = self.generate_finalized_simple_assignment_target_for_reference(&prop.binding) {
