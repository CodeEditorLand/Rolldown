...
impl<'me, 'ast> VisitMut<'ast> for ScopeHoistingFinalizer<'me, 'ast> {
	...
	fn visit_program(&mut self, program:&mut ast::Program<'ast>) {
		...
		// Skip the first statement info, which is the namespace variable
		// declaration
		stmt_infos.next();
		...
		if needs_wrapper {
			match self.ctx.linking_info.wrap_kind {
				...
				WrapKind::Esm => {
					...
					// Hoist all top-level "var" and "function" declarations out
					// of the closure
	...
	fn visit_expression(&mut self, expr:&mut ast::Expression<'ast>) {
		if let Some(call_expr) = expr.as_call_expression_mut() {
			if call_expr.is_global_require_call(self.scope) && !call_expr.span.is_empty() {
				//  `require` calls that can't be recognized by rolldown are
				// ignored in scanning, so they were not stored in
				// `NomralModule#imports`.  we just keep these `require`
				// calls as it is
					...
					match &self.ctx.modules[rec.resolved_module] {
						Module::Normal(importee) => {
							match importee.module_type {
								ModuleType::Json => {
									// Nodejs treats json files as an esm module
									// with a default export and rolldown
									// follows this behavior.
									// And to make sure the runtime behavior is
									// correct, we need to rewrite
									// `require('xxx.json')` to
									// `require('xxx.json').default` to align
									// with the runtime behavior of nodejs.

									// Rewrite `require(...)` to
									// `require_xxx(...)` or `(init_xxx(),
									// __toCommonJS(xxx_exports).default)`
									let importee_linking_info =
										&self.ctx.linking_infos[importee.idx];
									let wrap_ref_name = self.canonical_name_for(
										importee_linking_info.wrapper_ref.unwrap(),
									);
						...
						Module::External(importee) => {
							...
							// Rewrite `require('xxx')` to `require('fs')`, if
							// there is an alias that maps 'xxx' to 'fs'
							*request_path = ast::Argument::StringLiteral(
								self.snippet
									.alloc_string_literal(&importee.name, request_path.span()),
							);
						},
					}
				}
					...
					self.ctx.linking_info.resolved_member_expr_refs.get(&inner_expr.span)
				{
					match resolved {
						Some((object_ref, props)) => {
							...
							let replaced_expr = self.snippet.member_expr_or_ident_ref(
								object_ref_expr,
								props,
								inner_expr.span,
							);
					...
					Module::Normal(importee) => {
						...
						match importee_linking_info.wrap_kind {
							WrapKind::Esm => {
								// `(init_foo(), j)`
								let importee_linking_info = &self.ctx.linking_infos[importee_id];
								let importee_wrapper_ref_name = self
	...
	fn visit_object_pattern(&mut self, pat:&mut ast::ObjectPattern<'ast>) {
		for prop in pat.properties.iter_mut() {
			match &mut prop.value.kind {
				// Ensure `const { a } = ...;` will be rewritten to `const { a:
				// a$1 } = ...` instead of `const { a$1 } = ...`
				// Ensure `function foo({ a }) {}` will be rewritten to
				// `function foo({ a: a$1 }) {}` instead of `function foo({ a$1
				// }) {}`
				ast::BindingPatternKind::BindingIdentifier(ident) if prop.shorthand => {
					if let Some(symbol_id) = ident.symbol_id.get() {
						let canonical_name =
							self.canonical_name_for((self.ctx.id, symbol_id).into());
						if ident.name != canonical_name.as_str() {
							ident.name = self.snippet.atom(canonical_name);
							prop.shorthand = false;
						}
				...
				// Ensure `const { a = 1 } = ...;` will be rewritten to `const {
				// a: a$1 = 1 } = ...` instead of `const { a$1 = 1 } = ...`
				// Ensure `function foo({ a = 1 }) {}` will be rewritten to
				// `function foo({ a: a$1 = 1 }) {}` instead of `function foo({
				// a$1 = 1 }) {}`
				ast::BindingPatternKind::AssignmentPattern(assign_pat)
					if prop.shorthand
						&& matches!(
							assign_pat.left.kind,
							ast::BindingPatternKind::BindingIdentifier(_)
						) =>
				{
					let ast::BindingPatternKind::BindingIdentifier(ident) =
						&mut assign_pat.left.kind
					else {
						unreachable!()
					};
					if let Some(symbol_id) = ident.symbol_id.get() {
				...
				_ => {
					...
					//   }) {}`
					// They could keep correct semantics after renaming, so we
	...
	fn visit_import_expression(&mut self, expr:&mut ast::ImportExpression<'ast>) {
		// Make sure the import expression is in correct form. If it's not, we
