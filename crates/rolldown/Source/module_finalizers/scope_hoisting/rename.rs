...
impl<'ast> ScopeHoistingFinalizer<'_, 'ast> {
	/// return `None` if
	/// - the reference is for a global variable/the reference doesn't have a
	///   `SymbolId`
	...
	pub fn generate_finalized_expr_for_reference(
		...
		// Some `IdentifierReference`s constructed by bundler don't have `ReferenceId`
	...
	pub fn try_rewrite_identifier_reference_expr(
		...
		if let Some(new_expr) = self.generate_finalized_expr_for_reference(ident_ref, is_callee) {
			...
			// Nevertheless, this identifier is processed and don't need to be processed
			// again.
	...
	pub fn rewrite_object_pat_shorthand(&mut self, pat:&mut ast::ObjectPattern<'ast>) {
		for prop in pat.properties.iter_mut() {
			match &mut prop.value.kind {
				// Ensure `const { a } = ...;` will be rewritten to `const { a: a } = ...` instead
				// of `const { a } = ...` Ensure `function foo({ a }) {}` will be rewritten to
				// `function foo({ a: a }) {}` instead of `function foo({ a }) {}`
				...
				// Ensure `const { a = 1 } = ...;` will be rewritten to `const { a: a = 1 } = ...`
				// instead of `const { a = 1 } = ...` Ensure `function foo({ a = 1 }) {}` will
				// be rewritten to `function foo({ a: a = 1 }) {}` instead of `function foo({ a = 1
				// }) {}`
