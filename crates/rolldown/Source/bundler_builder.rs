...
impl BundlerBuilder {
	pub fn build(mut self) -> Bundler {
		...
		let NormalizeOptionsReturn { mut options, resolve_options, mut warnings } = normalize_options(self.options);
		...
		// TODO: error handling
		Self::merge_transform_config_from_ts_config(&mut options, tsconfig_filename, &resolver, &mut warnings).unwrap();
