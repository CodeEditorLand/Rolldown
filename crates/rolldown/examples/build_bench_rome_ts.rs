...
async fn main() {
	...
	let mut bundler = Bundler::new(BundlerOptions {
		...
		shim_missing_exports: Some(true), /* Need this due rome is not written with
		                                   * `isolatedModules: true` */
		resolve: Some(ResolveOptions {
			tsconfig_filename: Some(root.join("tmp/bench/rome/src/tsconfig.json").to_str().unwrap().to_string()),
			..Default::default()
		}),
		..Default::default()
	});
