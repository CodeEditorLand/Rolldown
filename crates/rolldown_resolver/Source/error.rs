...
pub fn oxc_resolve_error_to_reason(e: &ResolveError) -> String {
	match e {
		...
		ResolveError::TsconfigSelfReference(_) => {
			"Tsconfig's project reference path points to this tsconfig"
				.to_string()
		},
		...
		ResolveError::Specifier(_) => {
			"The provided path specifier cannot be parsed".to_string()
		},
		...
		ResolveError::InvalidModuleSpecifier(_, _) => {
			"Invalid module specifier".to_string()
		},
		ResolveError::InvalidPackageTarget(_, _, _) => {
			"Invalid package target".to_string()
		},
		...
		ResolveError::InvalidPackageConfig(_) => {
			"Invalid package config".to_string()
		},
