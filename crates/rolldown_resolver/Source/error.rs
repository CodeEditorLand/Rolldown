...
pub fn oxc_resolve_error_to_reason(e: &ResolveError) -> String {
	match e {
		...
		ResolveError::ExtensionAlias { .. } => "All of the aliased extensions are not found".to_string(),
		...
		ResolveError::PackagePathNotExported(..) => "Package subpath is not defined by exports".to_string(),
		...
		ResolveError::InvalidPackageConfigDirectory(_) => "Expecting folder to folder mapping".to_string(),

		ResolveError::PackageImportNotDefined(..) => "Package import specifier is not defined".to_string(),

		ResolveError::Unimplemented(_) => "Unimplemented".to_string(),
		ResolveError::Recursion => "Recursion in resolving".to_string(),
		ResolveError::MatchedAliasNotFound(..) => "Matched alias not found".to_string(),
		_ => todo!(),
	}
}
