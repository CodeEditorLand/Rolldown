...
use rolldown_common::{
	ImportKind, ModuleDefFormat, PackageJson, Platform, ResolveOptions,
};
...
impl<F: FileSystem + Default> Resolver<F> {
	pub fn new(
		...
		default_conditions
			.extend(raw_resolve.condition_names.clone().unwrap_or_default());
		match platform {
			Platform::Node => {
		...
		let main_fields =
			raw_resolve.main_fields.clone().unwrap_or_else(|| match platform {
				Platform::Node => {
					vec!["main".to_string(), "module".to_string()]
				},
				Platform::Browser => vec![
					"browser".to_string(),
					"module".to_string(),
					"main".to_string(),
				],
		...
		let resolve_options_with_default_conditions = OxcResolverOptions {
			tsconfig: raw_resolve.tsconfig_filename.map(|p| {
				...
				TsconfigOptions {
					config_file: if path.is_relative() {
						cwd.join(path)
					} else {
						path
					},
		...
		let default_resolver = ResolverGeneric::new_with_file_system(
			fs,
			resolve_options_with_default_conditions,
		);
		let import_resolver = default_resolver
			.clone_with_options(resolve_options_with_import_conditions);
		let require_resolver = default_resolver
			.clone_with_options(resolve_options_with_require_conditions);
...
impl<F: FileSystem + Default> Resolver<F> {
	pub fn resolve(
		...
		let importer_dir =
			importer.and_then(|importer| importer.parent()).and_then(|inner| {
		...
		if resolution.is_err() && is_user_defined_entry {
			...
			if need_rollup_resolve_compat {
				...
				let fallback = selected_resolver.resolve(
					context_dir,
					&self.cwd.join(specifier).normalize().to_string_lossy(),
				);
				if fallback.is_ok() {
					resolution = fallback;
				}
		...
		match resolution {
			Ok(info) => {
				let package_json =
					info.package_json().map(|p| self.cached_package_json(p));
	...
	fn cached_package_json(
		&self,
		oxc_pkg_json: &OxcPackageJson,
	) -> Arc<PackageJson> {
