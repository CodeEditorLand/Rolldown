...
use rolldown_common::{ImportKind, ModuleDefFormat, PackageJson, Platform, ResolveOptions, ResolvedId};
...
use oxc_resolver::{
	EnforceExtension, FsCache, PackageJsonSerde as OxcPackageJson, PackageType, Resolution, ResolveError,
	...
	pub fn new(raw_resolve: ResolveOptions, platform: Platform, cwd: PathBuf, fs: F) -> Self {
		...
		let resolve_options_with_default_conditions = OxcResolverOptions {
			...
			extensions: raw_resolve
				.extensions
				.unwrap_or_else(|| [".jsx", ".js", ".ts", ".tsx"].into_iter().map(str::to_string).collect()),
			fallback: vec![],
		...
		let resolve_options_for_css =
			OxcResolverOptions { prefer_relative: true, ..resolve_options_with_default_conditions.clone() };

		let resolve_options_for_new_url =
			OxcResolverOptions { prefer_relative: true, ..resolve_options_with_default_conditions.clone() };

		let default_resolver =
			ResolverGeneric::new_with_cache(Arc::new(FsCache::new(fs)), resolve_options_with_default_conditions);
		let import_resolver = default_resolver.clone_with_options(resolve_options_with_import_conditions);
		let require_resolver = default_resolver.clone_with_options(resolve_options_with_require_conditions);
...
impl<F: FileSystem + Default> Resolver<F> {
	pub fn resolve(
		...
		let selected_resolver = match import_kind {
			ImportKind::Import | ImportKind::DynamicImport | ImportKind::HotAccept => &self.import_resolver,
		...
		if resolution.is_err() && is_user_defined_entry {
			...
			if need_rollup_resolve_compat {
				...
				let fallback =
					selected_resolver.resolve(context_dir, &self.cwd.join(specifier).normalize().to_string_lossy());
				if fallback.is_ok() {
					resolution = fallback;
				}
			}
		...
		match self.package_json_cache.get(&oxc_pkg_json.realpath) {
			...
			_ => {
				...
				self.package_json_cache
					.insert(oxc_pkg_json.realpath.clone(), Arc::clone(&pkg_json));
				pkg_json
			},
		}
	}

	pub fn resolve_tsconfig<T: AsRef<Path>>(&self, path: &T) -> Result<Arc<TsConfigSerde>, ResolveError> {
...
fn infer_module_def_format<F: FileSystem + Default>(info: &Resolution<FsCache<F>>) -> ModuleDefFormat {
	...
	if let Some(package_json) = is_js_like_extension.then(|| info.package_json()).and_then(|item| item) {
...
fn impl_rewritten_file_extensions_via_extension_alias(extension_alias: &mut Vec<(String, Vec<String>)>) {
	...
	let mut rewritten_file_extensions = FxIndexMap::from_iter([
		...
		(
			".jsx".to_string(),
			vec![".jsx".to_string(), ".ts".to_string(), ".tsx".to_string()],
		),
