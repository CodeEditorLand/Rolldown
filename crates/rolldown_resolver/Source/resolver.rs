...
use oxc_resolver::{
	EnforceExtension,
	PackageJson as OxcPackageJson,
	Resolution,
	ResolveError,
	ResolveOptions as OxcResolverOptions,
	ResolverGeneric,
	TsconfigOptions,
};
...
impl<F:FileSystem + Default> Resolver<F> {
	...
	pub fn new(raw_resolve:ResolveOptions, platform:Platform, cwd:PathBuf, fs:F) -> Self {
		...
		let main_fields = raw_resolve.main_fields.clone().unwrap_or_else(|| {
			match platform {
				Platform::Node => {
		...
		let alias_fields = raw_resolve.alias_fields.clone().unwrap_or_else(|| {
			match platform {
					...
					alias
						...
						.map(|(key, value)| {
							(
								...
								value
									.into_iter()
									.map(oxc_resolver::AliasValue::Path)
									.collect::<Vec<_>>(),
							)
						})
						.collect::<Vec<_>>()
	...
	pub fn resolve(
		...
		if resolution.is_err() && is_user_defined_entry {
			...
			if need_rollup_resolve_compat {
				// Rolldown doesn't pursue to have the same resolve behavior as Rollup. Even
				// though, in most cases, rolldown have the same resolve result as Rollup. And
				// in this branch, it's the case that rolldown will perform differently from
				// Rollup.

				// The case is user writes config like `{ input: 'main' }`. `main` would be
				// treated as a npm package name in rolldown and try to resolve it from
				// `node_modules`. But rollup will resolve it to `<CWD>/main.{js,mjs,cjs}`.

				// So in this branch, to improve rollup-compatibility, we try to simulate the
				// Rollup's resolve behavior in this case. // Related rollup code: https://github.com/rollup/rollup/blob/680912e2ceb42c8d5e571e01c6ece0e4889aecbb/src/utils/resolveId.ts#L56.
...
// Some notices:
// - We are using `extension_alias` feature to simulate the esbuild's
//   `rewrittenFileExtensions` feature. But there are differences things, so we
...
fn impl_rewritten_file_extensions_via_extension_alias(
	...
	// The first alias is the original extension to make sure that `foo.js` will be
	// resolved to `foo.js` if `foo.js` exists.
