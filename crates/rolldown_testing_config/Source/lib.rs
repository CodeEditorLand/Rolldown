...
pub struct TestMeta {
	...
	/// If `false`, the compiled artifacts won't be executed, but `_test.mjs`
	/// will be still executed if exists.
	...
	/// If `true`, the bundle will be called with `write()` instead of
	/// `generate()`.
	#[serde(default = "true_by_default")]
