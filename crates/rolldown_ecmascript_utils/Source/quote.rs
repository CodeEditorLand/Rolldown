...
pub fn quote_expr<'alloc>(alloc: &'alloc Allocator, input: &str) -> oxc::ast::ast::Expression<'alloc> {
...
pub fn quote_stmts<'alloc>(alloc: &'alloc Allocator, input: &str) -> allocator::Vec<'alloc, ast::Statement<'alloc>> {
	let alloc_input = allocator::String::from_str_in(input, alloc).into_bump_str();
