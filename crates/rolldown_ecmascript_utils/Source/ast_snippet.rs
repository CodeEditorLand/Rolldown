use oxc::{
	...
	ast::{
		AstBuilder, NONE,
		ast::{
			self, Argument, BindingIdentifier, ClassElement, Declaration, Expression, FunctionType, ImportOrExportKind,
...
impl<'ast> AstSnippet<'ast> {
	...
	#[inline]
	pub fn alloc_id_ref(&self, name: PassedStr, span: Span) -> Box<'ast, ast::IdentifierReference<'ast>> {
	...
	pub fn member_expr_with_void_zero_object(&self, names: &[CompactStr], span: Span) -> ast::Expression<'ast> {
		if names.len() == 1 {
			self.void_zero()
		} else {
	...
	/// `name(arg)`
	pub fn call_expr_with_arg_expr_expr(&self, name: PassedStr, arg: ast::Expression<'ast>) -> ast::Expression<'ast> {
	...
	/// `var [name] = [init]`
	#[inline]
	pub fn var_decl_stmt(&self, name: PassedStr, init: ast::Expression<'ast>) -> ast::Statement<'ast> {
		ast::Statement::from(self.decl_var_decl(name, init))
	}

	/// `var [name] = [init]`
	pub fn decl_var_decl(&self, name: PassedStr, init: ast::Expression<'ast>) -> ast::Declaration<'ast> {
		let declarations =
	...
	/// `var [name] = [init]`
	pub fn var_decl(&self, name: PassedStr, init: ast::Expression<'ast>) -> Box<'ast, ast::VariableDeclaration<'ast>> {
		let declarations =
			self.builder.vec1(self.builder.variable_declarator(
				SPAN,
				ast::VariableDeclarationKind::Var,
		...
		self.builder
			.alloc_variable_declaration(SPAN, ast::VariableDeclarationKind::Var, declarations, false)
	}

	/// `var [name];`
	pub fn var_decl_without_init(&self, name: PassedStr) -> Box<'ast, ast::VariableDeclaration<'ast>> {
		let declarations =
			self.builder.vec1(self.builder.variable_declarator(
				SPAN,
				ast::VariableDeclarationKind::Var,
	...
	pub fn commonjs_wrapper_stmt(
		...
		//  __commonJS(...)
		let mut commonjs_call_expr = self
			.builder
	...
	pub fn esm_wrapper_stmt(
		...
		let params =
			self.builder
				.formal_parameters(SPAN, ast::FormalParameterKind::Signature, self.builder.vec(), NONE);
		let body = self.builder.function_body(SPAN, self.builder.vec(), statements);

		//  __esm(...)
		let mut esm_call_expr = self.builder.call_expression(SPAN, esm_fn_expr, NONE, self.builder.vec(), false);
	...
	/// (a, b)
	/// ```
	pub fn seq2_in_paren_expr(&self, a: ast::Expression<'ast>, b: ast::Expression<'ast>) -> ast::Expression<'ast> {
		let mut expressions = self.builder.vec_with_capacity(2);
		expressions.push(a);
		expressions.push(b);
		let seq_expr = ast::Expression::SequenceExpression(self.builder.alloc_sequence_expression(SPAN, expressions));
		ast::Expression::ParenthesizedExpression(self.builder.alloc_parenthesized_expression(SPAN, seq_expr))
	}
	...
	/// ￣￣ AssignmentTarget
	/// ```
	pub fn simple_id_assignment_target(&self, id: PassedStr, span: Span) -> ast::AssignmentTarget<'ast> {
	...
	pub fn alloc_string_literal(&self, value: PassedStr, span: Span) -> Box<'ast, ast::StringLiteral<'ast>> {
	...
	pub fn app_static_import_star_call_stmt(&self, as_name: &str, importee_source: &str) -> ast::Statement<'ast> {
	...
	pub fn require_call_expr(&self, source: &str) -> Expression<'ast> {
		self.builder.expression_call(
			...
			NONE,
			self.builder
				.vec1(Argument::from(self.builder.expression_string_literal(SPAN, source, None))),
			false,
		)
	}
	...
	pub fn promise_resolve_then_call_expr(
		...
		let callee = ast::Expression::StaticMemberExpression(self.builder.alloc_static_member_expression(
			SPAN,
		...
		ast::Expression::CallExpression(self.builder.alloc_call_expression(span, callee, NONE, arguments, false))
	}

	// return xxx
	pub fn return_stmt(&self, argument: ast::Expression<'ast>) -> ast::Statement<'ast> {
		ast::Statement::ReturnStatement(
			ast::ReturnStatement { argument: Some(argument), ..TakeIn::dummy(self.alloc()) }.into_in(self.alloc()),
		)
	}
	...
	pub fn wrap_with_to_esm(
		...
		let args = if node_mode {
			self.builder.vec_from_iter([
				Argument::from(expr),
				Argument::from(self.builder.expression_numeric_literal(SPAN, 1.0, None, NumberBase::Decimal)),
			])
		} else {
			self.builder.vec1(Argument::from(expr))
		};
		ast::Expression::CallExpression(self.builder.alloc_call_expression(SPAN, to_esm_fn_expr, NONE, args, false))
	}
	...
	pub fn module_exports_expr_stmt(&self, expr: Expression<'ast>) -> Statement<'ast> {
		...
		ast_builder.statement_expression(
			...
			ast_builder.expression_assignment(
				...
				ast::AssignmentTarget::from(ast::SimpleAssignmentTarget::from(ast_builder.member_expression_static(
					SPAN,
	...
	pub fn keep_name_call_expr_stmt(&self, original_name: PassedStr, new_name: PassedStr) -> Statement<'ast> {
	...
	pub fn alloc_simple_call_expr(&self, callee: Expression<'ast>) -> allocator::Box<'ast, ast::CallExpression<'ast>> {
		self.builder
