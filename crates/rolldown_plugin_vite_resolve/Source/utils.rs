...
pub fn clean_url(url:&str) -> &str { url.find(['?', '#']).map(|pos| (&url[..pos])).unwrap_or(url) }
...
pub fn get_extension(id:&str) -> &str { id.rsplit_once('.').map_or("", |(_, ext)| ext) }

// https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Regular_expressions/Character_class_escape#w
fn is_regex_w_character_class(c:char) -> bool { c.is_ascii_alphanumeric() || c == '_' }
...
pub fn is_in_node_modules(id:&str) -> bool { id.contains("node_modules") }
