...
impl AdditionalOptions {
	...
	fn as_u8(&self) -> u8 { bools_to_u8(self.as_bools()) }
}
...
impl From<u8> for AdditionalOptions {
	fn from(value:u8) -> Self { u8_to_bools(value).into() }
}
...
impl Resolvers {
	...
	pub fn get_for_external(&self) -> Arc<Resolver> { Arc::clone(&self.external_resolver) }
...
fn get_conditions(base_options:&BaseOptions, additional_options:&AdditionalOptions) -> Vec<String> {
...
impl Resolver {
	...
	pub fn normalize_oxc_resolver_result(
		...
		match result {
			...
			Err(oxc_resolver::ResolveError::Ignored(_)) => {
				Ok(Some(HookResolveIdOutput {
					id:BROWSER_EXTERNAL_ID.to_string(),
					..Default::default()
				}))
			},
	...
	fn get_nearest_package_json_optional_peer_deps(
		...
		let Ok(result) = self.inner.resolve(
			// actually this can be anything, as the specifier is absolute path
			&self.root,
	...
	pub fn resolve_bare_import(
		...
		let resolved =
			self.normalize_oxc_resolver_result(importer, dedupe, &oxc_resolved_result)?;
	...
	pub fn clear_cache(&self) { self.inner.clear_cache(); }
}
