...
impl Resolvers {
	pub fn new(base_options: &BaseOptions, external_conditions: &Vec<String>, runtime: String) -> Self {
...
impl Resolver {
	...
	pub fn resolve_raw<P: AsRef<Path>>(
		...
		match result_with_prefix {
			Err(oxc_resolver::ResolveError::NotFound(_) | oxc_resolver::ResolveError::ExtensionAlias(_, _, _)) => {
...
fn get_base_dir<'a>(specifier: &'_ str, importer: Option<&'a str>, dedupe: &FxHashSet<String>) -> Option<&'a str> {
