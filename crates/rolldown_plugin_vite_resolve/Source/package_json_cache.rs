...
impl PackageJsonCache {
	pub fn cached_package_json_side_effects(&self, oxc_pkg_json: &oxc_resolver::PackageJsonSerde) -> Arc<PackageJson> {
		match self.side_effects_cache.get(&oxc_pkg_json.realpath) {
			Some(v) => Arc::clone(v.value()),
			_ => {
				let pkg_json = Arc::new(
					PackageJson::new(oxc_pkg_json.path.clone()).with_side_effects(oxc_pkg_json.side_effects.as_ref()),
				);
				self.side_effects_cache
