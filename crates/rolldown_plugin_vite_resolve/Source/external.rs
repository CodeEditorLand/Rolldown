...
use crate::{
	resolver::Resolver,
	utils::{can_externalize_file, get_npm_package_name, is_bare_import, is_builtin, is_in_node_modules},
...
impl ExternalDecider {
	...
	fn is_configured_as_external(&self, id: &str, importer: Option<&str>) -> bool {
		...
		self.is_externalizable(id, importer, matches!(self.options.external, ResolveOptionsExternal::True))
	}

	fn is_externalizable(&self, id: &str, importer: Option<&str>, configured_as_external: bool) -> bool {
