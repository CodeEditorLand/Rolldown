...
impl ResolveOptionsNoExternal {
	pub fn new_true() -> Self { Self(ResolveOptionsNoExternalInner::True) }
	...
	pub fn is_true(&self) -> bool { matches!(self.0, ResolveOptionsNoExternalInner::True) }
...
impl ExternalDecider {
	...
	pub fn is_external(&self, id:&str, importer:Option<&str>) -> bool {
		...
		if !id.starts_with('.') && !Path::new(id).is_absolute() {
			is_external =
				is_builtin(id, &self.runtime) || self.is_configured_as_external(id, importer);
		}
