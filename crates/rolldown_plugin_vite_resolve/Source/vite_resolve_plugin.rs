...
use crate::{
	...
	utils::{
		BROWSER_EXTERNAL_ID, OPTIONAL_PEER_DEP_ID, clean_url, is_bare_import, is_builtin, is_in_node_modules,
...
use rolldown_plugin::{
	HookLoadArgs, HookLoadOutput, HookLoadReturn, HookNoopReturn, HookResolveIdArgs, HookResolveIdOutput,
	HookResolveIdReturn, Plugin, PluginContext, typedmap::TypedMapKey,
};
...
pub type FinalizeBareSpecifierCallback = dyn (Fn(&str, &str, Option<&str>) -> Pin<Box<(dyn Future<Output = anyhow::Result<Option<String>>> + Send + Sync)>>)
	+ Send
	+ Sync;

pub type FinalizeOtherSpecifiersCallback =
	dyn (Fn(&str, &str) -> Pin<Box<(dyn Future<Output = anyhow::Result<Option<String>>> + Send + Sync)>>) + Send + Sync;
...
impl ViteResolvePlugin {
	pub fn new(options: ViteResolveOptions) -> Self {
		...
		Self {
			...
			external_decider: ExternalDecider::new(
				ExternalDeciderOptions { external: options.external, no_external: Arc::clone(&no_external), dedupe },
				options.runtime,
	...
	async fn resolve_id_internal(&self, args: &HookResolveIdArgs<'_>) -> HookResolveIdReturn {
		let scan = args.custom.get(&ResolveIdOptionsScan {}).is_some_and(|v| *v) || self.resolve_options.scan;
      ...
      || args.specifier.starts_with("/virtual:")
		...
		if is_bare_import(args.specifier) {
			...
			let result = resolver.resolve_bare_import(args.specifier, args.importer, external, &self.dedupe)?;
              ...
              || !self.external.is_external_explicitly(args.specifier))
					{
						...
						if let Some(importer) = args.importer {
							let current_dir = env::current_dir().unwrap_or(PathBuf::from(&self.resolve_options.root));
...
fn is_external_url(id: &str) -> bool {
	if let Some(double_slash_pos) = id.find("//") {
		if double_slash_pos == 0 {
			...
			protocol
				.strip_suffix(':')
				.map(|p| p.bytes().all(|c| c.is_ascii_alphabetic()))
				.is_some()
		}
	} else {
		false
