...
impl ViteResolvePlugin {
	...
	fn name_internal(&self) -> &'static str { "rolldown:vite-resolve" }

	async fn resolve_id_internal(&self, args:&HookResolveIdArgs<'_>) -> HookResolveIdReturn {
		let scan = args.custom.get(&ResolveIdOptionsScan {}).map_or(false, |v| *v)
      ...
      || args.specifier.starts_with("/virtual:")
		...
		// data uri: pass through (this only happens during build and will be handled by
		// rolldown)
		...
		let additional_options = AdditionalOptions::new(
			...
			self.resolve_options.prefer_relative
				|| args.importer.map_or(false, |i| i.ends_with(".html")),
		...
		if is_bare_import(args.specifier) {
			...
			if let Some(mut result) = result {
				if let Some(finalize_bare_specifier) = &self.finalize_bare_specifier {
					if !scan && is_in_node_modules(&result.id) {
						let finalized =
							finalize_bare_specifier(&result.id, args.specifier, args.importer)
								.await?
								.unwrap_or(result.id);
			...
			if is_builtin(args.specifier, &self.runtime) {
              ...
              || !self.external.is_external_explicitly(args.specifier))
					{
						let mut message =
							format!("Cannot bundle Node.js built-in \"{}\"", args.specifier);

						if let Some(importer) = args.importer {
							let current_dir = env::current_dir()
								.unwrap_or(PathBuf::from(&self.resolve_options.root));
						...
						message.push_str(&format!(
							". Consider disabling environments.{}.noExternal or remove the \
							 built-in dependency.",
			...
			if !scan {
				if let Some(finalize_other_specifiers) = &self.finalize_other_specifiers {
					if let Some(finalized) =
						finalize_other_specifiers(&resolved.id, args.specifier).await?
					{
	...
	async fn load_internal(&self, args:&HookLoadArgs<'_>) -> HookLoadReturn {
		...
		if args.id.starts_with(OPTIONAL_PEER_DEP_ID) {
			if self.resolve_options.is_production {
				...
				let [_, peer_dep, parent_dep, _] =
					args.id.splitn(4, ":").collect::<Vec<&str>>()[..]
				else {
					unreachable!()
				};
	...
	fn watch_change_internal(&self, _path:&str, event:WatcherChangeKind) -> HookNoopReturn {
		// TODO(sapphi-red): we need to avoid using cache for files not watched by vite
		// or rollup https://github.com/vitejs/vite/issues/17760
		match event {
...
impl CallablePlugin for ViteResolvePlugin {
	fn name(&self) -> Cow<'static, str> { self.name_internal().into() }
