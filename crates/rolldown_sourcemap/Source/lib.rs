...
pub fn collapse_sourcemaps(mut sourcemap_chain: Vec<&SourceMap>) -> SourceMap {
	...
	let names_map = FxHashMap::from_iter(first_map.get_names().enumerate().map(|(i, name)| (name, i as u32)));

	let sources_map = FxHashMap::from_iter(first_map.get_sources().enumerate().map(|(i, source)| (source, i as u32)));

	let tokens = source_view_tokens
		.par_iter()
		.filter_map(|token| {
			let original_token =
				sourcemap_and_lookup_table
					.iter()
					.rev()
...
fn test_collapse_sourcemaps() {
	...
	let CodegenReturn { map, code, .. } = CodeGenerator::new()
		.with_options(CodegenOptions { source_map_path: Some(filename.into()), ..CodegenOptions::default() })
		.build(&ret1.program);
