...
impl NormalModule {
	...
	pub fn has_star_export(&self) -> bool { self.ecma_view.meta.has_star_export() }
	...
	pub fn get_exported_names<'modules>(
		...
		if include_default {
			...
			ret.extend(
				self.ecma_view.named_exports.keys().filter(|name| name.as_str() != "default"),
			);
		}
	}
	...
	pub fn render(
		...
		match args {
			ModuleRenderArgs::Ecma { ast } => {
				...
				// If here no extra sourcemap need remapping, we using it as final module
				// sourcemap. So here make sure using correct `source_name` and
				// `source_content.
				...
				if !self.ecma_view.mutations.is_empty() {
					...
					let map = render_output
						.map
						.map(|original| collapse_sourcemaps(vec![&original, &mutated_map]));
	...
	pub fn is_included(&self) -> bool { self.ecma_view.meta.is_included() }
}
...
impl Deref for NormalModule {
	...
	fn deref(&self) -> &Self::Target { &self.ecma_view }
}

impl DerefMut for NormalModule {
	fn deref_mut(&mut self) -> &mut Self::Target { &mut self.ecma_view }
}
