...
impl PackageJson {
	pub fn new(path:PathBuf) -> Self { Self { path, r#type:None, side_effects:None } }
	...
	pub fn r#type(&self) -> Option<&str> { self.r#type.as_deref() }
	...
	pub fn check_side_effects_for(&self, module_path:&str) -> Option<bool> {
		...
		match side_effects {
			...
			SideEffects::String(p) => {
				Some(glob_match_with_normalized_pattern(p.as_str(), module_path))
			},
			SideEffects::Array(pats) => {
				Some(
					pats.iter()
						.any(|p| glob_match_with_normalized_pattern(p.as_str(), module_path)),
				)
			},
		}
	}
