...
impl StmtInfos {
	pub fn get(&self, id:StmtInfoIdx) -> &StmtInfo { &self.infos[id] }

	pub fn get_mut(&mut self, id:StmtInfoIdx) -> &mut StmtInfo { &mut self.infos[id] }
	...
	/// # Panic
	/// Caller should guarantee the stmt is included in `stmts` before, or it
	/// will panic.
...
impl std::ops::Deref for StmtInfos {
	...
	fn deref(&self) -> &Self::Target { &self.infos }
}

impl std::ops::DerefMut for StmtInfos {
	fn deref_mut(&mut self) -> &mut Self::Target { &mut self.infos }
}
...
pub struct StmtInfo {
	...
	///
	/// We will create some facade statements while bundling, and the facade
	/// statements don't have a corresponding statement in the original module
	/// body, which means `stmt_idx` will be `None`.
	pub stmt_idx:Option<usize>,
	// currently, we only store top level symbols
	pub declared_symbols:Vec<SymbolRef>,
	// We will add symbols of other modules to `referenced_symbols`, so we need
	// `SymbolRef` here instead of `SymbolId`.
