...
impl ImportKind {
	pub fn is_static(&self) -> bool { matches!(self, Self::Import | Self::Require) }
}
...
pub struct RawImportRecord {
	...
	/// Why use start_offset instead of `Span`? Cause, directly pass `Span`
	/// will increase the type size from `40` to `48`(8 bytes alignment).
	/// Since the `RawImportRecord` will be created multiple time, Using this
	/// trick could save some memory.
...
pub struct ImportRecord {
	...
	pub kind:ImportKind,
	/// We will turn `import { foo } from './cjs.js'; console.log(foo);` to
	/// `var import_foo = require_cjs(); console.log(importcjs.foo)`;
	/// `namespace_ref` represent the potential `import_foo` in above example.
	/// It's useless if we imported n esm module.
	pub namespace_ref:SymbolRef,
	pub meta:ImportRecordMeta,
}
