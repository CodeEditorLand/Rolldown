...
pub struct ImportRecordStateInit {
	pub span:Span,
	/// The importee of this import record is asserted to be this specific
	/// module type.
...
pub struct ImportRecord<State:Debug> {
	...
	pub kind:ImportKind,
	/// We will turn `import { foo } from './cjs.js'; console.log(foo);` to `var
	/// import_foo = require_cjs(); console.log(importcjs.foo)`;
...
impl<State:Debug> ImportRecord<State> {
	pub fn is_unspanned(&self) -> bool { self.meta.contains(ImportRecordMeta::IS_UNSPANNED_IMPORT) }
}
