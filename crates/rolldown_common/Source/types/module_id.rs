...
/// - Users could stored the `ModuleId` to track the module in different
///   stages/hooks.
...
impl ModuleId {
	pub fn new(value:impl Into<ArcStr>) -> Self { Self { resource_id:value.into() } }

	pub fn resource_id(&self) -> &ArcStr { &self.resource_id }

	pub fn stabilize(&self, cwd:&Path) -> String { stabilize_module_id(&self.resource_id, cwd) }
}

impl AsRef<str> for ModuleId {
	fn as_ref(&self) -> &str { &self.resource_id }
}

impl std::ops::Deref for ModuleId {
	type Target = str;

	fn deref(&self) -> &Self::Target { &self.resource_id }
}
...
impl From<ArcStr> for ModuleId {
	fn from(value:ArcStr) -> Self { Self::new(value) }
}
