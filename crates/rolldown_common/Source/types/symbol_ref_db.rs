...
use oxc::{
	semantic::{NodeId, ScopeId, SymbolFlags, SymbolId, SymbolTable},
...
pub struct SymbolRefDataClassic {
	/// For case `import {a} from 'foo.cjs';console.log(a)`, the symbol `a`
	/// reference to `module.exports.a` of `foo.cjs`. So we will transform the
	/// code into `console.log(foo_ns.a)`. `foo_ns` is the namespace symbol of
	/// `foo.cjs and `a` is the property name. We use `namespace_alias` to
	/// represent this situation. If `namespace_alias` is not `None`, then this
	/// symbol must be rewritten to a property access.
...
impl SymbolRefDbForModule {
	...
	/// This method is used to hide the `SymbolTable::create_symbol` method
	/// since `SymbolRefDbForModule` impl `Deref` for `SymbolTable`.
...
impl Deref for SymbolRefDbForModule {
	...
	fn deref(&self) -> &Self::Target { &self.symbol_table }
}

impl DerefMut for SymbolRefDbForModule {
	fn deref_mut(&mut self) -> &mut Self::Target { &mut self.symbol_table }
}
...
impl GetLocalDb for SymbolRefDb {
	fn local_db(&self, owner:ModuleIdx) -> &SymbolRefDbForModule { self.inner[owner].unpack_ref() }
}
