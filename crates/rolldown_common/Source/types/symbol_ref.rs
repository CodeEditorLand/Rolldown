...
/// `SymbolRef` is used to represent a symbol in a module when there are
/// multiple modules.
...
impl From<(ModuleIdx, SymbolId)> for SymbolRef {
	fn from(value:(ModuleIdx, SymbolId)) -> Self { Self { owner:value.0, symbol:value.1 } }
}
...
impl SymbolRef {
	...
	pub fn set_name(&self, db:&mut SymbolRefDb, name:&str) {
		db.inner[self.owner]
			.unpack_ref_mut()
			.set_name(self.symbol, CompactStr::new(name));
	}
	...
	pub fn is_declared_by_const(&self, db:&SymbolRefDb) -> Option<bool> {
		...
		// Not having this flag means we don't know if it's declared by `const` instead
		// of it's not declared by `const`.
	...
	#[must_use]
	pub fn canonical_ref(&self, db:&SymbolRefDb) -> SymbolRef { db.canonical_ref_for(*self) }
	...
	pub fn is_created_by_import_stmt_that_target_external(
		...
		let Module::Normal(owner) = &modules[canonical_ref.owner] else {
			return false;
		};
		...
		match &modules[rec.resolved_module] {
			Module::Normal(_) => {
				// This branch should be unreachable. By `par_canonical_ref_for`, we should get
				// the canonical ref. An canonical ref is either declared by the module
				// itself or a `import { foo } from 'bar'` statement.
				false
			},
