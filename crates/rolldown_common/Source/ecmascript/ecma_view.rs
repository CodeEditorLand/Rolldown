...
impl EcmaViewMeta {
	#[inline]
	pub fn has_eval(&self) -> bool { self.contains(Self::EVAL) }

	#[inline]
	pub fn is_included(&self) -> bool { self.contains(Self::INCLUDED) }

	#[inline]
	pub fn has_lazy_export(&self) -> bool { self.contains(Self::HAS_LAZY_EXPORT) }

	#[inline]
	pub fn has_star_export(&self) -> bool { self.contains(Self::HAS_STAR_EXPORT) }
...
impl SourceMutation for ImportMetaRolldownAssetReplacer {
	fn apply(&self, magic_string:&mut string_wizard::MagicString<'_>) {
		magic_string.replace_all(
			"import.meta.__ROLLDOWN_ASSET_FILENAME",
			format!("\"{}\"", self.asset_filename),
		);
	}
}
