...
impl Chunk {
	...
	pub fn has_side_effect(&self, runtime_id:ModuleIdx) -> bool {
		// TODO: remove this special case, once `NormalModule#side_effect` is
		// implemented. Runtime module should always not have side effect
		if self.modules.len() == 1 && self.modules[0] == runtime_id {
			return false;
		}
		// TODO: Wether a chunk has side effect is determined by wether it's module has
		// side effect Now we just return `true`
		true
	}
	...
	pub async fn generate_css_preliminary_filename(
		...
		let filename_template =
			self.css_filename_template(options, rollup_pre_rendered_chunk).await?;
	...
	pub fn user_defined_entry_module_idx(&self) -> Option<ModuleIdx> {
		match &self.kind {
			ChunkKind::EntryPoint { module, is_user_defined, .. } if *is_user_defined => {
				Some(*module)
			},
			_ => None,
		}
	...
	pub fn user_defined_entry_module<'module>(
		...
		self.user_defined_entry_module_idx()
			.and_then(|idx| module_table.modules[idx].as_normal())
	}
