...
use types::{
	...
	output_option::GlobalsOutputOption,
	target::ESTarget,
...
use self::types::{
	...
	output_option::AddonOutputOption,
	platform::Platform,
...
pub struct BundlerOptions {
	...
	/// Key is the file extension. The extension should start with a `.`. E.g.
	/// `".txt"`.
					...
					match v {
						...
						_ => {
							Err(serde::de::Error::custom(
								"moduleSideEffects should be a `true` or `false`",
							))
						},
					}
				},
			...
			let annotations = obj.get("annotations").map_or_else(
				...
				|v| {
					match v {
						...
						_ => {
							Err(serde::de::Error::custom(
								"annotations should be a `true` or `false`",
							))
						},
					}
				},
				...
				match k.as_str() {
					"runtime" => {
						let runtime = v.as_str().ok_or_else(|| {
							serde::de::Error::custom("jsx.pragma should be a string")
						})?;
					...
					"pragma" => {
						let pragma = v.as_str().ok_or_else(|| {
							serde::de::Error::custom("jsx.pragma should be a string")
						})?;
