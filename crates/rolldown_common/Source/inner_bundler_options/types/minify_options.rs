...
impl From<RawMinifyOptions> for MinifyOptions {
	fn from(value: RawMinifyOptions) -> Self {
		match value {
			RawMinifyOptions::Bool(value) => {
				if value {
					Self::Enabled(MinifyOptionsObject { mangle: true, compress: true, remove_whitespace: true })
				} else {
					Self::Disabled
				}
			},
			RawMinifyOptions::DeadCodeEliminationOnly => {
				Self::Enabled(MinifyOptionsObject { mangle: false, compress: false, remove_whitespace: false })
			},
...
impl MinifyOptionsObject {
	pub fn to_oxc_minifier_options(&self, option: &SharedNormalizedBundlerOptions) -> oxc::minifier::MinifierOptions {
