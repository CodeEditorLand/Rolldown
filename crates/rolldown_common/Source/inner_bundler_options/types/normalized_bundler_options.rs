//! [crate::InputOptions] meant to provide dx-friendly options for the
...
use super::{
	advanced_chunks_options::AdvancedChunksOptions,
	...
	output_option::{AddonOutputOption, ChunkFilenamesOutputOption},
	platform::Platform,
...
pub struct NormalizedBundlerOptions {
	...
	/// The key is the extension. Unlike `BundlerOptions`, the extension doesn't
	/// start with a dot.
...
impl NormalizedBundlerOptions {
	pub fn is_sourcemap_enabled(&self) -> bool { self.sourcemap.is_some() }
	...
	/// make sure the `polyfill_require` is only valid for `esm` format with
	/// `node` platform
	#[inline]
