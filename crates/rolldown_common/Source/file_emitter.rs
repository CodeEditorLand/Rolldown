use crate::{
	AssetSource, FileNameRenderOptions, NormalizedBundlerOptions, Output,
	OutputAsset,
};
use arcstr::ArcStr;
...
impl FileEmitter {
	...
	pub fn emit_file(&self, mut file: EmittedAsset) -> ArcStr {
		...
		if file.file_name.is_none() {
			if let Some(reference_id) =
				self.source_hash_to_reference_id.get(&hash)
			{
		...
		if file.file_name.is_none() {
			self.source_hash_to_reference_id
				.insert(hash.clone(), reference_id.clone());
		}
	...
	pub fn try_get_file_name(
		...
		let file = self.files.get(reference_id).ok_or(format!(
			"Unable to get file name for unknown file: {reference_id}"
		))?;
		file.file_name
			.clone()
			.ok_or(format!("{reference_id} should have file name"))
	}

	pub fn get_file_name(&self, reference_id: &str) -> ArcStr {
		self.try_get_file_name(reference_id)
	...
	pub fn generate_file_name(&self, file: &mut EmittedAsset, hash: &ArcStr) {
		if file.file_name.is_none() {
			...
			let extension =
				path.and_then(|x| x.extension().and_then(OsStr::to_str));
			let name = path
				.and_then(|x| x.file_stem().and_then(OsStr::to_str))
				.map(|x| sanitize_file_name(x.into()));
			let extract_hash_pattern =
				extract_hash_pattern(self.options.asset_filenames.template());
			let mut file_name: ArcStr = self
				.options
				.asset_filenames
				.render(&FileNameRenderOptions {
					name: name.as_deref(),
					hash: extract_hash_pattern.map(|p| {
						&hash.as_str()
							[..p.len.map_or(8, |hash_len| hash_len.max(6))]
					}),
					ext: extension,
				})
				.into();
			...
			if let Some(count) =
				self.names.get_mut(file_name.as_str()).as_deref_mut()
			{
				*count += 1;
				let extension =
	...
	pub fn add_additional_files(&self, bundle: &mut Vec<Output>) {
		self.files.iter_mut().for_each(|mut file| {
			...
			bundle.push(Output::Asset(Box::new(OutputAsset {
				...
				original_file_name: std::mem::take(
					&mut value.original_file_name,
				),
			})));
		});
	}
