use crate::{
	AddEntryModuleMsg, FilenameTemplate, ModuleLoaderMsg, NormalizedBundlerOptions, Output, OutputAsset, StrOrBytes,
};
use anyhow::Context;
use arcstr::ArcStr;
...
impl FileEmitter {
	...
	pub fn emit_file(
		...
		let hash: ArcStr = xxhash_with_base(file.source.as_bytes(), self.options.hash_characters.base()).into();
	...
	pub fn generate_file_name(
		...
		if file.file_name.is_none() {
			...
			let filename_template = filename_template.expect("should has filename template without filename");
	...
	pub fn add_additional_files(&self, bundle: &mut Vec<Output>, warnings: &mut Vec<BuildDiagnostic>) {
		self.files.iter_mut().for_each(|mut file| {
			...
			if !self.emitted_filenames.insert(lowercase_filename) {
				warnings.push(BuildDiagnostic::filename_conflict(value.filename.clone()).with_severity_warning());
			}
	...
	pub async fn set_context_load_modules_tx(&self, tx: Option<tokio::sync::mpsc::Sender<ModuleLoaderMsg>>) {
