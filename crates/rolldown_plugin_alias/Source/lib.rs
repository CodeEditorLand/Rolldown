...
pub struct AliasPlugin {
	// We don't support `customResolver` and `resolverFunction`, it will generate many
	// threadSafeFunction in queue, and slowdown the performance, downstream user should fallback
	// to js alias plugin when needs advance feature.
...
impl Plugin for AliasPlugin {
	fn name(&self) -> Cow<'static, str> { Cow::Borrowed("builtin:alias") }
