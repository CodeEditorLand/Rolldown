...
use rolldown_plugin::{
	HookResolveIdOutput, Plugin, PluginContext, PluginContextResolveOptions,
};
...
impl Plugin for AliasPlugin {
	...
	async fn resolve_id(
		...
		let match_entry = self
			.entries
			.iter()
			.find(|alias| Self::matches(&alias.find, importee));
		let Some(match_entry) = match_entry else {
			return Ok(None);
		};

		let update_id = match &match_entry.find {
			StringOrRegex::String(find) => {
				importee.replace(find, &match_entry.replacement)
			},
			StringOrRegex::Regex(find) => {
				find.replace_all(importee, &match_entry.replacement)
			},
		};
		Ok(ctx
			.resolve(
