...
impl Plugin for TransformPlugin {
	...
	async fn transform(
		...
		let CodegenReturn { mut code, map, .. } = CodeGenerator::new()
			.with_options(CodegenOptions { source_map_path: Some(args.id.into()), ..CodegenOptions::default() })
...
impl TransformPlugin {
	fn filter(&self, ctx: &SharedTransformPluginContext, id: &str, module_type: &ModuleType) -> bool {
		...
		let result = pattern_filter(Some(&self.exclude), Some(&self.include), id, &normalized_id).inner();
