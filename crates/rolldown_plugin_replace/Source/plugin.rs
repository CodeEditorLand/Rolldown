...
fn is_variable_declaration_prefix(s: &str) -> bool {
	...
	(s.ends_with("const") && (s.len() == 5 || !s.chars().nth(s.len() - 6).unwrap_or(' ').is_alphanumeric()))
		|| (s.ends_with("let") && (s.len() == 3 || !s.chars().nth(s.len() - 4).unwrap_or(' ').is_alphanumeric()))
		|| (s.ends_with("var") && (s.len() == 3 || !s.chars().nth(s.len() - 4).unwrap_or(' ').is_alphanumeric()))
}
...
impl ReplacePlugin {
	...
	fn try_replace<'text>(&'text self, code: &'text str, magic_string: &mut MagicString<'text>) -> bool {
		match self.matcher {
