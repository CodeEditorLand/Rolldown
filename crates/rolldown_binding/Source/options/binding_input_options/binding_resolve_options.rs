...
use crate::types::{
	binding_resolve_alias_item::AliasItem,
	binding_resolve_extension_alias::ExtensionAliasItem,
};
...
pub struct BindingResolveOptions {
	// Option<Vec<(String, Vec<String>)>>> is better, maybe NAPI-RS should
	// support tuples.
	...
	fn from(value:BindingResolveOptions) -> Self {
		Self {
			...
			extension_alias:value.extension_alias.map(|alias| {
				alias
					.into_iter()
					.map(|item| (item.target, item.replacements))
					.collect::<Vec<_>>()
			}),
