...
use rolldown_plugin_vite_resolve::{
	FinalizeBareSpecifierCallback, FinalizeOtherSpecifiersCallback, ViteResolveOptions, ViteResolvePlugin,
...
pub struct BindingViteResolvePluginConfig {
	...
	pub finalize_bare_specifier: Option<JsCallback<FnArgs<(String, String, Option<String>)>, Option<String>>>,
...
impl From<BindingViteResolvePluginConfig> for ViteResolveOptions {
	fn from(value: BindingViteResolvePluginConfig) -> Self {
		...
		let no_external = match value.no_external {
			...
			napi::Either::B(v) => {
				rolldown_plugin_vite_resolve::ResolveOptionsNoExternal::new_vec(bindingify_string_or_regex_array(v))
			},
		};
