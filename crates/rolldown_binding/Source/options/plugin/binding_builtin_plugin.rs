...
use napi::{JsUnknown, bindgen_prelude::FromNapiValue};
...
use rolldown_plugin_vite_resolve::{
	FinalizeBareSpecifierCallback,
	FinalizeOtherSpecifiersCallback,
	ViteResolveOptions,
	ViteResolvePlugin,
...
use super::types::{
	...
	binding_js_or_regex::{BindingStringOrRegex, bindingify_string_or_regex_array},
...
impl TryFrom<BindingJsonPluginStringify> for JsonPluginStringify {
	...
	fn try_from(value:BindingJsonPluginStringify) -> Result<Self, Self::Error> {
		Ok(match value {
			...
			BindingJsonPluginStringify(napi::Either::B(s)) if s == "auto" => {
				JsonPluginStringify::Auto
			},
...
pub struct BindingViteResolvePluginConfig {
	...
	#[napi(
		ts_type = "(resolvedId: string, rawId: string, importer: string | null | undefined) => \
		           VoidNullable<string>"
	)]
	pub finalize_bare_specifier:
		Option<JsCallback<(String, String, Option<String>), Option<String>>>,
...
impl TryFrom<BindingViteResolvePluginConfig> for ViteResolveOptions {
	...
	fn try_from(value:BindingViteResolvePluginConfig) -> Result<Self, Self::Error> {
		...
		let no_external = match value.no_external {
			napi::Either::A(_) => {
				rolldown_plugin_vite_resolve::ResolveOptionsNoExternal::new_true()
			},
			napi::Either::B(v) => {
				rolldown_plugin_vite_resolve::ResolveOptionsNoExternal::new_vec(
			...
			finalize_other_specifiers:value.finalize_other_specifiers.map(
				|finalizer_fn| -> Arc<FinalizeOtherSpecifiersCallback> {
					Arc::new(move |resolved_id:&str, raw_id:&str| {
						...
						Box::pin(async move {
							finalizer_fn
								.invoke_async((resolved_id, raw_id))
								.await
								.map_err(anyhow::Error::from)
						})
					})
	...
	fn try_from(plugin:BindingBuiltinPlugin) -> Result<Self, Self::Error> {
		Ok(match plugin.__name {
			...
			BindingBuiltinPluginName::BuildImportAnalysis => {
				let config:BindingBuildImportAnalysisPluginConfig =
			...
			BindingBuiltinPluginName::Replace => {
				...
				Arc::new(ReplacePlugin::with_options(config.map_or_else(
