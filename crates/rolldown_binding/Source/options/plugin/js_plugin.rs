...
impl Plugin for JsPlugin {
	...
	async fn build_start(
		...
		if let Some(cb) = &self.build_start {
			cb.await_call((ctx.clone().into(), BindingNormalizedOptions::new(Arc::clone(args.options))).into())
				.await?;
		}
		Ok(())
	}
	...
	async fn banner(
		...
		match &self.banner {
			Some(cb) => Ok(cb
				.await_call((ctx.clone().into(), BindingRenderedChunk::new(Arc::clone(&args.chunk))).into())
				.await?
				.map(TryInto::try_into)
				.transpose()?),
			_ => Ok(None),
	...
	async fn augment_chunk_hash(
		...
		match &self.augment_chunk_hash {
			Some(cb) => Ok(cb
				.await_call((ctx.clone().into(), BindingRenderedChunk::new(chunk)).into())
				.await?),
			_ => Ok(None),
		}
	}
	...
	async fn close_bundle(&self, ctx: &rolldown_plugin::PluginContext) -> rolldown_plugin::HookNoopReturn {
	...
	async fn close_watcher(&self, ctx: &rolldown_plugin::PluginContext) -> rolldown_plugin::HookNoopReturn {
...
fn filter_transform(
	...
	if let Some(ref code_filter) = transform_filter.code {
		let code_res =
			pattern_filter::filter_code(code_filter.exclude.as_deref(), code_filter.include.as_deref(), code);
	...
	fn test_filter() {
		...
		let cases = [
			...
			TestCases {
				...
				input_code_filter: Some(InputFilter { exclude: None, include: string_filter("import.meta") }),
			...
			TestCases {
				input_id_filter: Some(InputFilter { exclude: string_filter("*.js"), include: string_filter("foo.ts") }),
				input_code_filter: Some(InputFilter { exclude: None, include: string_filter("import.meta") }),
				cases: vec![
					("foo.js", "import.meta", false),
					("foo.js", "import_meta", false),
