...
impl JsPlugin {
	...
	pub(super) fn new(inner:BindingPluginOptions) -> Self { Self { inner } }
...
impl Plugin for JsPlugin {
	fn name(&self) -> Cow<'static, str> { Cow::Owned(self.name.clone()) }
	...
	async fn load(
		...
		if let Some(cb) = &self.load {
			Ok(cb
				.await_call((ctx.clone().into(), args.id.to_string()))
				.await?
	...
	async fn banner(
		...
		if let Some(cb) = &self.banner {
			Ok(cb
				.await_call((ctx.clone().into(), args.chunk.clone().into()))
				.await?
				.map(TryInto::try_into)
				.transpose()?)
		} else {
	...
	async fn render_chunk(
		...
		if let Some(cb) = &self.render_chunk {
			Ok(cb
				.await_call((ctx.clone().into(), args.code.to_string(), args.chunk.clone().into()))
				.await?
				.map(TryInto::try_into)
				.transpose()?)
		} else {
	...
	async fn generate_bundle(
		...
		if let Some(cb) = &self.generate_bundle {
			...
			cb.await_call((
				ctx.clone().into(),
				BindingOutputs::new(old_bundle.weak_ref()),
				is_write,
			))
			.await?;
	...
	async fn write_bundle(
		...
		if let Some(cb) = &self.write_bundle {
			let old_bundle = UniqueArc::new(Mutex::new(mem::take(bundle)));
			cb.await_call((ctx.clone().into(), BindingOutputs::new(old_bundle.weak_ref())))
				.await?;
			*bundle = old_bundle.into_inner().into_inner()?;
		}
		Ok(())
