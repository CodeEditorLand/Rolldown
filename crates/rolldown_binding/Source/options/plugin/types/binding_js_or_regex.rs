...
pub struct BindingStringOrRegex {
	pub value:String,
	/// There is a more compact way to represent this, `Option<u8>` with
	/// bitflags, but it will be hard to use(in js side), since construct a
	/// `JsRegex` is not used frequently. Optimize it when it is needed.
