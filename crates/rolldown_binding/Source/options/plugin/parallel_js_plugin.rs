...
impl ParallelJsPlugin {
	pub fn new_boxed(plugins: Vec<BindingPluginOptions>, worker_manager: Arc<WorkerManager>) -> Box<dyn Pluginable> {
		let plugins = plugins.into_iter().map(JsPlugin::new).collect::<Vec<_>>().into_boxed_slice();
		Box::new(Self { plugins, worker_manager })
	}

	pub fn new_shared(plugins: Vec<BindingPluginOptions>, worker_manager: Arc<WorkerManager>) -> Arc<dyn Pluginable> {
		let plugins = plugins.into_iter().map(JsPlugin::new).collect::<Vec<_>>().into_boxed_slice();
	...
	#[cfg(not(target_family = "wasm"))]
	async fn run_all<'a, R, E: std::fmt::Debug, F: FnMut(&'a JsPlugin) -> BoxFuture<'a, Result<R, E>>>(
		&'a self,
		f: F,
	) -> Result<Vec<R>, E> {
