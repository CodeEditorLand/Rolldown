...
impl ParallelJsPlugin {
	...
	fn first_plugin(&self) -> &JsPlugin { &self.plugins[0] }
...
impl Plugin for ParallelJsPlugin {
	fn name(&self) -> Cow<'static, str> { self.first_plugin().call_name() }
	...
	async fn generate_bundle(
		...
		if self.first_plugin().generate_bundle.is_some() {
			self.run_single(|plugin| plugin.call_generate_bundle(ctx, bundle, is_write))
				.await
		} else {
			Ok(())
		}
