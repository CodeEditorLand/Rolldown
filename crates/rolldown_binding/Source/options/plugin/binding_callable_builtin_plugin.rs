...
use rolldown_plugin::{CustomField, HookLoadArgs, HookLoadOutput, HookResolveIdArgs, HookResolveIdOutput};
use rolldown_plugin_vite_resolve::{CallablePluginAsyncTrait, ResolveIdOptionsScan, ViteResolvePlugin};

use super::{
	binding_builtin_plugin::{BindingBuiltinPlugin, BindingViteResolvePluginConfig},
	types::{
		binding_builtin_plugin_name::BindingBuiltinPluginName, binding_resolved_external::BindingResolvedExternal,
	},
};
...
impl BindingCallableBuiltinPlugin {
	...
	#[napi]
	pub async fn watch_change(&self, path: String, event: BindingJsWatchChangeEvent) -> napi::Result<()> {
		self.inner
			.watch_change(&path, bindingify_watcher_change_kind(event.event)?)
			.await?;
		Ok(())
	}
}
