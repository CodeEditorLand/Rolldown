...
impl BindingPluginContext {
	#[napi(
		ts_args_type = "specifier: string, sideEffects: BindingHookSideEffects | undefined, fn: \
		                () => void"
	)]
	pub async fn load(
		&self,
		...
		self.inner
			.load(
				...
				Box::new(move || {
					...
					Box::pin(async move {
						load_callback_fn.invoke_async(()).await.map_err(anyhow::Error::from)
					})
				}),
			)
			.await
	...
	pub async fn resolve(
		...
		Ok(ret.map(|info| {
			BindingPluginContextResolvedId { id:info.id.to_string(), external:info.is_external }
		}))
	}
	...
	#[napi]
	pub fn get_module_ids(&self) -> Vec<String> { self.inner.get_module_ids() }

	#[napi]
	pub fn add_watch_file(&self, file:String) { self.inner.add_watch_file(&file); }
}

impl From<PluginContext> for BindingPluginContext {
	fn from(inner:PluginContext) -> Self { Self { inner } }
}
#[napi(object)]
