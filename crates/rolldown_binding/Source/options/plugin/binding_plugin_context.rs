...
impl BindingPluginContext {
	#[napi(ts_args_type = "specifier: string, sideEffects: BindingHookSideEffects | undefined, fn: () => void")]
	pub async fn load(
		&self,
		specifier: String,
		...
		self.inner
			.load(
				...
				Some(Box::new(move || {
					...
					Box::pin(async move { load_callback_fn.invoke_async(()).await.map_err(anyhow::Error::from) })
				})),
			)
			.await
	...
	pub async fn resolve(
		...
		let ret = self
			...
			.ok();
		Ok(ret.map(|info| BindingPluginContextResolvedId { id: info.id.to_string(), external: info.external.into() }))
	}
	...
	pub fn emit_chunk(&self, file: BindingEmittedChunk) -> anyhow::Result<String> {
		futures::executor::block_on(async { self.inner.emit_chunk(file.into()).await.map(|id| id.to_string()) })
	}
