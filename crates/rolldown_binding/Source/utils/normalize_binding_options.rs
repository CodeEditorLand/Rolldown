...
use rolldown_utils::{indexmap::FxIndexMap, rustc_hash::FxHashMapExt};
...
use crate::{
	options::{ChunkFileNamesOutputOption, binding_inject_import::normalize_binding_inject_import},
	...
	option
		.map(move |value| {
			match value {
				...
				Either::B(func) => {
					Ok(ChunkFilenamesOutputOption::Fn(Arc::new(move |chunk| {
						let func = Arc::clone(&func);

						let chunk = chunk.clone();

						Box::pin(async move {
							func.invoke_async(chunk.into()).await.map_err(anyhow::Error::from)
						})
					})))
				},
			}
...
fn normalize_globals_option(
	...
	option.map(move |value| {
		match value {
			...
			Either::B(func) => {
				rolldown_common::GlobalsOutputOption::Fn(Arc::new(move |name| {
					let func = Arc::clone(&func);
					let name = name.to_string();
					Box::pin(
						async move { func.invoke_async(name).await.map_err(anyhow::Error::from) },
					)
				}))
			},
		}
...
pub fn normalize_binding_options(
	...
	let bundler_options = BundlerOptions {
		...
		es_module:output_options.es_module.map(|es_module| {
			match es_module {
		...
		exports:output_options.exports.map(|format_str| {
		...
		format:output_options.format.map(|format_str| {
			match format_str.as_str() {
				"es" => OutputFormat::Esm,
		...
		hash_characters:output_options.hash_characters.map(|format_str| {
			match format_str.as_str() {
		...
		module_types,
		experimental:input_options.experimental.map(|inner| {
			ExperimentalOptions {
		...
		advanced_chunks:output_options.advanced_chunks.map(|inner| {
			AdvancedChunksOptions {
						...
						.map(|item| {
							MatchGroup {
								...
								test:item.test.map(|inner| {
									inner.try_into().expect("Invalid regex pass to test")
								}),
				...
				|plugin| {
					match plugin {
						...
						Either::B(builtin) => {
							...
							builtin.try_into().unwrap_or_else(|err| {
								panic!("Should convert to builtin plugin: {name} \n {err}")
							})
						},
					}
				},
