...
use rolldown_utils::{indexmap::FxIndexMap, js_regex::HybridRegex};
...
use crate::{
	options::{binding_inject_import::normalize_binding_inject_import, ChunkFileNamesOutputOption},
						...
						.map(|item| {
							MatchGroup {
								...
								test:item.test.map(|inner| {
									HybridRegex::new(&inner).expect("Invalid regex pass to test")
								}),
			...
			plugin.map_or_else(
				...
				|plugin| {
					match plugin {
						...
						Either::B(builtin) => {
							// Needs to save the name, since `try_into` will
		...
		.filter_map(|plugin| {
			plugin.map(|plugin| {
				match plugin {
					Either::A(plugin_options) => JsPlugin::new_shared(plugin_options),
					Either::B(builtin) => {
						// Needs to save the name, since `try_into` will consume
						// the ownership
						let name = format!("{:?}", builtin.__name);
						builtin.try_into().unwrap_or_else(|err| {
							panic!("Should convert to builtin plugin: {name} \n {err}")
