...
impl WorkerManager {
	pub fn new(worker_count: u16) -> Self {
		...
		(0..worker_count)
			.for_each(|value| sender.send_blocking(value).unwrap());

		Self {
			free_workers_sender: sender,
			free_workers_receiver: receiver,
			worker_count,
		}
	}
...
impl Drop for WorkerSemaphorePermit {
	fn drop(&mut self) {
		...
		self.sender
			.send_blocking(worker_index)
			.expect("failed to send worker_index");
	}
}
...
impl Drop for WorkerAllSemaphorePermit {
	fn drop(&mut self) {
		...
		(0..worker_count).for_each(|value| {
			self.sender.send_blocking(value).expect(
				"failed to send worker_index while sending all of them",
			);
		});
	}
}
