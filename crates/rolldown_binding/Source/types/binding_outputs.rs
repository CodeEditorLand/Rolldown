...
// The `BindingOutputs` take the data to js side, the rust side will not use it
// anymore.
#[napi]
...
impl BindingOutputs {
	#[napi(getter)]
	pub fn chunks(&mut self) -> Vec<BindingOutputChunk> { std::mem::take(&mut self.chunks) }

	#[napi(getter)]
	pub fn assets(&mut self) -> Vec<BindingOutputAsset> { std::mem::take(&mut self.assets) }
