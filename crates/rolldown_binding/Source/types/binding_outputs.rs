...
/// The `BindingOutputs` owner `Vec<Output>` the mutable reference, it avoid
/// `Clone` at call `writeBundle/generateBundle` hook, and make it mutable.
#[napi]
...
#[napi]
impl BindingOutputs {
	pub fn new(inner:WeakRefMutex<Vec<rolldown_common::Output>>) -> Self { Self { inner } }
			...
			inner.iter_mut().for_each(|o| {
				match o {
					rolldown_common::Output::Chunk(chunk) => {
						chunks.push(BindingOutputChunk::new(unsafe {
							std::mem::transmute(chunk.as_mut())
						}));
					},
				...
				match o {
					rolldown_common::Output::Asset(asset) => {
						assets.push(BindingOutputAsset::new(unsafe {
							std::mem::transmute(asset.as_mut())
						}));
					},
...
/// The `FinalBindingOutputs` is used at `write()` or `generate()`, it is
/// similar to `BindingOutputs`, if using `BindingOutputs` has unexpected
/// behavior. TODO find a way to export it gracefully.
#[napi]
