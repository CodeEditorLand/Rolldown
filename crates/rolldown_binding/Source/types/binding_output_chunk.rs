...
// Here using `napi` `getter` fields to avoid the cost of serialize larger data
// to js side.
...
impl BindingOutputChunk {
	pub fn new(inner:rolldown_common::OutputChunk) -> Self { Self { inner } }

	#[napi(getter)]
	pub fn is_entry(&self) -> bool { self.inner.is_entry }

	#[napi(getter)]
	pub fn is_dynamic_entry(&self) -> bool { self.inner.is_dynamic_entry }
	...
	#[napi(getter)]
	pub fn exports(&self) -> Vec<String> { self.inner.exports.clone() }

	// RenderedChunk
	#[napi(getter)]
	pub fn file_name(&self) -> String { self.inner.filename.to_string() }
	...
	// OutputChunk
	#[napi(getter)]
	pub fn code(&self) -> String { self.inner.code.clone() }
	...
	#[napi(getter)]
	pub fn sourcemap_file_name(&self) -> Option<String> { self.inner.sourcemap_filename.clone() }

	#[napi(getter)]
	pub fn preliminary_file_name(&self) -> String { self.inner.preliminary_filename.to_string() }
