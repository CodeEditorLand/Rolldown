...
impl BindingOutputAsset {
	pub fn new(inner:rolldown_common::OutputAsset) -> Self { Self { inner } }
	...
	#[napi(getter)]
	pub fn original_file_names(&self) -> Vec<String> { self.inner.original_file_names.clone() }

	#[napi(getter)]
	pub fn source(&self) -> BindingAssetSource { self.inner.source.clone().into() }

	#[napi(getter)]
	pub fn name(&self) -> Option<String> { self.inner.names.first().cloned() }

	#[napi(getter)]
	pub fn names(&self) -> Vec<String> { self.inner.names.clone() }
}
