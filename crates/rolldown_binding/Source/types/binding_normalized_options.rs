...
impl BindingNormalizedOptions {
	pub fn new(inner:SharedNormalizedBundlerOptions) -> Self { Self { inner } }

	// Notice: rust's HashMap doesn't guarantee the order of keys, so not sure if
	// it's a good idea to expose it to JS directly.
	#[napi(getter)]
					...
					.map(|input| {
						(
							input.name.clone().unwrap_or_else(|| {
								unreachable!(
									"Inputs passed from js side are either all have names or not"
								)
							}),
							input.import.clone(),
						)
	...
	#[napi(getter)]
	pub fn cwd(&self) -> Option<String> { Some(self.inner.cwd.to_string_lossy().to_string()) }
	...
	#[napi(getter)]
	pub fn shim_missing_exports(&self) -> bool { self.inner.shim_missing_exports }

	#[napi(getter)]
	pub fn name(&self) -> Option<String> { self.inner.name.clone() }

	// Some options can be set to `None`, and these values are converted to `null`
	// in JavaScript. To distinguish them from regular None values, `undefined` is
	// used to represent unsupported functions
	#[napi(getter)]
	...
	#[napi(getter)]
	pub fn asset_filenames(&self) -> String { self.inner.asset_filenames.template().to_string() }
	...
	#[napi(getter)]
	pub fn inline_dynamic_imports(&self) -> bool { self.inner.inline_dynamic_imports }
	...
	#[napi(getter)]
	pub fn external_live_bindings(&self) -> bool { self.inner.external_live_bindings }

	#[napi(getter)]
	pub fn extend(&self) -> bool { self.inner.extend }
	...
	pub fn hash_characters(&self) -> String { self.inner.hash_characters.to_string() }

	#[napi(getter)]
	pub fn sourcemap_debug_ids(&self) -> bool { self.inner.sourcemap_debug_ids }

	#[napi(getter)]
	pub fn minify(&self) -> bool { self.inner.minify }

	#[napi(getter)]
	pub fn polyfill_require(&self) -> bool { self.inner.polyfill_require }

	#[napi(getter, ts_return_type = "'none' | 'preserve-legal'")]
	pub fn comments(&self) -> String { self.inner.comments.to_string() }
}
