...
use crate::{
	bundler::{BindingBundlerOptions, Bundler},
	types::{
		binding_watcher_event::BindingWatcherEvent,
		js_callback::{MaybeAsyncJsCallback, MaybeAsyncJsCallbackExt},
	},
	utils::handle_result,
};
...
impl BindingWatcher {
	...
	#[napi]
	pub async fn close(&self) -> napi::Result<()> { handle_result(self.inner.close().await) }
