...
impl BindingWatcher {
	...
	pub async fn start(&self, listener: MaybeAsyncJsCallback<FnArgs<(BindingWatcherEvent,)>, ()>) -> napi::Result<()> {
			...
			while run {
				match rx.recv() {
					Ok(event) => {
						...
						if let Err(e) = listener.await_call(FnArgs { data: (BindingWatcherEvent::new(event),) }).await {
