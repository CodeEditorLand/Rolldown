...
use crate::{
	...
	types::{binding_log::BindingLog, binding_log_level::BindingLogLevel, binding_outputs::BindingOutputs},
	utils::{handle_result, normalize_binding_options::normalize_binding_options, try_init_custom_trace_subscriber},
};
use napi::{Env, tokio::sync::Mutex};
use napi_derive::napi;
use rolldown::{Bundler as NativeBundler, NormalizedBundlerOptions};
use rolldown_error::{BuildDiagnostic, BuildResult, DiagnosticOptions, filter_out_disabled_diagnostics};
...
impl Bundler {
	...
	pub fn new(env: Env, option: BindingBundlerOptions) -> napi::Result<Self> {
		...
		let BindingBundlerOptions { mut input_options, output_options, parallel_plugins_registry } = option;
...
impl Bundler {
	...
	async fn handle_warnings(&self, mut warnings: Vec<BuildDiagnostic>, options: &NormalizedBundlerOptions) {
