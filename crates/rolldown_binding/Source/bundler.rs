...
use crate::{
	...
	utils::{
		handle_result,
		normalize_binding_options::normalize_binding_options,
...
impl Bundler {
	...
	pub fn new(env:Env, option:BindingBundlerOptions) -> napi::Result<Self> {
		...
		Ok(Self {
			...
			inner:Arc::new(Mutex::new(NativeBundler::with_plugins(
				ret.bundler_options,
				ret.plugins,
			))),
			log_level,
	...
	pub async fn write(&self) -> napi::Result<BindingOutputs> { self.write_impl().await }

	#[napi]
	#[tracing::instrument(level = "debug", skip_all)]
	pub async fn generate(&self) -> napi::Result<BindingOutputs> { self.generate_impl().await }

	#[napi]
	#[tracing::instrument(level = "debug", skip_all)]
	pub async fn scan(&self) -> napi::Result<BindingOutputs> { self.scan_impl().await }
...
impl Bundler {
	...
	pub fn into_inner(self) -> Arc<Mutex<NativeBundler>> { self.inner }
