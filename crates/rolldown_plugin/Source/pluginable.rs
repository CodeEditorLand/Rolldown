...
/// `Pluginable` is under the hood trait that rolldown to run. It's not
/// recommended to use this trait directly. To create a plugin, you should use
/// [Plugin] trait instead.
///
/// The main reason we don't expose this trait is that it used `async_trait`,
...
pub trait Pluginable: Any + Debug + Send + Sync + 'static {
	...
	fn call_watch_change_meta(&self) -> Option<PluginHookMeta> { None }

	async fn call_close_watcher(&self, _ctx:&PluginContext) -> HookNoopReturn { Ok(()) }

	fn call_close_watcher_meta(&self) -> Option<PluginHookMeta> { None }

	fn call_transform_filter(&self) -> anyhow::Result<Option<TransformHookFilter>> { Ok(None) }

	fn call_resolve_id_filter(&self) -> anyhow::Result<Option<ResolvedIdHookFilter>> { Ok(None) }

	fn call_load_filter(&self) -> anyhow::Result<Option<LoadHookFilter>> { Ok(None) }
}

#[async_trait::async_trait]
impl<T:Plugin> Pluginable for T {
	fn call_name(&self) -> Cow<'static, str> { Plugin::name(self) }
	...
	fn call_build_start_meta(&self) -> Option<PluginHookMeta> { Plugin::build_start_meta(self) }
	...
	fn call_resolve_id_meta(&self) -> Option<PluginHookMeta> { Plugin::resolve_id_meta(self) }
	...
	fn call_load_meta(&self) -> Option<PluginHookMeta> { Plugin::load_meta(self) }
	...
	fn call_transform_meta(&self) -> Option<PluginHookMeta> { Plugin::transform_meta(self) }
	...
	fn call_module_parsed_meta(&self) -> Option<PluginHookMeta> { Plugin::module_parsed_meta(self) }
	...
	fn call_build_end_meta(&self) -> Option<PluginHookMeta> { Plugin::build_end_meta(self) }
	...
	fn call_render_start_meta(&self) -> Option<PluginHookMeta> { Plugin::render_start_meta(self) }
	...
	fn call_banner_meta(&self) -> Option<PluginHookMeta> { Plugin::banner_meta(self) }
	...
	fn call_footer_meta(&self) -> Option<PluginHookMeta> { Plugin::footer_meta(self) }
	...
	fn call_intro_meta(&self) -> Option<PluginHookMeta> { Plugin::intro_meta(self) }
	...
	fn call_outro_meta(&self) -> Option<PluginHookMeta> { Plugin::outro_meta(self) }
	...
	fn call_render_chunk_meta(&self) -> Option<PluginHookMeta> { Plugin::render_chunk_meta(self) }
	...
	fn call_render_error_meta(&self) -> Option<PluginHookMeta> { Plugin::render_error_meta(self) }
	...
	fn call_write_bundle_meta(&self) -> Option<PluginHookMeta> { Plugin::write_bundle_meta(self) }

	async fn call_close_bundle(&self, ctx:&PluginContext) -> HookNoopReturn {
		Plugin::close_bundle(self, ctx).await
	}

	fn call_close_bundle_meta(&self) -> Option<PluginHookMeta> { Plugin::close_bundle_meta(self) }
	...
	fn call_watch_change_meta(&self) -> Option<PluginHookMeta> { Plugin::watch_change_meta(self) }

	async fn call_close_watcher(&self, ctx:&PluginContext) -> HookNoopReturn {
		Plugin::close_watcher(self, ctx).await
	}

	fn call_close_watcher_meta(&self) -> Option<PluginHookMeta> { Plugin::close_watcher_meta(self) }
	...
	fn call_transform_ast_meta(&self) -> Option<PluginHookMeta> { Plugin::transform_ast_meta(self) }
