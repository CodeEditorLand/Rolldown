...
use crate::{
	...
	types::{
		hook_filter::{
			LoadHookFilter, ResolvedIdHookFilter, TransformHookFilter,
		},
		hook_render_error::HookRenderErrorArgs,
		hook_transform_ast_args::HookTransformAstArgs,
	},
	HookAddonArgs, HookBuildEndArgs, HookInjectionOutputReturn, HookLoadArgs,
...
pub trait Pluginable: Any + Debug + Send + Sync + 'static {
	...
	fn call_transform_filter(
		&self,
	) -> anyhow::Result<Option<TransformHookFilter>> {
		Ok(None)
	}

	fn call_resolve_id_filter(
		&self,
	) -> anyhow::Result<Option<ResolvedIdHookFilter>> {
		Ok(None)
	}
