...
fn is_data_url(s:&str) -> bool { s.trim_start().starts_with("data:") }
	...
	match resolved_id {
			...
			if !resolved_id.is_external {
				...
				if let Some(is_external) = bundle_options.external.as_ref() {
					resolved_id.is_external =
						is_external(resolved_id.id.as_str(), importer, true).await?;
				}
			}
			Ok(Ok(resolved_id))
		},
		...
		Err(e) => {
			if let ResolveError::NotFound(_) = &e {
				// If module can't resolve, check external with unresolved path with
				// `isResolved: true` ref https://github.com/rollup/rollup/blob/master/src/ModuleLoader.ts#L555
				if let Some(is_external) = bundle_options.external.as_ref() {
		...
		match err {
			ResolveError::Builtin { resolved, is_runtime_module } => {
				Ok(Ok(ResolvedId {
					...
					// specification. we needs to use `is_runtime_module` to get the original
					// specifier
