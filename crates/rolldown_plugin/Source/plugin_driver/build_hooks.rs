...
impl PluginDriver {
	...
	pub async fn build_start(&self, opts:&SharedNormalizedBundlerOptions) -> HookNoopReturn {
		...
		//     // `block_on_spawn_all(self.plugins.iter().map(|(plugin, ctx)|
		// plugin.build_start(ctx))).await;` will emit compile errors like     //
		// `implementation of `std::marker::Send` is not general enough`. It seems to
		// be the problem related to HRTB, async and iterator.     // I guess we need
		// some rust experts here.     let mut futures = vec![];
		//     for (_, plugin, ctx) in
	...
	pub async fn transform(
			...
			self.iter_plugin_with_context_by_order(&self.order_by_transform_meta)
		{
				...
				.await?
			{
				...
				if let Some(map) =
					Self::normalize_transform_sourcemap(r.map, id, &code, r.code.as_ref())
				{
	...
	fn normalize_transform_sourcemap(
		...
		if let Some(mut map) = map {
			...
			if original_code == code {
				...
				// If sourcemap is empty and code has changed, need to create one remapping
				// original code. Here using `hires: true` to get more accurate column
				// information, but it has more overhead. TODO: maybe it should be add a
				// option to control hires.
