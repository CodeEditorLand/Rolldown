	...
	pub async fn build_start(&self) -> HookNoopReturn {
		let ret = {
			...
			#[cfg(target_arch = "wasm32")]
			{
				...
				// `implementation of `std::marker::Send` is not general enough`. It seems to be
				// the problem related to HRTB, async and iterator. I guess we need some
				// rust experts here.
				let mut futures = vec![];
				for (_, plugin, ctx) in
					self.iter_plugin_with_context_by_order(&self.order_by_build_start_meta)
