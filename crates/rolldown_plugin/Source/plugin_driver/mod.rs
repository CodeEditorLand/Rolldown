...
impl Deref for PluginDriver {
	...
	fn deref(&self) -> &Self::Target { &self.order_indicates }
}
...
impl HookOrderIndicates {
	pub fn new(index_plugins:&IndexPluginable) -> Self {
		Self {
			...
			order_by_load_meta:Self::sort_plugins_by_hook_meta(index_plugins, |p| {
				p.call_load_meta()
			}),
			...
			order_by_intro_meta:Self::sort_plugins_by_hook_meta(index_plugins, |p| {
				p.call_intro_meta()
			}),
			order_by_outro_meta:Self::sort_plugins_by_hook_meta(index_plugins, |p| {
				p.call_outro_meta()
			}),
