...
use crate::{
	HookAddonArgs, HookBuildEndArgs, HookGenerateBundleArgs, HookLoadArgs, HookLoadOutput, HookRenderChunkArgs,
	HookRenderChunkOutput, HookResolveIdArgs, HookResolveIdOutput, HookTransformArgs, HookWriteBundleArgs,
...
pub trait Plugin: Any + Debug + Send + Sync + 'static {
	...
	fn close_bundle(&self, _ctx: &PluginContext) -> impl std::future::Future<Output = HookNoopReturn> + Send {
		async { Ok(()) }
	}
	...
	fn close_watcher(&self, _ctx: &PluginContext) -> impl std::future::Future<Output = HookNoopReturn> + Send {
		async { Ok(()) }
	}
