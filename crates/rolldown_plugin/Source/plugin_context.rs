...
use rolldown_common::{
	ModuleDefFormat, ModuleInfo, ModuleLoaderMsg, ResolvedId, SharedFileEmitter, SharedNormalizedBundlerOptions,
...
use crate::{
	PluginDriver,
	types::{
		hook_resolve_id_skipped::HookResolveIdSkipped, plugin_context_resolve_options::PluginContextResolveOptions,
...
impl PluginContext {
	#[must_use]
	pub fn new_shared_with_skipped_resolve_calls(&self, skipped_resolve_calls: Vec<Arc<HookResolveIdSkipped>>) -> Self {
...
type LoadCallbackFn =
	dyn Fn() -> Pin<Box<(dyn Future<Output = anyhow::Result<()>> + Send + 'static)>> + Send + Sync + 'static;
...
impl PluginContextImpl {
	pub async fn load(
		...
		if let Some(load_callback_fn) = load_callback_fn {
			self.context_load_modules
				.insert(specifier.into(), LoadCallback(load_callback_fn));
		}
		self
      .tx
      .lock()
	...
	pub fn emit_file(
		...
		let sanitized_file_name = match file.file_name {
			...
			None => Some(
				self.options
					.sanitize_filename
					.value(file.name_for_sanitize(), fn_sanitized_file_name),
			),
		};
	...
	pub async fn emit_file_async(&self, file: rolldown_common::EmittedAsset) -> anyhow::Result<ArcStr> {
		...
		Ok(self
			.file_emitter
			.emit_file(file, asset_filename.map(Into::into), sanitized_file_name))
	}
	...
	pub fn get_module_info(&self, module_id: &str) -> Option<Arc<rolldown_common::ModuleInfo>> {
		self.modules
			.get(module_id)
			.map(|v| Arc::<rolldown_common::ModuleInfo>::clone(v.value()))
	}
