...
impl PluginContextImpl {
	pub async fn load(
		...
		self.context_load_modules
			.insert(specifier.into(), LoadCallback(Box::new(load_callback_fn)));
		...
		self.tx
			...
			.as_ref()
			.context(
				"The `PluginContext.load` only work at `resolveId/load/transform/moduleParsed` \
				 hooks. If you using it at resolveId hook, please make sure it could not load the \
				 entry module.",
			)?
	...
	pub fn add_watch_file(&self, file:&str) { self.watch_files.insert(file.into()); }
}
