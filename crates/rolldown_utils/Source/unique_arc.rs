...
impl<T> UniqueArc<T> {
	...
	pub fn weak_ref(&self) -> WeakRef<T> { WeakRef(Arc::downgrade(&self.0)) }
}
...
impl<T> WeakRef<T> {
	/// This API pattern is intended to prevent users from getting `Arc<T>` and
	/// storing it. This will cause `UniqueArc#into_inner` panic if `Arc<T>`
	/// got stored.
	...
	where
		...
		self.try_with_inner(f).unwrap_or_else(|| {
			let t_name = pretty_type_name::<T>();
			panic!(
				"UniqueArc<{t_name}> is already dropped. You can't access it by this \
				 WeakRef<{t_name}>",
			)
		})
	}
...
impl<T> Clone for WeakRef<T> {
	fn clone(&self) -> Self { Self(Weak::clone(&self.0)) }
}
