...
/// `async` here is only used to satisfy the wasm shim version of
/// `block_on_spawn_all`. This function allow you to spawn non-static futures in
	...
	Iter::Item: Future<Output = Out> + Send,
{
	...
	let (_ret, collections) = async_scoped::Scope::scope_and_block(|scope: &mut TokioScope<'_, _>| {
