...
pub fn filter_code(
	...
	if let Some(exclude) = exclude {
		for pattern in exclude {
			let v = match pattern.as_ref() {
				StringOrRegex::String(pattern) => memmem::find(code.as_bytes(), pattern.as_bytes()).is_some(),
...
mod tests {
	...
	fn test_filter() {
		...
		let cases = [
			...
			TestCases {
				input_filter: InputFilter { exclude: glob_filter("bar.js"), include: glob_filter("foo.js") },
				cases: vec![
					("foo.js", FilterResult::Match(true)),
