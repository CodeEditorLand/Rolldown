...
fn parse_data_url_nom(input: &str) -> IResult<&str, ParsedDataUrl> {
	...
	if let Ok((remaining, data)) = preceded(char::<_, Error<_>>(','), take_while(|_| true)).parse(input) {
		return Ok((
			remaining,
	...
	let (input, _) = opt(preceded(tag(";charset="), take_till(|c| c == ';' || c == ','))).parse(input)?;
...
pub fn encode_as_shortest_dataurl(mime_ext: &MimeExt, buf: &[u8]) -> String {
	...
	match encode_as_percent_escaped(buf).map(|encoded| concat_string!("data:", mime_ext_string, ",", encoded)) {
