...
async function readTestSuiteSource(testSuiteName) {
	...
	try {
		...
		console.log(
			`Attempting to download it from ${testSuite.sourceGithubUrl}.`,
		);
		console.log("...");
...
function processEntryPath(node, binding) {
	...
	try {
		...
		console.error(
			`Error occurred when processEntryPath: ${chalk.red(err)}`,
		);
		return [];
	}
}
	...
	return new Promise((rsl, rej) => {
		nodeHttps.get(
			...
			(resp) => {
				...
				resp.on("error", rej);
				resp.pipe(
					nodeFs.createWriteStream(TREE_SITTER_WASM_GO_FILENAME),
				);
			},
		);
	});
